...
HWTEST_F(TextTimerModifierTest, setTextTimerOptionsTestOptionsIsCountDownValidValues, TestSize.Level1)
{
    ...
    WriteTo(initValueOptions).count =
        ArkValue<Opt_Number>(std::get<1>(Fixtures::testFixtureTimerInputCountValidValues[0]));
...
/*
 * @tc.name: setTextTimerOptionsTestOptionsCountValidValues
 * @tc.desc:
 * @tc.type: FUNC
 */
HWTEST_F(TextTimerModifierTest, setTextTimerOptionsTestOptionsCountValidValues, TestSize.Level1)
{
    Opt_TextTimerOptions initValueOptions;
    ...
    auto checkValue = [this, &initValueOptions](
        ...
        // Re-create node for 'options' attribute
        auto node = CreateNode();
        WriteTo(inputValueOptions).count = value;
        modifier_->setTextTimerOptions(node, &inputValueOptions);
        auto jsonValue = GetJsonValue(node);
        auto resultStr = GetAttrValue<std::string>(jsonValue, ATTRIBUTE_COUNT_NAME);
        DisposeNode(node);
        EXPECT_EQ(resultStr, expectedStr) <<
            "Input value is: " << input << ", method: setTextTimerOptions, attribute: options.count";
    };
...
/*
 * @tc.name: setTextTimerOptionsTestOptionsCountInvalidValues
 * @tc.desc:
 * @tc.type: FUNC
 */
HWTEST_F(TextTimerModifierTest, setTextTimerOptionsTestOptionsCountInvalidValues, TestSize.Level1)
{
    Opt_TextTimerOptions initValueOptions;
