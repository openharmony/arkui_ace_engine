...
HWTEST_F(TextPickerModifierTest, setTextPickerOptionsAsStringArray, TestSize.Level1)
{
    ...
    for (auto&& value: textPickerOptionsAsStringArray) {
        ...
        if (std::get<HAS_VALUES_ID>(value)) {
            ...
            arkTextPickerOptions.value = Converter::ArkUnion<Opt_Union_ResourceStr_Array_ResourceStr,
                Ark_ResourceStr>(arkValue);
        } else {
            arkTextPickerOptions.value = Converter::ArkUnion<Opt_Union_ResourceStr_Array_ResourceStr>(Ark_Empty());
        }
