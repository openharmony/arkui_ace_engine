 ...
 * @tc.type: FUNC
 */
HWTEST_F(LongPressGestureIntrfaceAccessorTest, OnActionCancelTest, TestSize.Level1)
{
    ASSERT_TRUE(peer_ && peer_->gesture && accessor_);
    static std::optional<CheckEvent> checkEvent = std::nullopt;
    ...
    auto callResult =
        reinterpret_cast<MockLongPressGesture*>(Referenced::RawPtr(peer_->gesture))->HandleOnActionCancel();
    ASSERT_TRUE(callResult);
    ASSERT_TRUE(checkEvent.has_value());
    EXPECT_EQ(checkEvent->resourceId, ID);
}
