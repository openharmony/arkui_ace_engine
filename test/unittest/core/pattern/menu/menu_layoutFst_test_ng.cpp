...
HWTEST_F(MenuLayout1TestNg, MenuLayoutAlgorithmTestNg049, TestSize.Level1)
{
    ...
    int32_t creaseYTop = 1064;
    layoutAlgorithm->targetOffset_ = OffsetF(100, 100);
    layoutAlgorithm->position_ = OffsetF(100, 100);
    layoutAlgorithm->SetWrapperRect(property, menuPattern);

    auto top = layoutAlgorithm->dumpInfo_.top;
    auto left = layoutAlgorithm->dumpInfo_.left;
    auto right = layoutAlgorithm->dumpInfo_.right;
    auto width = layoutAlgorithm->dumpInfo_.width;
...
HWTEST_F(MenuLayout1TestNg, MenuLayoutAlgorithmTestNg050, TestSize.Level1)
{
    ...
    int32_t creaseYBottom = 1235;

    layoutAlgorithm->targetOffset_ = OffsetF(100, 1400);
    layoutAlgorithm->position_ = OffsetF(100, 1400);
    layoutAlgorithm->SetWrapperRect(property, menuPattern);

    auto bottom = layoutAlgorithm->dumpInfo_.bottom;
    auto left = layoutAlgorithm->dumpInfo_.left;
    auto right = layoutAlgorithm->dumpInfo_.right;
    auto width = layoutAlgorithm->dumpInfo_.width;
    auto height = layoutAlgorithm->dumpInfo_.height;
 ...
 * @tc.desc: Verify menu component is in the crease area of the screen in the hover state.
 * @tc.type: FUNC
 */

HWTEST_F(MenuLayout1TestNg, MenuLayoutAlgorithmTestNg051, TestSize.Level1)
{
    MockPipelineContextGetTheme();
    std::vector<OptionParam> optionParams;
    MenuParam menuParam;
    ...
    layoutAlgorithm->targetOffset_ = OffsetF(100, 1100);
    layoutAlgorithm->position_ = OffsetF(100, 1100);
    layoutAlgorithm->SetWrapperRect(property, menuPattern);
