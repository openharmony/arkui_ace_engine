...
/**
 * @tc.name: ComputeSheetTypeInSubWindow009
 * @tc.desc: Branch:   if (container->IsUIExtensionWindow())
 *                     if (manager && manager->IsPcOrPadFreeMultiWindowMode())
 *                     if (sheetStyle.sheetType.has_value() &&
 *                       sheetStyle.sheetType.value() == SheetType::SHEET_POPUP && sheetKey_.hasValidTargetNode)
 ...
 *                      sheetStyle.sheetType.value() == SheetType::SHEET_BOTTOM  = false
 * @tc.type: FUNC
 */
HWTEST_F(SheetPresentationTestFiveNg, ComputeSheetTypeInSubWindow009, TestSize.Level1)
{
    /**
     * @tc.steps: step1. create sheet page.
     */
    SheetPresentationTestFiveNg::SetUpTestCase();
    auto callback = [](const std::string&) {};
    auto sheetNode = FrameNode::CreateFrameNode("Sheet", 126,
        AceType::MakeRefPtr<SheetPresentationPattern>(226, "SheetPresentation", std::move(callback)));
    ASSERT_NE(sheetNode, nullptr);
    auto sheetPattern = sheetNode->GetPattern<SheetPresentationPattern>();
    ASSERT_NE(sheetPattern, nullptr);
