...
/**
 * @tc.name: CreateArcScrollBar001
 * @tc.desc: Test ScrollBarModelNG Create with isCreateArc=true on WATCH device
 * @tc.type: FUNC
 */
HWTEST_F(ScrollBarTestNg, CreateArcScrollBar001, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Set device type to WATCH and create Arc ScrollBar
     * @tc.expected: ArcScrollBarPattern is created
     */
    ...
    int32_t directionValue = static_cast<int>(Axis::VERTICAL);
    int32_t stateValue = static_cast<int>(DisplayMode::ON);
    scrollBarModel.Create(scrollBarProxy, true, true, directionValue, stateValue, true);
    GetScrollBar();
    CreateDone();

    ASSERT_NE(frameNode_, nullptr);
    EXPECT_EQ(frameNode_->GetTag(), V2::ARC_SCROLL_BAR_ETS_TAG);

    auto arcPattern = frameNode_->GetPattern<ArcScrollBarPattern>();
    EXPECT_NE(arcPattern, nullptr);
}

/**
 * @tc.name: CreateArcScrollBar002
 * @tc.desc: Test ScrollBarModelNG Create with isCreateArc=true on WEARABLE device
 * @tc.type: FUNC
 */
HWTEST_F(ScrollBarTestNg, CreateArcScrollBar002, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Set device type to WEARABLE and create Arc ScrollBar
     * @tc.expected: ArcScrollBarPattern is created
     */
    Container::Current()->SetApiTargetVersion(static_cast<int32_t>(PlatformVersion::VERSION_TWELVE));
    SystemProperties::SetDeviceType(DeviceType::WEARABLE);
    CreateStack();
    CreateScroll();
    ScrollBarModelNG scrollBarModel;
    auto scrollBarProxy = scrollPattern_->GetScrollBarProxy();
...
/**
 * @tc.name: CreateArcScrollBar003
 * @tc.desc: Test ScrollBarModelNG Create with isCreateArc=false on PHONE device
 * @tc.type: FUNC
 */
HWTEST_F(ScrollBarTestNg, CreateArcScrollBar003, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Set device type to PHONE, create regular ScrollBar (isCreateArc=false)
     * @tc.expected: Regular ScrollBarPattern is created
     */
    Container::Current()->SetApiTargetVersion(static_cast<int32_t>(PlatformVersion::VERSION_TWELVE));
    ...
    int32_t directionValue = static_cast<int>(Axis::VERTICAL);
    int32_t stateValue = static_cast<int>(DisplayMode::ON);
    scrollBarModel.Create(scrollBarProxy, true, true, directionValue, stateValue, false);
    GetScrollBar();
    CreateDone();

    ASSERT_NE(frameNode_, nullptr);
    EXPECT_EQ(frameNode_->GetTag(), V2::SCROLL_BAR_ETS_TAG);

    auto scrollBarPattern = frameNode_->GetPattern<ScrollBarPattern>();
    EXPECT_NE(scrollBarPattern, nullptr);

    auto arcPattern = frameNode_->GetPattern<ArcScrollBarPattern>();
    EXPECT_EQ(arcPattern, nullptr);
}

/**
 * @tc.name: CreateArcScrollBar004
 * @tc.desc: Test ScrollBarModelNG Create with isCreateArc=false on WATCH device
 * @tc.type: FUNC
 */
HWTEST_F(ScrollBarTestNg, CreateArcScrollBar004, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Set device type to WATCH but isCreateArc=false
     * @tc.expected: Regular ScrollBarPattern is created (not Arc)
     */
    Container::Current()->SetApiTargetVersion(static_cast<int32_t>(PlatformVersion::VERSION_TWELVE));
    SystemProperties::SetDeviceType(DeviceType::WATCH);
    CreateStack();
...
/**
 * @tc.name: CreateArcScrollBar005
 * @tc.desc: Test ScrollBarModelNG Create with isCreateArc=false on TABLET device
 * @tc.type: FUNC
 */
HWTEST_F(ScrollBarTestNg, CreateArcScrollBar005, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Set device type to TABLET, create regular ScrollBar (isCreateArc=false)
     * @tc.expected: Regular ScrollBarPattern is created
     */
    Container::Current()->SetApiTargetVersion(static_cast<int32_t>(PlatformVersion::VERSION_TWELVE));
    ...
    int32_t directionValue = static_cast<int>(Axis::HORIZONTAL);
    int32_t stateValue = static_cast<int>(DisplayMode::AUTO);
    scrollBarModel.Create(scrollBarProxy, true, true, directionValue, stateValue, false);
    GetScrollBar();
    CreateDone();
...
/**
 * @tc.name: CreateArcScrollBar006
 * @tc.desc: Test ScrollBarModelNG Create with isCreateArc and different axis/mode on WATCH
 * @tc.type: FUNC
 */
HWTEST_F(ScrollBarTestNg, CreateArcScrollBar006, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Test with WATCH device and HORIZONTAL axis, AUTO mode
     * @tc.expected: ArcScrollBarPattern is created with correct properties
     */
    Container::Current()->SetApiTargetVersion(static_cast<int32_t>(PlatformVersion::VERSION_TWELVE));
    SystemProperties::SetDeviceType(DeviceType::WATCH);
    CreateStack();
    ...
    auto arcPattern = frameNode_->GetPattern<ArcScrollBarPattern>();
    ASSERT_NE(arcPattern, nullptr);

    auto layoutProperty = frameNode_->GetLayoutProperty<ScrollBarLayoutProperty>();
    ASSERT_NE(layoutProperty, nullptr);
    EXPECT_EQ(layoutProperty->GetAxisValue(), Axis::HORIZONTAL);
    EXPECT_EQ(layoutProperty->GetDisplayModeValue(), DisplayMode::AUTO);
}
} // namespace OHOS::Ace::NG
