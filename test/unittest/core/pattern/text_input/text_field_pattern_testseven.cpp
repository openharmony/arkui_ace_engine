 ...
 * @tc.desc: Test TextFieldPattern GetTextBoxesForSelect
 * @tc.type: FUNC
 */
HWTEST_F(TextFieldPatternTestSeven, GetTextBoxesForSelect001, TestSize.Level0)
{
    auto textFieldNode = FrameNode::GetOrCreateFrameNode(V2::TEXTINPUT_ETS_TAG,
        ElementRegister::GetInstance()->MakeUniqueId(), []() { return AceType::MakeRefPtr<TextFieldPattern>(); });
    ASSERT_NE(textFieldNode, nullptr);
    ...
    pattern->paragraph_ = MockParagraph::GetOrCreateMockParagraph();
    pattern->textRect_.height_ = 150.0f;
    auto rectVector = pattern->GetTextBoxesForSelect();
    ASSERT_EQ(rectVector.empty(), true);
}
