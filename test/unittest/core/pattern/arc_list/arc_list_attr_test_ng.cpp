...
HWTEST_F(ArcListAttrTestNg, AttrInitIndex001, TestSize.Level1)
{
    ...
    EXPECT_EQ(pattern_->GetTotalOffset() + MID_OFFSET, 0.f);
}
...
HWTEST_F(ArcListAttrTestNg, AttrInitIndex002, TestSize.Level1)
{
    ...
    EXPECT_EQ(GetChildY(frameNode_, 1), 0.f + MID_OFFSET);
    EXPECT_EQ(pattern_->GetTotalOffset() + MID_OFFSET, ITEM_HEIGHT);
}
...
HWTEST_F(ArcListAttrTestNg, AttrInitIndex004, TestSize.Level1)
{
    ...
    EXPECT_TRUE(NearEqual(GetChildX(frameNode_, 2), LIST_WIDTH * (1 - ARC_LIST_ITER_SCALE) / 2.0));
    EXPECT_EQ(pattern_->GetTotalOffset() + MID_OFFSET, ITEM_HEIGHT * 5);
}
...
HWTEST_F(ArcListAttrTestNg, AttrInitIndex006, TestSize.Level1)
{
    ...
    CreateDone(frameNode_);
    EXPECT_EQ(GetChildY(frameNode_, 2), 50.f);
    EXPECT_EQ(GetChildY(frameNode_, 3), 150.f);
}
...
HWTEST_F(ArcListAttrTestNg, AttrScrollSnapAlign001, TestSize.Level1)
{
    ...
    EXPECT_EQ(scrollableDistance, -LIST_HEIGHT / 2.f);
...
HWTEST_F(ArcListAttrTestNg, AttrScrollSnapAlign002, TestSize.Level1)
{
    ...
    EXPECT_EQ(pattern_->GetTotalOffset(), -endPos);
...
HWTEST_F(ArcListAttrTestNg, AttrScrollSnapAlign003, TestSize.Level1)
{
    ...
    float scrollableDistance = pattern_->GetScrollableDistance();
    EXPECT_EQ(scrollableDistance, -200.f);
    ...
    EXPECT_EQ(pattern_->GetTotalOffset(), -ARC_LIST_ITEM_SNAP_SIZE / 2.0);
}
...
HWTEST_F(ArcListAttrTestNg, AttrScrollSnapAlign004, TestSize.Level1)
{
    ...
    endPos -= ARC_LIST_ITEM_SNAP_SIZE;
    ScrollSnap(-51.f, 1200.f);
    EXPECT_EQ(pattern_->GetTotalOffset(), -endPos); // 100.f
}
...
HWTEST_F(ArcListAttrTestNg, GetMidIndex002, TestSize.Level1)
{
     ...
     * @tc.expected: mid index is 0
     */
    EXPECT_EQ(pattern_->GetMidIndex(), 0);
}
...
HWTEST_F(ArcListAttrTestNg, GetOneItemSnapPosByFinalPos001, TestSize.Level1)
{
    ...
    float finalPos = 10.0;
    float snapPos = 0.0;
    pattern_->OnScrollCallback(100.0, SCROLL_FROM_START);
...
HWTEST_F(ArcListAttrTestNg, GetOneItemSnapPosByFinalPos002, TestSize.Level1)
{
    ...
    EXPECT_TRUE(pattern_->GetOneItemSnapPosByFinalPos(mainPos, finalPos, snapPos));
}
...
HWTEST_F(ArcListAttrTestNg, GetItemSnapPosition001, TestSize.Level1)
{
    ...
    for (int32_t i = 0; i < count; ++i) {
        ...
        EXPECT_EQ(snapInfo.snapLow, MID_OFFSET + ARC_LIST_ITEM_SNAP_SIZE / 2.0 + i * ITEM_HEIGHT);
        EXPECT_EQ(snapInfo.snapHigh, MID_OFFSET + ITEM_HEIGHT - ARC_LIST_ITEM_SNAP_SIZE / 2.0 + i * ITEM_HEIGHT);
        EXPECT_EQ(snapInfo.moveThreshold, ARC_LIST_ITEM_SNAP_SIZE * ARC_LIST_ITEM_MOVE_THRESHOLD_RATIO);
    }
}
...
HWTEST_F(ArcListAttrTestNg, GetItemSnapPosition002, TestSize.Level1)
{
    ...
    for (int32_t i = 0; i < count; ++i) {
        pattern_->GetItemSnapPosition(i, snapInfo);
        EXPECT_EQ(snapInfo.snapLow, LIST_HEIGHT / 2.0 + i * itemHeight);
        EXPECT_EQ(snapInfo.snapHigh, LIST_HEIGHT / 2.0 + i * itemHeight);
        EXPECT_EQ(snapInfo.moveThreshold, itemHeight * ARC_LIST_ITEM_MOVE_THRESHOLD_RATIO);
    }
}
...
HWTEST_F(ArcListAttrTestNg, FadingEdge001, TestSize.Level1)
{
    ...
    CreateListItems(1);
    CreateDone(frameNode_);
    EXPECT_TRUE(layoutProperty_->GetFadingEdgeValue(true));
}
...
HWTEST_F(ArcListAttrTestNg, FadingEdge002, TestSize.Level1)
{
    ...
    EXPECT_EQ(pattern_->endMainPos_, 540.f);
