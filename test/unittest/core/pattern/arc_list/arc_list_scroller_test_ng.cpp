...
HWTEST_F(ArcListScrollerTestNg, ScrollToIndex009, TestSize.Level1)
{
    ...
    CreateDone(frameNode_);
    float scrollableDistance = pattern_->GetScrollableDistance();
    EXPECT_EQ(scrollableDistance, 800.f);
    int32_t index = 8;
    EXPECT_TRUE(ScrollToIndex(index, false, ScrollAlign::START, scrollableDistance - MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, false, ScrollAlign::CENTER, scrollableDistance - MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, false, ScrollAlign::END, scrollableDistance - MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, false, ScrollAlign::AUTO, scrollableDistance - MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, true, ScrollAlign::START, -MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, true, ScrollAlign::CENTER, -MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, true, ScrollAlign::END, -MID_OFFSET));
    EXPECT_TRUE(ScrollToIndex(index, true, ScrollAlign::AUTO, -MID_OFFSET));
...
HWTEST_F(ArcListScrollerTestNg, PositionController001, TestSize.Level1)
{
    ...
    EXPECT_EQ(controller->GetCurrentOffset(), Offset(0.0, -150.0));
...
HWTEST_F(ArcListScrollerTestNg, Pattern012, TestSize.Level1)
{
    ...
    EXPECT_EQ(json->GetDouble("itemStartPos"), MID_OFFSET);
...
HWTEST_F(ArcListScrollerTestNg, Pattern013, TestSize.Level1)
{
    ...
    EXPECT_TRUE((GreatNotEqual(pattern_->GetTotalOffset(), 400)));
    ...
    ScrollTo(1200);
    ScrollToIndex(1, true, ScrollAlign::AUTO);
    EXPECT_TRUE((GreatNotEqual(pattern_->GetTotalOffset(), 1200)));
    ...
    EXPECT_EQ(pattern_->GetTotalOffset(), 400 * ARC_LIST_ITER_SCALE);
}
...
HWTEST_F(ArcListScrollerTestNg, Pattern017, TestSize.Level1)
{
    ...
    EXPECT_TRUE(pattern_->endIndex_ > 3);
    EXPECT_EQ(pattern_->currentOffset_, -MID_OFFSET);

    pattern_->ScrollPage(false, false);
    FlushLayoutTask(frameNode_);
    EXPECT_EQ(pattern_->startIndex_, 0);
    EXPECT_EQ(pattern_->endIndex_, 8);
    EXPECT_EQ(pattern_->currentOffset_, -MID_OFFSET + 400);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(ArcListScrollerTestNg, ListPattern_UpdateScrollSnap001, TestSize.Level1)
{
    CreateList();
    CreateListItems(TOTAL_ITEM_NUMBER);
    CreateDone(frameNode_);
