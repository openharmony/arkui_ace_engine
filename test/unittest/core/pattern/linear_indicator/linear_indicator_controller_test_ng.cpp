...
constexpr int32_t ANIMATION_TIME = 0;
constexpr int32_t INTERVAL_TIME = -1;
constexpr int32_t LINEAR_INITIAL_INDEX = 3;
constexpr int32_t LINEAR_INITIAL_PROGRESS = 10;
...
HWTEST_F(LinearIndicatorControllerTestNg, LinearIndicatorController001, TestSize.Level1)
{
    ...
    auto onChange = [&currentIndex](int index, float progress) { currentIndex = index; };
    ...
    controller_->Start(ANIMATION_TIME, INTERVAL_TIME);
    ...
    controller_->Start(ANIMATION_TIME + 1, INTERVAL_TIME + 1);
    EXPECT_EQ(controller_->animationData_.State(), STATE::ANIMATION);
    controller_->Start(ANIMATION_TIME + 2, INTERVAL_TIME + 2);
    controller_->Stop();
...
HWTEST_F(LinearIndicatorControllerTestNg, LinearIndicatorController002, TestSize.Level1)
{
    ...
    controller_->Start(ANIMATION_TIME, INTERVAL_TIME);
    EXPECT_EQ(controller_->animationData_.State(), STATE::ANIMATION);
    controller_->SetProgress(LINEAR_INITIAL_INDEX, LINEAR_INITIAL_PROGRESS);
    controller_->UpdateProgressSize(LINEAR_INITIAL_INDEX - 1);
    controller_->StartProgressInterval(INTERVAL_TIME);
    EXPECT_EQ(controller_->animationData_.State(), STATE::INTERVAL);
    controller_->Start(ANIMATION_TIME + 3, INTERVAL_TIME + 2);
    controller_->Pause();
...
HWTEST_F(LinearIndicatorControllerTestNg, LinearIndicatorController003, TestSize.Level1)
{
    ...
    controller_->SetProgress(LINEAR_INITIAL_INDEX - 1, LINEAR_INITIAL_PROGRESS - 2);
    EXPECT_EQ(controller_->animationData_.State(), STATE::INTERVAL);
    controller_->Start(ANIMATION_TIME + 1, INTERVAL_TIME + 10);
