...
HWTEST_F(EventHubTestNg, EventHubFrameNodeTest003, TestSize.Level1)
{
     ...
     */
    std::function<void()> flagFunc2 = []() { ++flag; };
    eventHub->SetOnAppear(std::move(flagFunc2));
    eventHub->FireOnAppear();
    EXPECT_NE(eventHub->onAppear_, nullptr);
}
...
/**
 * @tc.name: EventHubTest032
 * @tc.desc: Test AddHoverListener when stateStyleMgr_ is nullptr
 * @tc.type: FUNC
 */
HWTEST_F(EventHubTestNg, EventHubTest032, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Create EventHub and FrameNode.
     * @tc.expected: eventHub and frameNode are not null.
     */
    ...
    /**
     * @tc.steps: step2. Reset stateStyleMgr_ to nullptr.
     */
    eventHub->stateStyleMgr_ = nullptr;

    /**
     * @tc.steps: step3. Call AddHoverListener when stateStyleMgr_ is nullptr.
     * @tc.expected: Should not add hover listener, inputEventHub remains null.
     */
    eventHub->AddHoverListener();
    EXPECT_EQ(eventHub->stateStyleMgr_, nullptr);
    EXPECT_EQ(eventHub->GetInputEventHub(), nullptr);
}

/**
 * @tc.name: EventHubTest033
 * @tc.desc: Test AddHoverListener when stateStyleMgr_ exists but has no UI_STATE_HOVERED
 * @tc.type: FUNC
 */
HWTEST_F(EventHubTestNg, EventHubTest033, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Create EventHub and FrameNode.
     * @tc.expected: eventHub and frameNode are not null.
    ...
    /**
     * @tc.steps: step2. Set supported state to UI_STATE_PRESSED (not UI_STATE_HOVERED).
     */
    eventHub->AddSupportedState(UI_STATE_PRESSED);
    ASSERT_NE(eventHub->stateStyleMgr_, nullptr);

    /**
     * @tc.steps: step3. Call AddHoverListener when stateStyleMgr_ has no UI_STATE_HOVERED.
     * @tc.expected: Should not add hover listener due to HasStateStyle returning false.
     */
    eventHub->AddHoverListener();
    EXPECT_FALSE(eventHub->stateStyleMgr_->HasStateStyle(UI_STATE_HOVERED));
}

/**
 * @tc.name: EventHubTest034
 * @tc.desc: Test AddHoverListener when stateStyleMgr_ exists and has UI_STATE_HOVERED
 * @tc.type: FUNC
 */
HWTEST_F(EventHubTestNg, EventHubTest034, TestSize.Level1)
{
    /**
     * @tc.steps: step1. Create EventHub and FrameNode.
     * @tc.expected: eventHub and frameNode are not null.
     ...
     */
    eventHub->AddSupportedState(UI_STATE_HOVERED);
    ASSERT_NE(eventHub->stateStyleMgr_, nullptr);
    EXPECT_TRUE(eventHub->stateStyleMgr_->HasStateStyle(UI_STATE_HOVERED));

    /**
     * @tc.steps: step3. Call AddHoverListener when stateStyleMgr_ has UI_STATE_HOVERED.
     * @tc.expected: Should add hover listener to inputEventHub.
     */
    eventHub->AddHoverListener();
