...
HWTEST_F(GestureEventHubTestCoverageNg, GestureEventHubTestCollectRecognizers001, TestSize.Level1)
{
    ...
    gestureEventHub->ProcessParallelPriorityGesture(offset, touchId, targetComponent, host, current, recognizers, parallelIndex);
    auto touchPoint = clickRecognizer->GetTouchPoints();
    EXPECT_EQ(touchPoint.size(), 1);
    EXPECT_EQ(parallelIndex, 0);
    EXPECT_EQ(current, recognizer);
}
} // namespace OHOS::Ace::NG