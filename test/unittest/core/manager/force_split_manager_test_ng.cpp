...
/**
 * @tc.name: SetDialogSupportSplit001
 * @tc.desc: Test SetDialogSupportSplit sets dialogSupportSplit_
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, SetDialogSupportSplit001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->SetDialogSupportSplit(true);
    EXPECT_TRUE(manager->GetDialogSupportSplit());

    manager->SetDialogSupportSplit(false);
    EXPECT_FALSE(manager->GetDialogSupportSplit());
}

/**
 * @tc.name: GetDialogSupportSplit001
 * @tc.desc: Test GetDialogSupportSplit returns correct value
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, GetDialogSupportSplit001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->dialogSupportSplit_ = true;
    EXPECT_TRUE(manager->GetDialogSupportSplit());

    manager->dialogSupportSplit_ = false;
    EXPECT_FALSE(manager->GetDialogSupportSplit());
}

/**
 * @tc.name: IsFullScreenPage001
 * @tc.desc: Test IsFullScreenPage checks if page is in fullScreenPages_
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, IsFullScreenPage001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    std::unordered_set<std::string> pages = { "page1", "page2" };
    manager->SetFullScreenPages(std::move(pages));

    EXPECT_TRUE(manager->IsFullScreenPage("page1"));
    EXPECT_TRUE(manager->IsFullScreenPage("page2"));
    EXPECT_FALSE(manager->IsFullScreenPage("page3"));
}

/**
 * @tc.name: IsFullScreenPage002
 * @tc.desc: Test IsFullScreenPage with empty fullScreenPages_
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, IsFullScreenPage002, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->fullScreenPages_.clear();
    EXPECT_FALSE(manager->IsFullScreenPage("anyPage"));
}

/**
 * @tc.name: SetHomePageName001
 * @tc.desc: Test SetHomePageName sets homePageName_
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, SetHomePageName001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->SetHomePageName("home");
    EXPECT_EQ(manager->GetHomePageName(), "home");
}

/**
 * @tc.name: GetHomePageName001
 * @tc.desc: Test GetHomePageName returns correct value
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, GetHomePageName001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->homePageName_ = "testPage";
    EXPECT_EQ(manager->GetHomePageName(), "testPage");
}

/**
 * @tc.name: SetRelatedPageName001
 * @tc.desc: Test SetRelatedPageName sets relatedPageName_
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, SetRelatedPageName001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->SetRelatedPageName("detail");
    EXPECT_EQ(manager->GetRelatedPageName(), "detail");
}

/**
 * @tc.name: GetRelatedPageName001
 * @tc.desc: Test GetRelatedPageName returns correct value
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, GetRelatedPageName001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->relatedPageName_ = "testDetail";
    EXPECT_EQ(manager->GetRelatedPageName(), "testDetail");
}

/**
 * @tc.name: HasRelatedPage001
 * @tc.desc: Test HasRelatedPage returns true when relatedPageName_ is not empty
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, HasRelatedPage001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->relatedPageName_ = "detail";
    EXPECT_TRUE(manager->HasRelatedPage());
...
/**
 * @tc.name: HasSetForceSplitConfig001
 * @tc.desc: Test HasSetForceSplitConfig returns correct value
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, HasSetForceSplitConfig001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->hasSetForceSplitConfig_ = false;
    EXPECT_FALSE(manager->HasSetForceSplitConfig());

    manager->hasSetForceSplitConfig_ = true;
    EXPECT_TRUE(manager->HasSetForceSplitConfig());
}

/**
 * @tc.name: NotifyForceFullScreenChange001
 * @tc.desc: Test NotifyForceFullScreenChange with true
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, NotifyForceFullScreenChange001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->NotifyForceFullScreenChange(true);
    EXPECT_TRUE(true);
}

/**
 * @tc.name: NotifyForceFullScreenChange002
 * @tc.desc: Test NotifyForceFullScreenChange with false
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, NotifyForceFullScreenChange002, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->NotifyForceFullScreenChange(false);
    EXPECT_TRUE(true);
}

/**
 * @tc.name: SetBehaviorMode001
 * @tc.desc: Test SetBehaviorMode sets behaviorMode_
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, SetBehaviorMode001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);

    manager->SetBehaviorMode(ForceSplitBehaviorMode::NAVIGATION);
    EXPECT_EQ(manager->GetBehaviorMode(), ForceSplitBehaviorMode::NAVIGATION);

    manager->SetBehaviorMode(ForceSplitBehaviorMode::DISPLACE);
    EXPECT_EQ(manager->GetBehaviorMode(), ForceSplitBehaviorMode::DISPLACE);
}

/**
 * @tc.name: GetBehaviorMode001
 * @tc.desc: Test GetBehaviorMode returns correct value
 * @tc.type: FUNC
 */
HWTEST_F(ForceSplitManagerTestNg, GetBehaviorMode001, TestSize.Level1)
{
    auto manager = GetForceSplitManager();
    ASSERT_NE(manager, nullptr);
