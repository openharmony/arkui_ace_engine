...
public:
    ...
    NG::SafeAreaInsets navArea =
        NG::SafeAreaInsets({ NAV_LEFT_START, NAV_LEFT_END }, { NAV_TOP_START, NAV_TOP_END },
    ...
    NG::SafeAreaInsets navAreaNotValid =
        NG::SafeAreaInsets({ NAV_LEFT_END, NAV_LEFT_START }, { NAV_TOP_END, NAV_TOP_START },
            { NAV_RIGHT_END, NAV_RIGHT_START }, { NAV_BOTTOM_END, NAV_BOTTOM_START });
};
...
HWTEST_F(SafeAreaManagerTest, SafeAreaToPaddingTest, TestSize.Level1)
{
    ...
    auto funExpect = [this](bool withoutProcess, float left, float right, float top, float bottom) {
        ...
        CommonExpectEQ(Rect{ret.left.value_or(0.0f), ret.right.value_or(0.0f), ret.top.value_or(0.0f),
            ret.bottom.value_or(0.0f)}, Rect{left, right, top, bottom});
    };
    ...
    EXPECT_EQ(ret1, ret);
    CommonExpectEQ(Rect{ret.left.value_or(0.0f), ret.right.value_or(0.0f), ret.top.value_or(0.0f),
            ret.bottom.value_or(0.0f)}, Rect{NAV_LEFT_END-SYSTEM_LEFT_START, SYSTEM_RIGHT_END-NAV_RIGHT_START,
            NAV_TOP_END-SYSTEM_TOP_START, SYSTEM_BOTTOM_END-NAV_BOTTOM_START});
...
HWTEST_F(SafeAreaManagerTest, NodesTest, TestSize.Level1)
{
    ...
    auto frameNode0 = FrameNode::CreateFrameNode(V2::PAGE_ETS_TAG, 0,
    ...
    auto frameNode1 = FrameNode::CreateFrameNode(V2::PAGE_ETS_TAG, 1,
                        AceType::MakeRefPtr<PagePattern>(AceType::MakeRefPtr<PageInfo>()), true);
    frameNode1->SetRootNodeType(RootNodeType::PAGE_ETS_TAG);
    frameNode1->SetRootNodeId(0);
    auto frameNode2 = FrameNode::CreateFrameNode(V2::PAGE_ETS_TAG, 2,
                        AceType::MakeRefPtr<PagePattern>(AceType::MakeRefPtr<PageInfo>()), false);
    frameNode2->SetRootNodeType(RootNodeType::NAVDESTINATION_VIEW_ETS_TAG);
    frameNode2->SetRootNodeId(0);
    auto frameNode3 = FrameNode::CreateFrameNode(V2::NAVDESTINATION_VIEW_ETS_TAG, 3,
                        AceType::MakeRefPtr<PagePattern>(AceType::MakeRefPtr<PageInfo>()), false);
    frameNode3->SetRootNodeType(RootNodeType::NAVDESTINATION_VIEW_ETS_TAG);
    frameNode3->SetRootNodeId(0);
    auto frameNode4 = FrameNode::CreateFrameNode(V2::CHECK_BOX_ETS_TAG, 4,
                        AceType::MakeRefPtr<CheckBoxPattern>());
