...
/**
 * @tc.name: EndStateEventGenerateTimeout001
 * @tc.desc: EndState::EventGenerate with timeout generates UP event and transitions to IDLE.
 * @tc.type: FUNC
 */
HWTEST_F(CompatibleManagerTestNg, EndStateEventGenerateTimeout001, TestSize.Level1)
{
    CompatibleManager mgr;
    GoToEndState(mgr);
    // Backdate rawDownEvent so IsTimeout returns true.
    mgr.rawDownEvent_->SetTime(std::chrono::high_resolution_clock::now() - std::chrono::milliseconds(400));
    auto event = mgr.EventGenerate();
    EXPECT_TRUE(event.has_value());
    EXPECT_EQ(mgr.GetCurrentStateType(), StateType::IDLE);
}
