...
FrameNode* ViewStackProcessor::GetMainFrameNode() const
{
    ...
    return static_cast<FrameNode*>(AceType::RawPtr(elementsStack_.top()));
}

void ViewStackProcessor::Push(const RefPtr<UINode>& node, bool /* isOwner */)
{
    if (node) {
        elementsStack_.push(node);
    }
}
} // namespace OHOS::Ace::NG
