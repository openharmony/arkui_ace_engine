...
bool MouseStyleOhos::SetPointerStyle(int32_t windowId, MouseFormat pointerStyle) const
{
    ...
    int32_t setResult = -1;
    if (isUIExtension) {
        auto winToken = container->GetToken();
        if (winToken == nullptr) {
            TAG_LOGE(AceLogTag::ACE_MOUSE, "SetPointerStyle winToken is null!");
            return false;
        }
        setResult = inputManager->SetPointerStyle(windowId, style, winToken);
    } else {
        setResult = inputManager->SetPointerStyle(windowId, style);
    }
    if (setResult == -1) {
...
int32_t MouseStyleOhos::GetPointerStyle(int32_t windowId, int32_t& pointerStyle) const
{
    ...
    if (isUIExtension) {
        auto winToken = container->GetToken();
        if (winToken == nullptr) {
            TAG_LOGE(AceLogTag::ACE_MOUSE, "GetPointerStyle winToken is null!");
            return -1;
        }
        getResult = inputManager->GetPointerStyle(windowId, style, winToken);
    } else {
        getResult = inputManager->GetPointerStyle(windowId, style);
    }
...
void MouseStyleOhos::SetUeaCustomCursor(
    int32_t realHostWindowId, RefPtr<PixelMap> pixelMap, int32_t focusX, int32_t focusY) const
{
    ...
    MMI::CustomCursor cursor;
    cursor.pixelMap = pixelMap->GetRawPixelMapPtr();
    cursor.focusX = focusX;
    cursor.focusY = focusY;

    auto container = Container::Current();
    if (!container) {
        TAG_LOGE(AceLogTag::ACE_MOUSE, "SetUeaCustomCursor container is null!");
        return;
    }
    MMI::CursorOptions options;
    int32_t status = -1;
    if (container->IsUIExtensionWindow()) {
        auto winToken = container->GetToken();
        if (winToken == nullptr) {
            TAG_LOGE(AceLogTag::ACE_MOUSE, "SetUeaCustomCursor winToken is null!");
            return;
        }
        status = inputManager->SetCustomCursor(realHostWindowId, cursor, options, winToken);
    } else {
        status = inputManager->SetCustomCursor(realHostWindowId, cursor, options);
    }

    if (status != 0) {
        TAG_LOGE(AceLogTag::ACE_MOUSE, "SetUeaCustomCursor failed %{public}d", status);
        return;
    }
    inputManager->SetPointerVisible(true);
}
