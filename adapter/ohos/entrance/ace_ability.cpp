...
{
    CHECK_NULL_VOID(callbackOwner_);
    callbackOwner_->OnSizeChange(rect, reason, callback, syncId);
}
    ...
    const std::function<void()>& callback, const uint64_t syncId)
{
    ...
    CHECK_NULL_VOID(taskExecutor);
    taskExecutor->PostTask(
        [rect, density = density_, reason, container, callback, syncId]() {
            ...
            Platform::FlutterAceView::SurfaceChanged(flutterAceView, rect.width_, rect.height_,
                rect.height_ >= rect.width_ ? 0 : 1, static_cast<WindowSizeChangeReason>(reason), callback, syncId);
        },
