...
#define __UISERVICE_FILENAME__ (__builtin_strrchr(__FILE__, '/') ? __builtin_strrchr(__FILE__, '/') + 1 : __FILE__)
...
#define PRINT_LOG(level, fmt, ...) \
    HILOG_IMPL(LOG_CORE, LOG_##level, UISERVICE_LOG_DOMAIN, UISERVICE_LOG_TAG, \
        ACE_FMT_PREFIX fmt, __UISERVICE_FILENAME__, __FUNCTION__, __LINE__, ##__VA_ARGS__)
