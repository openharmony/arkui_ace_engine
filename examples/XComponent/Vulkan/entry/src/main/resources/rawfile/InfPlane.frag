...
{
    vec4 gridColorThickTemp = gridColorThick;
    if (uv.x > 0) {
        gridColorThickTemp.x = 1;
    }
    if (uv.y > 0) {
        gridColorThickTemp.z = 1;
    }
	...
	vec4 c = lod2a > 0.0 ? gridColorThickTemp : lod1a > 0.0 ? mix(gridColorThickTemp, gridColorThin, lodFade) : gridColorThin;
