...
//scrollbar显示效果库
std::vector<int32_t> scrollModePool = { ARKUI_SCROLL_BAR_DISPLAY_MODE_AUTO, ARKUI_SCROLL_BAR_DISPLAY_MODE_OFF, ARKUI_SCROLL_BAR_DISPLAY_MODE_ON , -1};
//scrollColor颜色库
...
//alignItem属性设置库
std::vector<int32_t> alignItemsPool = { GRID_ITEM_ALIGNMENT_DEFAULT , GRID_ITEM_ALIGNMENT_STRETCH ,napi_undefined,napi_null, 2, -1};
...
//clip属性设置库
std::vector<int32_t> clipModePool = {ARKUI_CONTENT_CLIP_MODE_CONTENT_ONLY, ARKUI_CONTENT_CLIP_MODE_BOUNDARY, ARKUI_CONTENT_CLIP_MODE_SAFE_AREA, -1};
//fadingEdge设置库
std::vector<float> fadingEdgePool = {10.0f, 30.0f, 40.0f, 400.0f, -1.0f};
//nestedMode设置
std::vector <int32_t> nestedModePool = {ARKUI_SCROLL_NESTED_MODE_SELF_ONLY, ARKUI_SCROLL_NESTED_MODE_SELF_FIRST, ARKUI_SCROLL_NESTED_MODE_PARENT_FIRST, ARKUI_SCROLL_NESTED_MODE_PARALLEL, -1, 4};
//itemStyle设置
...
//测试 OH_ArkUI_GridLayoutOptions_SetIrregularIndexes 及 OH_ArkUI_GridLayoutOptions_RegisterGetIrregularSizeByIndexCallback
std::shared_ptr<GridComponent> GridTest::CreatGrid2(int32_t type) {
    ...
    OH_ArkUI_GridLayoutOptions_RegisterGetIrregularSizeByIndexCallback(option, userData, [](int32_t itemIndex, void *userData) -> ArkUI_GridItemSize {
        if (userData) {
          std::string* str = static_cast<std::string*>(userData);
          OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "GridTest", "CreatGrid2 userData:%{public}s", str->c_str());
        }
        if (itemIndex == 0) {
...
std::shared_ptr<GridComponent> GridTest::CreatGrid3(int32_t type) {
    ...
    OH_ArkUI_GridLayoutOptions_RegisterGetRectByIndexCallback(option, userDataWorld, [](int32_t itemIndex, void *userData) -> ArkUI_GridItemRect {
        if (userData) {
            std::string* str = static_cast<std::string*>(userData);
            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreatGrid3 grid userData:%{public}s", str->c_str());
        }
        if (itemIndex == 0) {
...
//测试未设置 OH_ArkUI_GridLayoutOptions_RegisterGetRectByIndexCallback时 OH_ArkUI_GridLayoutOptions_GetIrregularIndexes返回值
std::shared_ptr<GridComponent> GridTest::CreatGrid4(int32_t type) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        auto itemStyle = gridItem->getAttribute(NODE_GRID_ITEM_STYLE)->value->i32;
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreatGrid4 itemStyle:%{public}d", itemStyle);
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreatGrid4 Option ret: %{public}d, size:%{public}d, index[0]:%{public}d, index[1]:%{public}d", ret, size, irregularIndexes[0], irregularIndexes[1]);
    grid->SetGridLayoutOptions(option);
    OH_ArkUI_GridLayoutOptions_Dispose(option);
    return grid;
}
...
napi_value GridTest::CreateNativeNode(napi_env env, napi_callback_info info) {
    ...
    grid->SetOnGridWillStopDragging([grid](float velocity) {
         OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "OnGridWillStopDragging velocity: %{public}f", velocity);
    ...
    nestedScrollButton->RegisterOnClick([grid, nestedScrollText, getNestedScrollText]() {
         ...
         nestedScrollText->SetTextContent("nestedScroll forward:" + std::to_string(forwardValue) + " ,back:" + std::to_string(backValue) +",");
         ...
         if (getValue != nullptr) {
             ...
             OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest nestedScroll", 
                "nestedScroll size: %{public}d", size);
            auto forward = getValue->value[0].i32;
            auto back = getValue->value[1].i32;
            getNestedScrollText->SetTextContent("getNestedScroll: forward:" + std::to_string(forward) + ",back:" + std::to_string(back));
         } else {
    ...
    void* userDataWorld = static_cast<void*>(str);
    OH_ArkUI_GridLayoutOptions_RegisterGetIrregularSizeByIndexCallback(option2, userDataWorld, [](int32_t itemIndex, void *userData) -> ArkUI_GridItemSize {
        if (userData) {
            std::string* str = static_cast<std::string*>(userData);
            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreateNativeNode grid2 itemIndex:%{public}d", itemIndex);
        if (itemIndex == 0) {
                OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreateNativeNode grid2 itemIndex == 0");
            return {2, 3};
        }
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreateNativeNode grid2 itemIndex != 0");
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "GridTest", 
                "CreateNativeNode Option ret: %{public}d, size:%{public}d, index[0]:%{public}d, index[1]:%{public}d", ret, size, irregularIndexes[0], irregularIndexes[1]);
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id),  scroll->GetComponent()) ==
        INVALID_PARAM) {
