...
| DFX 设计已覆盖 | 通过 | `design.md` 增加 DFX 章节，复用 Connect 传入的 EventHandler 投递单次 Get callback timeout task；超时清 callback、snapshot cache 和调用方 pid；连续 Start 初始无文本不自动 End；译文等待 watchdog 默认 5s 且 SA 未死亡时只输出告警 |
...
2026-06-23：根据 MR 86308 检视问题处理结果修正 DFX 定界：单次 Get request timeout 做 callback/snapshot/pid 清理；连续 Start 初始无文本保持会话；译文等待 watchdog 只对真实节点告警，不自动 End/Reset。
