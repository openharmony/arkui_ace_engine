...
2. 通过 PreviewerCLI `remoteRequest` 命令模拟 uisession IPC 调用，触发 `pageTranslate/getText/start/collect/sendResult/reset/end/simulateResultWatchdog`。
3. 验证 ArkUI 文本提取结果只包含真实节点文本，例如 `nodeId=6, text="UISession Translate Hello", version=1`，并包含空内容 TextInput 当前实际展示的 placeholder 提示文本。
...
11. 验证 DFX 异常场景：Start 后已正常回填部分译文，后续译文长时间不返回时，Previewer 测试入口触发译文等待 watchdog，确认仅输出非正文告警且连续翻译会话不被自动 End；随后通过 reset/end 路径确认临时译文清除、原文恢复。
...
| DFX-RESULT-WATCHDOG | Previewer 测试入口 `simulateResultWatchdog` 仅用于 Host Preview 验证，模拟 SA 长时间不回填真实节点译文后的 5s watchdog；必须只输出非正文告警，不自动 End/Reset，会话保留，后续 reset/end 再恢复原文 |
...
| 新增/修改 | `examples/UISessionTranslate/tools/host_preview/run_page_translate.sh` | HostPreview getText/start/collect/sendResult/screenshot/reset/end/result watchdog 自动化验证脚本；sendResult 按阶段组装单次批量 `results` payload |
...
13. 再次执行 `start`，对当前可见文本只发送部分译文，故意不对剩余文本发送译文；通过 `simulateResultWatchdog` 模拟等待超时后的 5s watchdog，用日志/remoteRequest 返回确认只记录告警、不自动结束会话，再通过 `reset/end` 和 `simpleTree` 确认临时译文可按正式恢复路径清除、原文恢复。
...
- DFX result watchdog 场景能通过 PreviewerCLI 触发并返回成功，只产生非正文告警、不自动 End/Reset；随后 `reset/end` 结果验证临时译文清理和原文恢复。
...
- [ ] DFX result watchdog remoteRequest 返回成功，验证只告警不自动 End/Reset，并通过 reset/end 验证译文清除、原文恢复
