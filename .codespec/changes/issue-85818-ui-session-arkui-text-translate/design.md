...
| `adapter/ohos/entrance/ui_session/ui_report_stub.cpp:586` | `HandlePageTranslateCallbackTimeout` 已能在 timeout 时清 callback | 需要增加 requestId 校验和按请求类型的 cleanup：单次 Get 超时清 callback 和调用方 pid；连续 Start 不因初始无文本 timeout 自动 End；译文 result watchdog 与请求 callback timeout 分离 |
...
| ADR-15 | 查询接口高频并发限制 | `GetPageTranslateText` 与 `StartPageTranslate` 共用 `UiReportStub::RegisterPageTranslateTextCallback` 作为接口级并发闸门；同一 `IUiContentService` 连接上一次 Get/Start callback 未释放前，新的 Get/Start 返回 `LAST_UNFINISH`，不覆盖 callback、不排队、不重复进入 manager。`GetCurrentAbilityLanguageInfo` 是同步 IPC 简单查询，使用 proxy 侧 in-flight guard；上一笔查询未返回前重复调用返回 `LAST_UNFINISH`，不重复发送 IPC | 在 `UiSessionManagerOhos` 增加 pending 变量；允许多次 Get 覆盖旧 callback；按请求排队；由 SA 自行去重；语言查询不做限制 | callback 注册点已经是 SA 请求生命周期入口，复用 `GetInspectorTree` 的 callback gating 模式最局部；语言查询没有 callback 生命周期，放在 proxy 侧以同步 IPC 在途状态做最小限制；避免 manager 维护复杂 pending 状态，也避免高频轮询导致重复遍历页面、重复 IPC 或重复同步查询 | proxy/stub/report UT 覆盖重复注册和同步查询 guard；单次 Get 在 `SendPageTranslateResult` 成功、timeout、IPC 失败或连接清理后释放；连续 Start 在 End/SA death/清理路径释放；语言查询在 `SendRequest` 返回或错误路径释放 |
...
|------|--------|----------|-----------|------|
| 1 | SA | `UIContentServiceProxy` | `GetPageTranslateText(request, callback)` | 单次获取；request 包含 scope；若同一 report 已有未完成 Get/Start callback，则返回 `LAST_UNFINISH`，不进入 manager |
| 2 | SA | `UIContentServiceProxy` | `StartPageTranslate(request, callback)` | 启动连续翻译；注册 callback；Start 期间重复 Get/Start 返回 `LAST_UNFINISH`，直到 End/SA death/清理路径释放 callback |
...
| 11 | Timeout/watchdog | `UiReportStub` / `UiSessionManagerOhos` | timeout/watchdog task | 单次 Get 长时间无 callback 时记录故障日志，清 callback、snapshot cache 和调用方 pid；连续 Start 不因初始无文本 End，译文等待 watchdog 只告警 |
| 12 | SA | `UIContentServiceProxy` / `UiSessionManagerOhos` | `GetCurrentAbilityLanguageInfo(language, region)` | 同步 IPC 获取当前 Ability 实例生效的 language/region，不触发页面遍历或翻译状态变更；同一 proxy 上上一笔查询未返回时返回 `LAST_UNFINISH` |
...
1. UI_SA 调用 `UIContentServiceProxy::StartPageTranslate(request, callback)`，request 中 `scope` 指定 `ARKUI_ONLY`、`ARKWEB_ONLY` 或 `ARKUI_ARKWEB`；proxy 校验 callback 和 request，并在 `UiReportStub::RegisterPageTranslateTextCallback` 中登记回调。该注册函数持有内部 mutex，若已有页面翻译 callback 未释放则返回失败，proxy 对 `GetPageTranslateText` 和 `StartPageTranslate` 统一返回 `LAST_UNFINISH`，不在 manager 侧额外维护并发 pending 变量。
2. `UIContentServiceProxy` 对连续 `StartPageTranslate` 只注册文本 callback，不用初始快照 timeout 自动结束会话，避免空页面或占位布局场景提前 End；随后 proxy 通过 `START_PAGE_TRANSLATE` transaction 发送 request 到 UI 进程。单次 `GetPageTranslateText` 的差异是会注册 callback timeout cleanup 并投递请求超时任务，成功发送 `SendPageTranslateResult` 后释放单次 callback；连续翻译 callback 到 `EndPageTranslate`、SA death 或 timeout 清理路径才释放。
...
7. 脚本调用 `simulateResultWatchdog` 验证长时间无译文只输出非正文告警、不自动 End/Reset；再调用 `reset`、`end` 验证正式恢复路径，并通过 `PreviewerCLI inspector --business simpleTree` 调 `GetVisibleInspectorTree`，确认临时译文字段清理或在展示译文时输出 `translated` DFX 字段。
...
| 回填校验 | nodeId 存在且 version 匹配时才进入应用；单次 snapshot 结果还必须校验当前源文本 hash 与 snapshot 记录一致，避免旧译文覆盖新文本。`SendPageTranslateResult` 接收单对象、顶层数组或 `{"results":[...]}`，每条合法结果必须有 `nodeId >= 0`、数字 `version` 和非空译文；批量中合法结果独立校验和应用，全批无合法项返回参数错误。sourceType 可作为 UI 内部辅助校验，不要求 SA 传入或感知 |
...
| 死亡恢复 | SA report remote death 时先从 translate process map 中移除死亡 pid；若死亡 pid 是当前连续翻译 owner，或 owner 未明确但会话已 started，则调用统一 `ResetTranslate(-1)` 或等价全量恢复，清理 scope、owner pid 和 callbacks。不得依赖 translate pid 集合大小为 1 才恢复 |
| 超时恢复 | `UiReportStub` 通过 `EventHandler` 投递单次 Get 请求超时任务；超时后清除 pending callback，记录非正文故障日志，并通知 manager 清理单次请求现场和调用方 pid。连续 Start 初始无文本不触发 End/Reset 清理；译文等待 watchdog 单独记录 nodeId/version 告警 |

单次 `GetPageTranslateText` 不得写入 `translateTextNodes_` / `translateTextVersions_` 等连续翻译容器，也不得把 `textTranslateActive_` 作为复用开关长期置位。允许使用独立 snapshot cache 保存本次 node 弱引用、snapshot version 和 source hash，用于迟到译文的 nodeId/version/hash 校验；每次单次扫描开始前必须清理上一轮 snapshot cache，Start/End/Reset/timeout 也必须清理相关 cache。该 cache 不参与连续增量监听，不产生 pending 变化上报。
...
| 按需写入 | 没有连续翻译任务时，ContentChangeManager 不写入连续 translate 专用容器、不维护 pending、不投递 translate 延迟任务；单次 Get 只写隔离 snapshot cache，并在请求边界清理 | content_change_manager 单测/trace |
...
译文等待 timeout 与请求 callback timeout 分开定义：单次 Get 请求 callback timeout 用于清 pending callback 和一次性现场清理；连续 Start 不因初始没有可翻译文本而注册 End/Reset 型 cleanup。原文发给 SA 后等待译文回填的 watchdog 默认 5s，只对 `nodeId >= 0` 的真实文本节点启动；`nodeId < 0` 的 empty/no-content 哨兵不启动 watchdog。若连续翻译场景 5s 内未收到对应 `nodeId/version` 的 `SendTranslateResult` 且 SA 未死亡，只输出非正文告警日志和现场摘要，不强制 End/Reset，不清理会话，后续迟到译文仍按 `nodeId/version/source hash` 校验处理。

| 场景 | 设计 |
|------|------|
| 高频/并发重复请求 | `UiReportStub::RegisterPageTranslateTextCallback` 作为并发闸门，同一 `report` 对象上已有未完成 Get/Start callback 时拒绝新的 Get/Start；proxy 返回 `LAST_UNFINISH`。单次 Get 在译文结果成功发送或 timeout cleanup 后释放；连续 Start 在 End/SA death/清理路径释放 |
| 单次获取长时间无回调 | `GetPageTranslateText` 注册 callback 后生成 requestId 并投递 `PageTranslateTextCallbackTimeout`；超时按 requestId 清 callback，记录故障日志，触发一次性请求现场清理并移除调用方 translate pid |
| Start 后初始快照无可翻译文本 | `StartPageTranslate` 进入连续翻译 owner 会话，但不注册初始快照 timeout cleanup；当前页面无实际绘制文本时允许暂不上报，后续新节点上树或内容变化仍按增量上报 |
| 原文发出后长时间无译文 | manager 为已发送原文的 `nodeId >= 0` /version 投递 5s 译文等待 watchdog；SA 未死亡时只记录告警日志，不强制结束会话、不恢复原文、不阻塞后续译文；`nodeId < 0` 哨兵不投递 watchdog |
...
| SendTranslateResult 回填失败 | 单条失败记录 nodeId/version/errorCode，不打印正文；UI 内部可附带 sourceType；批量中其他合法项继续。缺少 nodeId/version、nodeId 为负、译文为空或全批无合法项属于入参错误，直接返回 `PARAM_INVALID` |
| EventHandler 为空或投递失败 | 接口返回 `FAILED` 或记录 `LOGW/LOGE`，不得进入 pending 状态；已注册 callback 必须回滚 |
| 超时任务取消 | 单次请求收到真实文本回调、请求失败、End、Reset、SA death 时移除对应 timeout/watchdog task，避免清理或告警下一次请求；`nodeId < 0` no-content 哨兵不取消译文 watchdog，因为它不会启动 watchdog |
...
4. 若是单次 Get timeout，通知 `UiSessionManagerOhos` 移除调用方 translate pid 并清理 snapshot cache；若是连续翻译，只有 End、Reset 或 owner SA death 才结束会话。
5. 需要结束会话时，`ContentChangeManager` 清理 ArkUI 翻译节点版本状态，并批量调用 Pattern 清理临时译文。
6. 需要结束会话且 scope 覆盖 Web 时，Web 分支调用现有 End/Reset 翻译路径。
7. 输出非正文故障日志。
...
| Review hardening 单测 | translate pid 清理、owner pid death reset、连续 Start 初始无文本不 End、非法 result parser、`nodeId < 0` 哨兵无 watchdog、单次 snapshot 版本/hash 防迟到回填 | mock UiSessionManager、PageTranslateNode、UiReportStub | `ui_content_stub_unittest_linux` + `content_change_manager_translate_test_ng` |
...
| 多 SA 注册 translate callback | 可靠性 | 中 | 沿用现有 processMap 语义，同一 translate 会话明确单 owner pid；Start 成功后标记 owner，End/参数失败/单次完成/timeout 清理调用方 pid，owner death 直接全量恢复；后续需求再扩展多 SA | Uisession Owner |
...
| 页面遍历收集 | 增加 translate snapshot 接口，根据当前 page/root 收集 ArkUI 原生可翻译文本节点快照；过滤仅占位布局、未实际绘制文本的节点；单次 Get 使用独立 snapshot cache，扫描前清理旧 cache |
| 文本版本管理 | 在 translate mode 下为节点维护 version/hash，用于变化去重和回填校验；单次 snapshot 使用独立递增 snapshot version 和 source hash，避免固定 version 造成迟到译文误回填 |
| 增量上报生成 | Start 后复用现有文本实际绘制和文本变化触发点，结合节点级内容变化生成 `NODE_ADDED` / `CONTENT_CHANGED` 内部变化原因；对 SA 仍只发送 `nodeId/text/version`；仅占位布局不上报 |
| 临时译文恢复 | Reset/End/SA death 时批量调用 Pattern 清理临时译文状态，并清理连续 translate 版本状态；单次 Get timeout 只清理 snapshot cache 和一次性状态，不误清连续容器 |
...
| 按需开销 | translate inactive 时触发点只做常量级开关判断后返回，不写连续容器、不维护 pending、不投递 watchdog/延迟任务；GetPageTranslateText 单次快照只写隔离 snapshot cache，并在请求边界清理，不得留下连续监听状态 |
