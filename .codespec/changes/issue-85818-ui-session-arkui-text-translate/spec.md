...
|------|----------|------|--------|-------------|
| `GetPageTranslateText` | 同步 `SendRequest` 接收受理结果；文本通过 `PageTranslateTextCallback` 异步返回 | `request` 为 JSON object 或兼容字符串；`eventCallback` 不可为空 | `NO_ERROR` 表示请求已受理；`PARAM_INVALID` 表示 callback 为空或 scope 非法；`FAILED` 表示未连接/写 parcel 失败；`LAST_UNFINISH` 表示同一 report 已有未完成页面翻译 Get/Start callback；`REPLY_ERROR` 表示 IPC 发送失败 | 获取当前快照，不开启连续监听；并发限制复用 `UiReportStub::RegisterPageTranslateTextCallback` 的 mutex 和 callback 非空判断，不新增 manager 侧 pending 状态；可使用单次 snapshot cache 保存 nodeId/version/hash 用于迟到译文校验，但每次请求开始前必须清理上一轮 snapshot cache，请求完成、失败或 timeout 后必须清理调用方 translate pid |
| `StartPageTranslate` | 同步 `SendRequest` 接收受理结果；初始快照和后续增量通过同一 callback 返回 | 同 `GetPageTranslateText` | 同 `GetPageTranslateText` | 进入连续翻译状态，保存 scope 和 owner pid，先上报当前节点；若当前页面没有已实际绘制可翻译文本，不得因初始无文本 timeout 提前 End，Start 到 End/SA death 期间持续上报新增和内容变化节点；连续 callback 在 End/SA death/清理路径释放，期间重复 Get/Start 返回 `LAST_UNFINISH` |
...
| `SendPageTranslateResult` | 异步 IPC | `result` 为单对象、顶层数组或 `{"results":[...]}` JSON 字符串 | `NO_ERROR` / `PARAM_INVALID` / `FAILED` / `REPLY_ERROR` | UI 侧先校验至少存在一条合法结果；合法项逐条按 nodeId/version/source text hash 回填，单条回填失败不阻塞同批其他合法结果；成功写入 IPC 后取消可解析 nodeId/version 的译文等待 watchdog |
| `GetCurrentAbilityLanguageInfo` | 同步 IPC，从 reply 直接读取结果 | `language`、`region` 为输出引用 | 远端结果码；`NO_ERROR` 时输出字段有效；上一笔同步查询未返回时返回 `LAST_UNFINISH` | 返回当前 Ability/UIContent 实例已生效的语言和地区，不注册 report callback，不改变翻译会话状态；同一 proxy 上同步 IPC 在途期间不得重复发送查询 |

### 接口并发与调用频率限制

`GetPageTranslateText` 与 `StartPageTranslate` 不支持同一 `IUiContentService` 连接上的多线程并发重入和高频轮询调用。SA 发起一次 Get 或 Start 后，在对应 `PageTranslateTextCallback` 生命周期结束前再次调用 Get 或 Start，接口必须返回 `LAST_UNFINISH`，不得覆盖已有 callback、不得排队缓存请求、不得重复进入 manager 文本遍历或连续翻译启动链路。该限制以 `UiReportStub::RegisterPageTranslateTextCallback` 为统一并发闸门：注册函数持 mutex 判断当前 callback 是否为空。

释放条件按请求类型区分：单次 Get 在 `SendPageTranslateResult` 成功发送、请求 timeout cleanup、IPC 发送失败或连接/死亡清理后释放；连续 Start 在 `EndPageTranslate`、SA death/连接断开或清理路径释放，期间允许通过同一 callback 接收初始快照和后续增量。SA 侧需要对 Get/Start 做节流，批量译文应通过一次 `SendPageTranslateResult` 的 `results` 数组回填，不应通过重复 Get/Start 模拟批处理。

`GetCurrentAbilityLanguageInfo` 是同步简单查询，不依赖 report callback，但也不支持同一 proxy 上多线程并发重入。proxy 侧必须在发送 `GET_CURRENT_ABILITY_LANGUAGE_INFO` IPC 前设置 in-flight guard；若上一笔查询尚未从 `SendRequest` 返回，新的查询必须返回 `LAST_UNFINISH`，不得重复发送 IPC。该 guard 在 `SendRequest` 返回、写 parcel 失败或任意错误返回路径自动释放，不影响后续正常查询。
...
| 字段 | 类型 | 必选 | 说明 |
|------|------|------|------|
| `nodeId` | number | 是 | 回填目标节点 ID，必须来自本次或连续翻译期间 UI 上报的 `nodeId`，且必须 `>= 0` |
| `version` | number | 是 | 回填目标版本；缺失、非数字或版本不匹配时该条结果无效 |
| `translatedText` | string | 是，优先 | 译文文本，不能为空。若包含 `\\uXXXX` 字面量，UI 侧回填前解码为 UTF-8 |
| `text` | string | 否 | `translatedText` 的兼容别名，仅当 `translatedText` 为空时读取；最终译文为空时该条结果无效 |
| `results` | array | 批量格式必选 | 单次 IPC 批量回填多节点，验证不得用多次单条发送替代；数组中合法项可继续处理，若没有任何合法项则返回 `PARAM_INVALID` |
...
`nodeId < 0` 的哨兵不代表可翻译节点，不启动译文等待 watchdog，也不允许出现在 `SendPageTranslateResult` 合法结果中。
...
| REFINED | 检视问题加固 | 明确 translate 调用方 pid 生命周期、连续翻译 owner pid、单次 snapshot version/hash 校验、Start 初始无文本不自动 End、非法译文 payload 拒绝、`nodeId < 0` 哨兵不启动译文 watchdog |
...
- **AC-4:** WHEN StartTranslate scope 为 ArkUI-only THEN ArkUI 进入连续翻译状态，并上报当前 ArkUI 原生可翻译文本节点列表；若当前没有已实际绘制可翻译文本，连续翻译状态仍保持到 End/Reset/SA death，后续节点上树或内容变化仍应上报。
...
- **AC-13:** WHEN 回填目标 ArkUI 节点离树、版本不匹配、source text hash 已变化或 nodeId 不存在 THEN ArkUI 忽略该条结果、记录非正文日志，并继续处理同批次其他合法结果；WHEN `SendPageTranslateResult` 缺失 `nodeId/version`、`nodeId < 0` 或译文为空且没有任何合法项 THEN 返回 `PARAM_INVALID`。
...
- **AC-16:** WHEN SA 进程意外死亡或 IPC 断开且当前存在已翻译展示 THEN ArkUI 在死亡监听中恢复所有临时译文为原文，并清理会话状态；死亡恢复判断以当前连续翻译 owner pid 或已启动状态为准，不得因 process map 残留旧 pid 而跳过恢复。
- **AC-17:** WHEN SA 进程意外死亡或 IPC 断开且当前处于连续翻译状态 THEN ArkUI 停止变化上报并清理连续翻译状态；End、单次获取完成、参数失败和 timeout 清理路径必须移除对应 translate pid。
...
| BR-5 | DFX timeout | 单次获取请求必须有 timeout 清理任务；Start 连续翻译不得因初始无文本或 no-content 哨兵超时而自动 End。原文发出后等待译文回填默认 5s，连续翻译场景 SA 未死亡时只输出告警日志和现场摘要，不强制结束会话 | AC-16, AC-17, AC-18, AC-19 |
| BR-5a | 查询接口并发限制 | `GetPageTranslateText` 和 `StartPageTranslate` 共用 `UiReportStub::RegisterPageTranslateTextCallback` 作为多线程并发闸门；同一 report 上一次 Get/Start callback 未释放前，新的 Get/Start 必须返回 `LAST_UNFINISH`，不得重复注册 callback、不得重复进入 manager 执行链路。`GetCurrentAbilityLanguageInfo` 使用 proxy 侧同步 in-flight guard；上一笔同步查询未返回前重复调用返回 `LAST_UNFINISH`，不得重复发送 IPC | AC-1, AC-4, AC-16, AC-17, AC-21, AC-22 |
...
| BR-8 | 按需写入和低开销 | 未处于 StartTranslate 连续翻译状态时，`ContentChangeManager` 不写入连续 translate 容器、不维护增量 pending、不投递 translate 延迟任务；单次 Get 只能写入隔离的 snapshot cache，并在请求开始前清理上一轮 cache，通过 snapshot version/source hash 防止迟到译文误回填，不留下连续监听状态 | AC-1, AC-4, AC-6, AC-7, AC-14, AC-14a |
...
| BR-13 | 批量回填语义 | `SendPageTranslateResult` 兼容单条对象、顶层数组和 `results` 数组；每条合法结果必须包含 `nodeId >= 0`、数字 `version` 和非空译文。批量中每个合法节点按自身 `nodeId/version/source hash` 独立校验，单条回填失败不得阻断其他合法节点；全批无合法项时返回 `PARAM_INVALID`。Preview 和 sample 验证不得通过按节点循环多次发送替代批量能力 | AC-12, AC-13, AC-20, AC-25 |
...
| EX-3 | 参数非法 | scope 为 0、scope 包含未定义 bit、`SendPageTranslateResult` 缺少 `nodeId/version`、字段类型非法、`nodeId < 0`、译文为空或全批无合法项 | 返回 `PARAM_INVALID` 或等价参数错误，不进入会话、不注册 callback、不上报；`XCOMPONENT`/`CANVAS_NODE` 作为已定义保留 bit 不属于非法 bit，但当前只解析不处理 | AC-13, AC-18 至 AC-20 |
| EX-4 | 非范围控件 | TextInput/TextArea 用户可编辑文本、Canvas/OCR 等 | 不收集、不上报、不回填；TextInput 空内容且 placeholder 实际展示时按展示提示文本处理 | AC-3 |
| EX-5 | 翻译请求超时 | 单次获取长时间无响应 | 记录非正文故障日志，清 pending callback，清理单次请求现场和调用方 pid；连续翻译 Start 的初始无文本不按该异常结束会话 | AC-16, AC-17 |
...
| EX-10 | 单次 snapshot 迟到译文 | 单次获取上报后节点文本已变化，旧 version/source hash 的译文迟到 | 忽略该条结果并记录非正文日志，不覆盖当前节点显示 | AC-13 |
...
| RC-4 | 请求 timeout | 单次获取 timeout task 触发 | 清 callback，移除调用方 translate pid，清理一次性请求现场，输出 DFX 日志 | 迟到回调不得重新进入会话；连续翻译 Start 初始无文本不触发该恢复契约 |
...
| `ContentChangeManager` | 已存在 `StartTextTranslateReport`、`ReportCurrentTranslateTextNodes`、`ReportTranslateTextNode`、`ApplyTranslateResult` 等基础入口 | 单次 Get 改为隔离 snapshot cache，保存 node 弱引用、snapshot version 和 source hash 用于迟到译文校验；连续翻译容器与纯 content change 容器隔离；容器类型改为 `WeakPtr<PageTranslateNode>`；补实际绘制过滤、按需写入、版本清理和测试 |
...
| 无连续翻译时低开销 | translate inactive 时仅允许常量级开关判断，不允许写连续 translate 容器、维护 pending 或投递连续 translate 任务；单次 Get 只允许写隔离 snapshot cache，并在请求边界清理 | AC-14, AC-14a |
| SA death 必须恢复 | DeathRecipient 调用全量恢复 | AC-16, AC-17 |
| DFX timeout 必须定界 | 复用 `Connect` 传入的 `EventHandler` 投递单次 Get callback timeout task，超时清 callback、snapshot cache 和调用方 pid；连续 Start 初始无文本不自动 End；译文等待 watchdog 默认 5s，只对 `nodeId >= 0` 真实节点启动且只告警 | AC-16, AC-17, AC-18, AC-19 |
...
Feature: Uisession unified page text translation
  ...
  Scenario: One-shot translate request timeout cleanup
    Given SA has connected with an EventHandler
    When SA calls GetPageTranslateText and no text callback arrives before timeout
    Then the pending callback is cleared
    And one-shot snapshot cache and caller pid are cleared
    And active StartPageTranslate session state is not ended by this timeout
    And logs contain request id, scope and cleanup result without original text

  Scenario: Translate result watchdog
    Given SA has started page translation and UI has reported nodeId, text and version
    When no translated result for that nodeId/version arrives within 5 seconds
    Then UI logs a non-content warning with nodeId, version, textLength and elapsedMs
    And the continuous translation session remains active
  ...
  Scenario: Host Preview getText and sendResult
    ...
    When the script calls remoteRequest pageTranslate/sendResult with one batch results payload for returned nodes
    Then the Preview environment accepts the batch result and reset/end can restore the session
```
