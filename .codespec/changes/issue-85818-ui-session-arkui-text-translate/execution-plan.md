...
**不建议延后：** 5s 译文等待 watchdog、`LargeStringAshmem` 大文本路径、SA death 恢复、实际文本绘制过滤、版本校验和 Reset/End 恢复。
...
|---------|------|----------|---------|----------|----------|----------|
| TASK-001 | 建立统一 IPC/API/report payload、Ability 语言地区接口与共享内存/DFX 基础 | `interfaces/inner_api/ui_session`, `adapter/ohos/entrance/ui_session` | AC-1, AC-2, AC-16 至 AC-22 | 无 | 新接口、parcel、翻译 callback、LargeStringAshmem、timeout/watchdog、Ability language/region 同步 IPC 基础可编译并有单测 | 相关 ui_session/adapter so 目标 + Uisession 单测 |
| TASK-002 | 建立 manager 会话、启动期 callback 注册、Ability 语言地区读取、Web 兼容和死亡恢复 | `adapter/ohos/entrance/ui_content_impl.*`, `ace_container.cpp`, `ui_session_manager_ohos.*`, `ace_translate_manager.*`, `ui_translate_manager.h` | AC-4, AC-5, AC-8, AC-9, AC-15 至 AC-17, AC-21, AC-22 | TASK-001 | Start/End/Reset/Get 分发到当前 instance，SA death 恢复；Web 旧能力通过 `PageTranslateNode` hook 继续脚本注入且不回退；当前 Ability language/region 能按 instance 返回 | 相关 adapter/entrance so 目标 + manager/death 单测 |
| TASK-003 | 增强 `ContentChangeManager`，新增待翻译文本容器并实现快照/增量/版本/过滤 | `frameworks/core/components_ng/manager/content_change_manager`, `page_translate_node.h`, `pipeline_context.*`, 相关 test/mock | AC-1, AC-4, AC-6, AC-7, AC-10, AC-11, AC-13, AC-14, AC-14a | TASK-002 | translate 专用容器与纯 content change 容器隔离，容器仅保存 `WeakPtr<PageTranslateNode>`；无连续翻译任务时不写容器、不算 version/hash；snapshot 和 Start 增量只上报实际绘制文本；空内容 TextInput placeholder 纳入，用户可编辑内容排除 | content_change_manager 单测 |
...
| DFX | 请求 timeout 清 callback 并清理现场；译文等待 watchdog 默认 5s，SA 未死亡时只输出非正文告警 |
...
| ON-DEMAND-WRITE | 未 StartTranslate 时不得写 translate 容器、提取文本、计算 hash/version 或投递 translate 任务 |
