...
|---------|----------|----------|-------------|---------|---------|---------|---------|
| ImageSourceInfo | 使用中的对象非法销毁检查 | 可能导致cpp_crash | RefPtr智能指针在被使用时，如果被重新赋值将可能导致所管理的对象被析构失效，进而造成UAF问题 | 严重 | 识别是不是有对象使用中被销毁的问题，避免程序带着问题继续运行进而出现难以定位的问题现场 | "this object is still in use" | 不涉及 |
| ImageDfxConfig | 使用中的对象非法销毁检查 | 可能导致cpp_crash | RefPtr智能指针在被使用时，如果被重新赋值将可能导致所管理的对象被析构失效，进而造成UAF问题 | 严重 | 识别是不是有对象使用中被销毁的问题，避免程序带着问题继续运行进而出现难以定位的问题现场 | "this object is still in use" | 不涉及 |
