...
release_change_log:
  - date: 2026-08-22
    summary: "Split ArkUI PageScene onlyVisible into component-visibility filtering, add optional rectCulling viewport filtering (default false), optimize rect/page viewport calculation, and add opacity plus parent-subtree dirty recomputation while preserving node-ID deduplication."
  - date: 2026-07-21
...
related_decisions:
  ...
  - DEC-005: onlyVisible 仅负责组件属性可见性过滤；rectCulling 独立负责页面 viewport 相交过滤，缺省为 false
  - DEC-006: onlyVisible 过滤 visible/active、transform 后非零尺寸及自身/祖先最终 opacity 为零的节点；opacity 同值更新不产生 dirty
  - DEC-007: 父级可见属性变化只合并一次 dirty，稳定点从当前 pageRoot 重算父子状态；去重只比较命中节点 ID 列表，坐标变化不构成新状态
code_refs:
