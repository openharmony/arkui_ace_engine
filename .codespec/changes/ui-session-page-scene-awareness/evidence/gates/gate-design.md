...
| Web 透传边界已明确 | 通过 | 明确当前只做 `webRules` 注册透传、反注册和查询请求透传；不设计 `webRules` 规格，不做 Web 内部匹配、回传和验证 |
| UIExtension 透传边界已明确 | 通过 | 明确当前只做规则注册、反注册、查询请求透传，不做 UIExtension 内部匹配、回传和验证 |
| 隐私和安全约束已明确 | 通过 | 默认不上报文本正文和完整树，日志只输出摘要，非法 ruleJson 不保存不触发 |
