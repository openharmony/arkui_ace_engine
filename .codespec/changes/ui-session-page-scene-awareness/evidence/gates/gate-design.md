...
| 首批 ArkUI 匹配设计已明确 | 通过 | 明确 `TEXT_EDITOR` tag 白名单、稳定点全量扫描、`onlyVisible` 属性过滤、`rectCulling` viewport 过滤、opacity/可获焦/rect 计算和节点 ID 列表去重策略 |
...
| 实现阶段稳定点约束已回写 | 通过 | 已将 PageScene-only 页面稳定点、Pipeline 不直接 flush PageScene、Text/Image 具体事件隔离、opacity 及父子树 dirty 重算等关键点同步到 `spec.md`、`design.md`、`spec-for-test.md` 和 `execution-plan.md` |
