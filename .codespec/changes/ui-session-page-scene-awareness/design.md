...
sequenceDiagram
    ...
    Note over Web,Uie: Rule lifecycle pass-through
```
...
- 所有接口日志只能输出 pid、ruleSetId、ruleId、source、错误码、matchedCount 等摘要，禁止输出输入文本正文。
...
- ArkUI 宿主来源通过 `ReportPageSceneEvent` 上报命中或退出事件；Web / UIExtension 接入为规则生命周期透传。
- 日志仅打印 `ruleSetId`、`ruleId`、`sceneType`、`matchedCount`、错误码和结果长度，不打印文本正文；`includeText=true` 的完整回调结果只能通过 callback 或 sample `-tofile` 验证。
...
| `webRules` | 可选字段。仅当 Web 来源启用时生效；宿主校验其为可透传 JSON 值并保存原文。 |
...
| 子来源规则透传 | 规则允许 Web / UIExtension 来源参与 | 宿主把规则生命周期请求透传到对应控件。 |
...
当前特性为 Web 和 UIExtension 设计宿主到控件的规则透传通路。
...
- Web 来源启用时，宿主向 Web 控件透传 `ruleJson.webRules`。
- UIExtension 来源启用时，宿主向 UIExtension 控件透传原始 `ruleJson`。
- `RegisterPageSceneRulesForSubSource` 要求宿主找到页面内对应控件并调用透传入口。
- `UnregisterPageSceneRulesForSubSource` 只要求宿主把 `ruleSetId` 透传给已下发过规则的控件，用于生命周期对齐。
- `GetPageSceneForSubSource` 用作查询请求透传。
...
| Web 透传验证 | 验证宿主对 Web 控件的 `webRules` 注册透传、反注册、查询请求透传入口被调用。 |
| UIExtension 透传验证 | 验证宿主对 UIExtension 控件的规则注册、反注册、查询请求透传入口被调用。 |
...
Stage 2 设计已覆盖新增接口、规则模型、状态模型、ArkUI 匹配、Web/UIExtension 规则透传边界、错误恢复、隐私约束和验证计划。允许进入 Stage 3 实现拆解。
