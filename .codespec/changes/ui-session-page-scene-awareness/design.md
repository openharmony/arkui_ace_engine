...
| `PageSceneReportState` | `lastReportTime`、`lastPageName`、`lastNodeSignature` | 去重和节流。 |
...
| 文本输入类控件下树 | 已注册 `TEXT_EDITOR` 规则 | 从 tracker 移除该节点并更新数量；数量低于阈值时只更新状态，不补发未命中事件。 |
...
- 当数量从低于阈值变为大于等于阈值时只挂起待检测规则；实际上报必须在页面稳定点执行，并受 `deduplicate` 和 `minReportIntervalMs` 控制。
- 当数量从大于等于阈值降到低于阈值时首版只更新内部状态，不上报“未命中”事件，除非后续规则新增 `reportUnmatched`。
...
| ArkUI tracker 单元测试 | `PageSceneInputCountTracker` 初始化扫描、输入类控件上树计数增加、下树计数减少、可见性/可获焦变化、阈值跨越后挂起检测。 |
