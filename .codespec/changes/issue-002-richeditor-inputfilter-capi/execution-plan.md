...
- frameworks/core/components_ng/pattern/text_field/text_filter_utils.h — 新建：共享过滤函数声明
- frameworks/core/components_ng/pattern/text_field/text_filter_utils.cpp — 新建：共享过滤函数实现
- frameworks/core/components_ng/pattern/text_field/content_controller.cpp — 修改：FilterWithRegex 改用共享调用
...
| 操作 | 文件 | 说明 |
|------|------|------|
| Create | frameworks/core/components_ng/pattern/text_field/text_filter_utils.h | FilterWithRegex + 辅助函数声明 |
| Create | frameworks/core/components_ng/pattern/text_field/text_filter_utils.cpp | 从 content_controller.cpp 提取 FilterWithRegex(L314-324) + ContentToWstring + ContentToU16string + RemoveErrorTextFromValue |
| Modify | frameworks/core/components_ng/pattern/text_field/content_controller.cpp | FilterWithRegex 改为 text_filter_utils::FilterWithRegex 调用 |
| Modify | frameworks/core/components_ng/pattern/text_field/BUILD.gn 或相关 gni | 新增 text_filter_utils 源文件 |
...
| 停止条件 | 发现事件 kind 必须使用 RICH_EDITOR 而非 TEXT_INPUT（与 design.md D-3 不符），停止并回传 |
...
AC-5: WHEN 输入被拒字符 THEN OH_ArkUI_NodeEvent_GetStringAsyncEvent 可获取 .pStr
