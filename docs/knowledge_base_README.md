...
- `docs/` 下 Markdown 文档总数：147
- 命名符合 `*_Knowledge_Base*.md` 的旧版知识库文档：43
...
- `context_registry.json` 新版条目数：86
- `knowledge_base_INDEX.json` 旧 KB 索引条目总数：44
- 旧 KB 索引路径有效性：44/44（全部存在）
...
├── focus/ (2 MD)
├── layout/ (0 KB)
├── pattern/ (26 KB + 6 非 `*_Knowledge_Base*` 命名文档)
├── sdk/ (3 KB)
├── svg/ (0 KB，SVG 已迁移)
└── syntax/ (7 MD, 其中 4 KB)
```
...
- 2026-07-30：依据 05-03-01 与 05-03-10 Spec 重建滚动公共能力、迁移 WaterFlow/FlowItem；移除旧 Scroll/WaterFlow KB 与索引入口，新版 registry 增至 86 条，旧索引减至 44 条；同步修复 Swiper 组件化路由。
