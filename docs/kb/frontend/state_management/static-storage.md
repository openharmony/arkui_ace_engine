...
- 静态 V1 核心（`StateDecoratedVariable` 是存储 `link()` 返回基础）：[static-v1-core](static-v1-core.md)
- 静态 V1 装饰器（存储联动装饰器 `@StorageLink`/`@StoragePropRef` 等）：[static-v1-decorators](static-v1-decorators.md)
- [跨前端互操作（存储桥完整机制）](static-interop-bridge.md)
- [动态侧存储体系（对照参考）](storage.md)