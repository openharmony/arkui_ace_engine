...
- [V1 对应核心](v1-core.md)
- [V2 装饰器具体语义](v2-decorators.md)
- 基础设施（`WeakRefPool`、`UINodeRegisterProxy`）：[infrastructure](infrastructure.md)
- 存储体系（`AppStorageV2`/`PersistenceV2`）：[storage](storage.md)
- [自定义组件机制（`ViewV2`）：custom-component](custom-component.md)
- [Interop 桥接（V2 builder/storage 跨前端）](interop-bridge.md)
- [静态 V2 核心（arkoala 侧）](static-v2-core.md)