...
- V2 核心逻辑（`ObserveV2`、ID 分段、惰性 Proxy、调度）：[v2-core](v2-core.md)
- [V1 装饰器（V2 的上一代对应）](v1-decorators.md)
- 存储体系（`@Type`/PersistenceV2/AppStorageV2）：[storage](storage.md)
- [自定义组件机制（`@ComponentV2`/`ViewV2`）：custom-component](custom-component.md)
- 基础设施（`UIUtils`/`WeakRefPool`）：[infrastructure](infrastructure.md)
- [静态 V2 装饰器与数据对象（arkoala 侧）](static-v2-decorators.md)