...
- NG 布局框架：[docs/kb/architecture/layout-framework.md](layout-framework.md)（FrameNode/LayoutWrapper/LayoutAlgorithm/GeometryNode 细节）
- [多级渲染管线：[docs/kb/architecture/multi-level-render-pipeline.md](multi-level-render-pipeline.md)（子管线与多容器 VSync 协调）](multi-level-render-pipeline.md)
- OH 平台适配层：[docs/kb/architecture/oh-platform-adapter.md](oh-platform-adapter.md)（Container/AceContainer/RosenWindow）
- [编译构建：[docs/kb/architecture/build-system.md](build-system.md)（旧管线 vs NG 管线构建选择）](build-system.md)
