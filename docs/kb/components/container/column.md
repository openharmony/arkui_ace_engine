...
Column 是 ArkUI 的纵向线性布局容器，应用侧可配置子项间距、主轴分布、交叉轴对齐和反向排列。NG 实现与 Row 共享 `linear_layout/` 下的 Pattern 和 Property，并通过 `LinearLayoutAlgorithm` 复用 Flex 布局算法。
...
Column 的核心 Model、Pattern 和布局算法未发现组件特有的跨仓依赖，因此不建立组件专属外部依赖表。公共布局调度和渲染同步应转到 Layout Framework；系统 API `pointLight` 走 `JSViewAbstract` / `ViewAbstractModelStatic` 的公共渲染属性路径，不属于 Column 线性布局算法本身。
...
| Feat-04 | Column PointLight（系统 API）光效 | `Feat-04-column-point-light-spec.md` |
...
- ColumnSplit 纵向分割布局：`docs/kb/components/container/column-split.md`
