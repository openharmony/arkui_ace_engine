...
Navigation 是 ArkUI 的导航容器组件，面向应用侧提供页面路由栈管理（NavPathStack）、NavDestination 推入/退出、转场动画、标题栏/工具栏配置、安全区避让、分栏/强制分屏等能力。Navigation + NavPathStack + NavDestination 构成当前推荐的导航体系。NavRouter 自 API version 13 起已标记 `@deprecated`，推荐使用 Navigation(NavPathStack) + NavDestination 替代。
...
| NavRouter Pattern（已废弃） | `frameworks/core/components_ng/pattern/navrouter/navrouter_pattern.cpp` | NavRouter 已于 API 13 deprecated，推荐 NavPathStack 替代 |
...
| ArkTS Bridge（NavDestination） | `frameworks/bridge/declarative_frontend/engine/jsi/nativeModule/arkts_native_nav_destination_bridge.cpp` | NavDestination Bridge → ModelNG |
| ArkTS Bridge（NavRouter，已废弃） | `frameworks/bridge/declarative_frontend/engine/jsi/nativeModule/arkts_native_nav_router_bridge.cpp` | NavRouter Bridge，API 13 deprecated |
...
| NavRouter 单元测试 | `test/unittest/core/pattern/navrouter/` | NavRouter、NavDestination Pattern 测试（NavRouter 部分仍保留但已 deprecated） |
...
- NavDestination：`docs/kb/components/container/navdestination.md`
