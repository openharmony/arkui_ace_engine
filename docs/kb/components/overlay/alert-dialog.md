...
AlertDialog **尚未组件化**，属性解析仍采用双路径架构，编译产物在主 `libace_compatible.z.so` 中。

| 路径 | 入口文件 | 说明 |
|------|----------|------|
| **JSView（声明式组件）** | `frameworks/bridge/declarative_frontend/jsview/dialog/js_alert_dialog.cpp` | `JSAlertDialog` → `AlertDialogModelNG::SetXxx()` |
| **node_modifier 层** | `interfaces/native/node/dialog_model.cpp` | C API 属性实现 |
| **NAPI** | `interfaces/napi/kits/promptaction/js_prompt_action.cpp` | `@ohos.promptAction.showDialog` NAPI 入口 |
...
- [ActionSheet 列表选择弹窗](action-sheet.md)
- [Dialog 弹窗组件](dialog.md)
- [弹窗类属性](../../capabilities/modal-attributes.md)