...
| C-API 实现 | `interfaces/native/node/node_utils.cpp/256/268` + `frameworks/core/interfaces/native/node/frame_node_modifier.cpp/945/1002` | NDK 入口 + modifier 转发 |
| ANI | `frameworks/core/interfaces/native/ani/common_ani_modifier.cpp/492` | SetCustomPropertyCallBack/GetCustomProperty |
...
| 路径 | 入口文件 | 说明 |
|------|----------|------|
| JS 设置 | `js_view_abstract.cpp` | JsCustomProperty 构建回调→SetJSCustomProperty |
| JS 存储 | `ArkComponent.ts` | __setCustomProperty__ 写 JS Map（value===undefined 移除） |
| 原生存储 | `frame_node.cpp/8426` | SetJSCustomProperty 写+置 flag "0"(stale)；GetJSCustomProperty flag "0" 经回调重取置 "1"(懒物化) |
| 读取 | `frame_node.ts` | __getCustomProperty__ JS 优先，miss 回退 getCustomPropertyCapiByKey |
| C-API | `node_utils.cpp/268` | AddCustomProperty 直写 {value,"1"}；GetCustomProperty 先 Capi 后 JS |
...
- **双存储状态**：`frame_node.h/1962` 的 `customPropertyMap_`(flag "0"=stale/"1"=valid) + `extraCustomPropertyMap_`(void* 侧信道)。
- **JS Map**：`ArkComponent.ts` `__elementIdToCustomProperties__` 为 Object 值源。
- **Inspector**：`frame_node.cpp` 序列化 customPropertyMap_ 为 JSON。

## 相关主题

- [FrameNode](./frame-node.md) — customProperty 存储于 FrameNode（04-06-02）
- [DrawModifier](./draw-modifier.md) — 自定义绘制（04-05-01）
- [Attribute Modifier](./attribute-modifier.md) — 动态属性通路（04-05-02）
- [Component Modifier](./component-modifier.md) — 命令式 Modifier 类（04-05-06）