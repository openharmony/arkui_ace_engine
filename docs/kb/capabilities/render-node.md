...
- [FrameNode](./frame-node.md) — `getRenderNode()` 返回的节点类型；RenderNode 可挂载到 FrameNode
...
- [BuilderNode（`04-06-04`）— 可将 RenderNode 嵌入；嵌入时 selfIdealSize 须显式](builder-node.md)
- [Shape/Mask 类型（Graphics.d.ts）— ShapeMask/ShapeClip 形状定义](../components/shape/shape.md)