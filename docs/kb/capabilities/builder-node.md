...
- [FrameNode](./frame-node.md) — `getFrameNode()` 返回的不可改根；BuilderNode 挂载到其他 FrameNode
- [RenderNode](./render-node.md) — 可嵌入 BuilderNode 的绘制节点
- [NodeAdapter（`04-06-06`）、ComponentContent（`04-06-05`）、TypedFrameNode（`04-06-07`）— 同属自定义节点能力簇（KB 待建）](node-adapter.md)