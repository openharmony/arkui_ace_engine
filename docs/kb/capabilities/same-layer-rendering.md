...
- [NodeContainer（生产者宿主）：`docs/kb/components/custom-node/node-container.md`](../components/custom-node/node-container.md)
- [ContentSlot（命令式节点占位）：`docs/kb/components/custom-node/content-slot.md`](../components/custom-node/content-slot.md)
- [占位组件（自定义节点概念入口）：`docs/kb/capabilities/placeholder-component.md`](placeholder-component.md)
