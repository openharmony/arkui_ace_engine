...
| 页面翻译请求发起 | `SendPageTranslateRequest` 校验 callback/request，注册 `PageTranslateTextCallback`，再注册 timeout cleanup callback | callback 为空或 scope 非法返回 `PARAM_INVALID`；已有未完成请求返回 `LAST_UNFINISH`；timeout cleanup callback 发送 `END_PAGE_TRANSLATE` | `lushi/translate:adapter/ohos/entrance/ui_session/ui_content_proxy.cpp:691`、`lushi/translate:adapter/ohos/entrance/ui_session/ui_content_proxy.cpp:717`、`lushi/translate:adapter/ohos/entrance/ui_session/ui_content_proxy.cpp:723` |
...
| 单次快照完成 | `SendPageText` 收到 `nodeId < 0 && text.empty()` finish signal | 取消当前 callback timeout，并以 `notifyTimeout=false` 清理 pending callback；不会误触发 End cleanup | `lushi/translate:adapter/ohos/entrance/ui_session/ui_report_stub.cpp:798`、`lushi/translate:adapter/ohos/entrance/ui_session/ui_report_stub.cpp:811`、`lushi/translate:adapter/ohos/entrance/ui_session/ui_report_stub.cpp:817` |
...
| 译文回填成功发送 | `SendPageTranslateResult` 解析 result 中可识别的 nodeId/version identity | IPC 发送成功后取消对应 result watchdog；解析失败不影响 IPC 失败返回码，但无法取消 watchdog | `lushi/translate:adapter/ohos/entrance/ui_session/ui_content_proxy.cpp:1253`、`lushi/translate:adapter/ohos/entrance/ui_session/ui_content_proxy.cpp:1258`、`lushi/translate:adapter/ohos/entrance/ui_session/ui_content_proxy.cpp:1272` |
