...
### 2. 后端的布局和绘制
 ...
 组件在后端的布局和绘制，需要相应地新增以下几个类：`MyCircleComponent`、`MyCircleElement`、`RenderMyCircle`、`RosenRenderMyCircle`。
...
#### 2.4 新增`RosenRenderMyCircle`类

##### 2.4.1 新增`rosen_render_mycircle.h`

文件路径：`frameworks\core\components\mycircle\rosen_render_mycircle.h`

```c++
class RosenRenderMyCircle final : public RenderMyCircle {
    DECLARE_ACE_TYPE(RosenRenderMyCircle, RenderMyCircle);

public:
    RosenRenderMyCircle() = default;
    ~RosenRenderMyCircle() override = default;
...
##### 2.4.2 新增`rosen_render_mycircle.cpp`

文件路径：`frameworks\core\components\mycircle\rosen_render_mycircle.cpp`
...
{
    return AceType::MakeRefPtr<RosenRenderMyCircle>();
}
```

`RenderMyCircle::Create()`在基类`RenderMyCircle`中定义，因为当前使用的是 Rosen 渲染后端，所以在`rosen_render_mycircle.cpp`里面实现，返回在 Rosen 后端上自渲染的`RosenRenderMyCircle`类。
...
```c++
void RosenRenderMyCircle::Paint(RenderContext& context, const Offset& offset)
{
