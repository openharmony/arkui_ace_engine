...
Array_ResourceColor GetResourceColorArray(int64_t thisArray, int32_t thisLength)
{
    ...
    Array_ResourceColor array_resource_color_buf = {};
    thisDeserializer.resizeArray<std::decay<decltype(array_resource_color_buf)>::type,
        std::decay<decltype(*array_resource_color_buf.array)>::type>(&array_resource_color_buf, colorArraySize);
    ...
    for (int i = 0; i < colorArraySize; i++) {
        ...
        if (value_buf_selector == 0) {
            ...
            value_buf.value0 = static_cast<Ark_Color>(thisDeserializer.readInt32());
            ...
            value_buf.value2 = static_cast<Ark_String>(thisDeserializer.readString());
        } else if (value_buf_selector == 3) {
            value_buf.selector = 3;
            value_buf.value3 = thisDeserializer.readResource();
        } else {
            LOGI("One of the branches for value_buf has to be chosen through deserialisation.");
        }
        array_resource_color_buf.array[i] = static_cast<Ark_ResourceColor>(value_buf);
    }
