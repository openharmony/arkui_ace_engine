...
export function StorageLinkState<Value>(storage: LocalStorage, name: string, value: Value): MutableState<Value> {
    return storage.map.link<Value>(name, value).state
}
...
export function AppStorageLinkState<Value>(name: string, value: Value): MutableState<Value> {
    return StorageMap.shared.link<Value>(name, value).state
}
...
export class AppStorage {
     ...
     */
    static get<T>(propName: string): T | undefined {
        return StorageMap.shared.get(propName)
    }
    ...
    static IsMutable(propName: string): boolean {
        return StorageMap.shared.entry(propName)?.mutable === true
    }
...
export class Environment {
    ...
    static EnvProps(props: { key: string, defaultValue: any }[]): void {
        Environment.envProps(props as EnvPropsOptions[])
    }
    /**
     * Called when multiple property values are checked.
     * @since 10
    ...
    static envProps(props: EnvPropsOptions[]): void {
        for (const prop of props) this.envProp(prop.key, prop.defaultValue)
    }
...
export class PersistentStorage {
    ...
    static PersistProps(properties: { key: string, defaultValue: any }[]): void {
        PersistentStorage.persistProps(properties as PersistPropsOptions[])
    }
    /**
     * Called when multiple persistence properties are stored.
     * @since 10
    ...
    static persistProps(properties: PersistPropsOptions[]): void {
        for (const prop of properties) this.persistProp(prop.key, prop.defaultValue)
    }
...
export class LocalStorage {
    ...
    get<T>(propName: string): T {
        return this.map.get(propName) as any // fix API
    }
...
///// Expose classes to the global space /////
...
Object.assign(globalThis, {
    ...
    SubscribedAbstractProperty: SubscribedAbstractProperty,
})

//////////////////////////////////////////////////

class StorageEntry<T> extends SubscribedAbstractProperty<T> {
    constructor(readonly state: MutableState<T>, readonly mutable = true) {
        super()
    }
...
class StorageMap {
    ...
    private readonly map = new Map<string, StorageEntry<any>>()

    static get shared(): StorageMap {
        if (this._shared?.manager != GlobalStateManager.instance) {
            this._shared = new StorageMap() // recreate shared storage if state manager is changed
        }
        return this._shared
    }
    ...
    constructor(initializer?: any) {
        if (initializer) {
            for (const key in initializer) {
                this.create(key, initializer[key])
            }
        }
    }
    ...
    set<T>(key: string, value: T, create = true, mutable = true): boolean {
    ...
    private create<T>(key: string, value: T, mutable = true): StorageEntry<T> {
        const entry = new StorageEntry(this.manager.mutableState<T>(value, true), mutable)
