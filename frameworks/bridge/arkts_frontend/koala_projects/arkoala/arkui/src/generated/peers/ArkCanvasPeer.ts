...
export class ArkCanvasPeer extends ArkCommonMethodPeer {
    ...
    setCanvasOptions0Attribute(context?: CanvasRenderingContext2D | DrawingRenderingContext): void {
        ...
        thisSerializer.writeInt8(context_type)
        if ((RuntimeType.UNDEFINED) != (context_type)) {
            ...
            if (((RuntimeType.OBJECT) == (context_value_type)) && (context_value instanceof CanvasRenderingContext2D)) {
                thisSerializer.writeInt8(0)
                const context_value_0 = unsafeCast<CanvasRenderingContext2D>(context_value)
                thisSerializer.writeCanvasRenderingContext2D(context_value_0)
            }
            else if (((RuntimeType.OBJECT) == (context_value_type)) && (context_value instanceof DrawingRenderingContext)) {
                thisSerializer.writeInt8(1)
                const context_value_1 = unsafeCast<DrawingRenderingContext>(context_value)
                thisSerializer.writeDrawingRenderingContext(context_value_1)
            }
        }
        ArkUIGeneratedNativeModule._CanvasInterface_setCanvasOptions0(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
    ...
    setCanvasOptions1Attribute(context: CanvasRenderingContext2D | DrawingRenderingContext, imageAIOptions: ImageAIOptions): void {
        ...
        let context_type: int32 = RuntimeType.UNDEFINED
        context_type = runtimeType(context)
        if (((RuntimeType.OBJECT) == (context_type)) && (context instanceof CanvasRenderingContext2D)) {
            thisSerializer.writeInt8(0)
            const context_0 = unsafeCast<CanvasRenderingContext2D>(context)
            thisSerializer.writeCanvasRenderingContext2D(context_0)
        }
        else if (((RuntimeType.OBJECT) == (context_type)) && (context instanceof DrawingRenderingContext)) {
            thisSerializer.writeInt8(1)
            const context_1 = unsafeCast<DrawingRenderingContext>(context)
            thisSerializer.writeDrawingRenderingContext(context_1)
        }
        thisSerializer.writeImageAIOptions(imageAIOptions)
        ArkUIGeneratedNativeModule._CanvasInterface_setCanvasOptions1(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
    onReadyAttribute(value: VoidCallback): void {
        const thisSerializer: Serializer = Serializer.hold()
        thisSerializer.holdAndWriteCallback(value)
        ArkUIGeneratedNativeModule._CanvasAttribute_onReady(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
    enableAnalyzerAttribute(value: boolean): void {
        ArkUIGeneratedNativeModule._CanvasAttribute_enableAnalyzer(this.peer.ptr, +value)
    }
}
