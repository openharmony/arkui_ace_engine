...
import { ArkRelativeContainerPeer } from "./peers/ArkRelativeContainerPeer"
...
import { GuideLineStyle, BarrierStyle, LocalizedBarrierStyle, GuideLinePosition, BarrierDirection, LocalizedBarrierDirection, RelativeContainerAttribute } from "./ArkRelativeContainerInterfaces"
/** @memo:stable */
export class ArkRelativeContainerComponent extends ArkCommonMethodComponent {
    ...
    /** @memo */
    public guideLine(value: Array<GuideLineStyle>): this {
        if (this.checkPriority("guideLine")) {
            const value_casted = value as (Array<GuideLineStyle>)
    ...
    /** @memo */
    public barrier(value: Array<BarrierStyle> | Array<LocalizedBarrierStyle>): this {
        if (this.checkPriority("barrier")) {
            const value_type = runtimeType(value)
            if (((RuntimeType.OBJECT) == (value_type)) && (value instanceof Array<BarrierStyle>)) {
                const value_casted = value as (Array<BarrierStyle>)
                this.getPeer()?.barrier0Attribute(value_casted)
                return this
            }
            if (((RuntimeType.OBJECT) == (value_type)) && (value instanceof Array<LocalizedBarrierStyle>)) {
                const value_casted = value as (Array<LocalizedBarrierStyle>)
...
/** @memo */
export function ArkRelativeContainer(
  /** @memo */
  style: ((attributes: ArkRelativeContainerComponent) => void) | undefined,
  /** @memo */
