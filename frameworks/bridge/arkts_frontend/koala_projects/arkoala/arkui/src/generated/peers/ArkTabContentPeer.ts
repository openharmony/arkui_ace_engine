...
import { TabBarOptions, SelectedMode, BoardStyle, TabBarSymbol, LayoutMode, TabBarIconStyle, TabContentAttribute } from "./../ArkTabContentInterfaces"
...
export class ArkTabContentPeer extends ArkCommonMethodPeer {
    ...
    tabBar0Attribute(value: string | Resource | CustomBuilder | TabBarOptions): void {
        ...
        if (RuntimeType.STRING == value_type) {
            ...
            const value_2 = unsafeCast<CustomBuilder>(value)
            thisSerializer.holdAndWriteCallback(CallbackTransformer.transformFromCustomBuilder(value_2))
        }
        else if (((RuntimeType.OBJECT) == (value_type)) && (((value!.hasOwnProperty("icon")) || (value!.hasOwnProperty("text"))))) {
            thisSerializer.writeInt8(3)
            const value_3 = unsafeCast<TabBarOptions>(value)
            thisSerializer.writeTabBarOptions(value_3)
        }
        ArkUIGeneratedNativeModule._TabContentAttribute_tabBar0(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
    ...
    tabBar1Attribute(value: SubTabBarStyle | BottomTabBarStyle): void {
        ...
        let value_type: int32 = RuntimeType.UNDEFINED
        value_type = runtimeType(value)
        if (((RuntimeType.OBJECT) == (value_type)) && (value instanceof SubTabBarStyle)) {
            thisSerializer.writeInt8(0)
            const value_0 = unsafeCast<SubTabBarStyle>(value)
            thisSerializer.writeSubTabBarStyle(value_0)
        }
        else if (((RuntimeType.OBJECT) == (value_type)) && (value instanceof BottomTabBarStyle)) {
            thisSerializer.writeInt8(1)
            const value_1 = unsafeCast<BottomTabBarStyle>(value)
            thisSerializer.writeBottomTabBarStyle(value_1)
        }
        ArkUIGeneratedNativeModule._TabContentAttribute_tabBar1(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
    onWillShowAttribute(value: VoidCallback): void {
        const thisSerializer: Serializer = Serializer.hold()
        thisSerializer.holdAndWriteCallback(value)
        ArkUIGeneratedNativeModule._TabContentAttribute_onWillShow(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
    onWillHideAttribute(value: VoidCallback): void {
        const thisSerializer: Serializer = Serializer.hold()
        thisSerializer.holdAndWriteCallback(value)
        ArkUIGeneratedNativeModule._TabContentAttribute_onWillHide(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
}
export interface ArkTabContentAttributes extends ArkCommonMethodAttributes {
    tabBar?: string | Resource | CustomBuilder | TabBarOptions
    onWillShow?: VoidCallback
    onWillHide?: VoidCallback
}
