...
export class StructsResolver {
    ...
    getOrCreateTable(fileName: string, restart: boolean) {
        ...
        if (!result) {
            result = new StructTable(fileName, restart ? undefined : this)
...
export class StructTable {
    ...
    constructor(private fileName: string, private resolverToSync: StructsResolver|undefined) {
        if (!resolverToSync) {
            this.readDB()
        }
    }
    ...
    update() {
        ...
        fs.writeFileSync(metaDatabase(this.fileName), result)
    }
}
