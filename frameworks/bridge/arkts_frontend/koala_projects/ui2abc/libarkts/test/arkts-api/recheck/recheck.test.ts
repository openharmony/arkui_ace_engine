...
function applyTransformOnStage(transform?: arkts.ProgramTransformer, onlyModifyMain?: boolean, stage: arkts.Es2pandaContextState = arkts.Es2pandaContextState.ES2PANDA_STATE_CHECKED) {
    arkts.programGetExternalSources(arkts.arktsGlobal.compilerContext.program).forEach(it => {
        ...
        it.programs.forEach(program => {
            const ast = program.ast as arkts.ETSModule
            const importStorage = new arkts.ImportStorage(program, false)
            if (!onlyModifyMain) {
                transform?.(program, { isMainProgram: false, name: "", stage }, new arkts.PluginContextImpl())
            }
    ...
    transform?.(arkts.arktsGlobal.compilerContext.program, { isMainProgram: true, name: "", stage }, new arkts.PluginContextImpl())
