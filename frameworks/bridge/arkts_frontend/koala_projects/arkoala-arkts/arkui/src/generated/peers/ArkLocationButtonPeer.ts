...
export class ArkLocationButtonPeer extends ArkSecurityComponentMethodPeer {
    ...
    setLocationButtonOptions1Attribute(options: LocationButtonOptions): void {
        ...
        ArkUIGeneratedNativeModule._LocationButtonInterface_setLocationButtonOptions1(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
    onClickAttribute(value: ((event: ClickEvent,result: LocationButtonOnClickResult) => void)): void {
        const thisSerializer : Serializer = Serializer.hold()
        thisSerializer.holdAndWriteCallback(value)
        ArkUIGeneratedNativeModule._LocationButtonAttribute_onClick(this.peer.ptr, thisSerializer.asArray(), thisSerializer.length())
        thisSerializer.release()
    }
}
