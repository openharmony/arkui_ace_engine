...
public:
    ...
    DeserializerBase(uint8_t* data, int32_t length) : data(data), length(length), position(0) {}
    ...
    void check(int32_t count)
    {
        if (position + count > length) {
            fprintf(
                stderr, "Incorrect serialized data, check for %d, buffer %d position %d\n", count, length, position);
            ASSERT(false);
            abort();
        }
    }
    ...
    InteropCustomObject readCustomObject(std::string kind)
    {
        ...
        auto tag = readTag();
        if (tag == INTEROP_TAG_UNDEFINED)
            LOGE("Undefined interop tag");
        // Skip undefined tag!.
        InteropCustomObject result;
        InteropStringCopy(result.kind, sizeof(result.kind), "Error");
    ...
    InteropCallbackResource readCallbackResource()
    {
        ...
        result.hold = reinterpret_cast<void (*)(InteropInt32)>(
            readPointerOrDefault(reinterpret_cast<void*>(holdManagedCallbackResource)));
        result.release = reinterpret_cast<void (*)(InteropInt32)>(
            readPointerOrDefault(reinterpret_cast<void*>(releaseManagedCallbackResource)));
        return result;
    }
