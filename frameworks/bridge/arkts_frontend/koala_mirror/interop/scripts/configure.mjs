...
import { OH_SDK_COMPONENT, ohSdkInfo } from "../../tools/ohos-tools/ohos-sdk/download-ohos-sdk.mjs"
import url from "url"
const require = createRequire(import.meta.url);

const __dirname = path.dirname(url.fileURLToPath(import.meta.url))

const OHCONF = ohConf(path.join(__dirname, "../../tools/ohos-tools/ohos-sdk/.ohconf.json"))
...
function createOhosCrossFile(target) {
    ...
    if (platform == 'linux') {
        compilersPath = path.resolve('../arkoala-arkts/tools/compiler/llvm-toolchain/bin')
    } else {
    ...
    let cflags = [
        '--sysroot=' + path.resolve(`../arkoala-arkts/tools/compiler/sysroot-${ target.includes('arm64') ? 'arm64' : 'arm' }`)
    ]
    let cpu = 'unknown'
