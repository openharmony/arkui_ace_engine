...
import { Size, Frame, Vector2, Vector3, Matrix4, Edges, BorderRadiuses_graphics, ShapeMask, ShapeMaskInternal, ShapeClip, ShapeClipInternal, LengthMetricsUnit, DrawContext } from "./arkui-graphics"
...
export class RenderNodeInternal implements MaterializedBase,RenderNode {
    ...
    static ctor_rendernode(): KPointer {
        const retval  = ArkUIGeneratedNativeModule._RenderNode_ctor()
        return retval
    }
    constructor() {
        const ctorPtr : KPointer = RenderNodeInternal.ctor_rendernode()
        this.peer = new Finalizable(ctorPtr, RenderNodeInternal.getFinalizer())
    }
    ...
    private insertChildAfter_serialize(child: RenderNode, sibling: RenderNode | undefined): void {
        ...
        thisSerializer.writeInt8(sibling_type as int32)
        if ((RuntimeType.UNDEFINED) != (sibling_type)) {
            const sibling_value  = sibling!
            thisSerializer.writeRenderNode(sibling_value)
        }
    ...
    public static fromPtr(ptr: KPointer): RenderNodeInternal {
        const obj : RenderNodeInternal = new RenderNodeInternal()
        obj.peer = new Finalizable(ptr, RenderNodeInternal.getFinalizer())
        return obj
    }
}
