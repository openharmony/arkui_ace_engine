...
export class GlobalScope {
    ...
    static borderRadiuses_serialize(all: number): BorderRadiuses {
        ...
        let retvalDeserializer : Deserializer = new Deserializer(retval, retval.length)
        const buffer_topLeft_buf_runtimeType  = (retvalDeserializer.readInt8() as int32)
        let buffer_topLeft_buf : Length | undefined
        if ((RuntimeType.UNDEFINED) != (buffer_topLeft_buf_runtimeType))
        {
            const buffer_topLeft_buf__selector : int32 = retvalDeserializer.readInt8()
            let buffer_topLeft_buf_ : string | number | Resource | undefined
            if (buffer_topLeft_buf__selector == 0) {
                buffer_topLeft_buf_ = (retvalDeserializer.readString() as string)
            }
            else if (buffer_topLeft_buf__selector == 1) {
                buffer_topLeft_buf_ = (retvalDeserializer.readNumber() as number)
            }
            else if (buffer_topLeft_buf__selector == 2) {
                buffer_topLeft_buf_ = retvalDeserializer.readResource()
            }
            else {
                throw new Error("One of the branches for buffer_topLeft_buf_ has to be chosen through deserialisation.")
            }
            buffer_topLeft_buf = (buffer_topLeft_buf_ as string | number | Resource)
        }
        const buffer_topLeft : undefined | Length = buffer_topLeft_buf
        const buffer_topRight_buf_runtimeType  = (retvalDeserializer.readInt8() as int32)
        let buffer_topRight_buf : Length | undefined
        if ((RuntimeType.UNDEFINED) != (buffer_topRight_buf_runtimeType))
        {
            const buffer_topRight_buf__selector : int32 = retvalDeserializer.readInt8()
            let buffer_topRight_buf_ : string | number | Resource | undefined
            if (buffer_topRight_buf__selector == 0) {
                buffer_topRight_buf_ = (retvalDeserializer.readString() as string)
            }
            else if (buffer_topRight_buf__selector == 1) {
                buffer_topRight_buf_ = (retvalDeserializer.readNumber() as number)
            }
            else if (buffer_topRight_buf__selector == 2) {
                buffer_topRight_buf_ = retvalDeserializer.readResource()
            }
            else {
                throw new Error("One of the branches for buffer_topRight_buf_ has to be chosen through deserialisation.")
            }
            buffer_topRight_buf = (buffer_topRight_buf_ as string | number | Resource)
        }
        const buffer_topRight : undefined | Length = buffer_topRight_buf
        const buffer_bottomLeft_buf_runtimeType  = (retvalDeserializer.readInt8() as int32)
        let buffer_bottomLeft_buf : Length | undefined
        if ((RuntimeType.UNDEFINED) != (buffer_bottomLeft_buf_runtimeType))
        {
            const buffer_bottomLeft_buf__selector : int32 = retvalDeserializer.readInt8()
            let buffer_bottomLeft_buf_ : string | number | Resource | undefined
            if (buffer_bottomLeft_buf__selector == 0) {
                buffer_bottomLeft_buf_ = (retvalDeserializer.readString() as string)
            }
            else if (buffer_bottomLeft_buf__selector == 1) {
                buffer_bottomLeft_buf_ = (retvalDeserializer.readNumber() as number)
            }
            else if (buffer_bottomLeft_buf__selector == 2) {
                buffer_bottomLeft_buf_ = retvalDeserializer.readResource()
            }
            else {
                throw new Error("One of the branches for buffer_bottomLeft_buf_ has to be chosen through deserialisation.")
            }
            buffer_bottomLeft_buf = (buffer_bottomLeft_buf_ as string | number | Resource)
        }
        const buffer_bottomLeft : undefined | Length = buffer_bottomLeft_buf
        const buffer_bottomRight_buf_runtimeType  = (retvalDeserializer.readInt8() as int32)
        let buffer_bottomRight_buf : Length | undefined
        if ((RuntimeType.UNDEFINED) != (buffer_bottomRight_buf_runtimeType))
        {
            const buffer_bottomRight_buf__selector : int32 = retvalDeserializer.readInt8()
            let buffer_bottomRight_buf_ : string | number | Resource | undefined
            if (buffer_bottomRight_buf__selector == 0) {
                buffer_bottomRight_buf_ = (retvalDeserializer.readString() as string)
            }
            else if (buffer_bottomRight_buf__selector == 1) {
                buffer_bottomRight_buf_ = (retvalDeserializer.readNumber() as number)
            }
            else if (buffer_bottomRight_buf__selector == 2) {
                buffer_bottomRight_buf_ = retvalDeserializer.readResource()
            }
            else {
                throw new Error("One of the branches for buffer_bottomRight_buf_ has to be chosen through deserialisation.")
            }
            buffer_bottomRight_buf = (buffer_bottomRight_buf_ as string | number | Resource)
        }
        const buffer_bottomRight : undefined | Length = buffer_bottomRight_buf
        const returnResult : BorderRadiuses = ({topLeft: buffer_topLeft, topRight: buffer_topRight, bottomLeft: buffer_bottomLeft, bottomRight: buffer_bottomRight} as BorderRadiuses)
        return returnResult
    }
