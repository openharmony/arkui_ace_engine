...
import { assertResultArray, GlobalStateHolder } from "../utils"
...
// Improve: This test fails to compile @memo annotation attached to typedef, please fix
...
//         type MemoT =
//         /** @memo */
//         (this: Dummy, log: Array<string>) => void
//         /** @memo */
//         let f: MemoT = (this: Dummy, log: Array<string>): void => {
...
class TestGetterAndSetter extends Log {
    /** @memo */
    testGetter() {
        GlobalStateHolder.globalState.value
        this.prop(123)
    ...
    private newFunc:
        ...
        /** @memo */
        (x: number) => {
            this.log.push(`prop function call: ${2*x}`);
        }

    public callSetter() {
        this.prop = this.newFunc
    }

    get prop():
    /** @memo */
    (x: number) => void {
        this.log.push("getter call");
		return (
            /** @memo */
            (x: number) => {
                this.log.push(`prop function call: ${x}`);
            }
        )
	}
	...
	set prop(
        ...
        this.log.push(`setter call`);
	}
}

class TestTrailingLambda extends Log {
    /** @memo */
    test() {
        GlobalStateHolder.globalState.value
        this.log.push(`test function call`)
        this.trailingLambda() { this.log.push(`trailing lambda call`) }
    }

    /** @memo */
    private trailingLambda(
        /** @memo */
        f: (s: string) => void
    ) {
        f("123")
...
suite("[ARKTS+MEMO PLUGIN]", () => {
    suite("Memoizing non-primitive arguments", () => {
        ...
        // Improve: trailing lambda with receiver, trailing lambda with optional param
        test("Memo is assumed when pasing a TRAILING lambda argument to a memo parameter", () => {
            const test = new TestTrailingLambda()
            const root = testRoot(
                /** @memo */
                () => { test.test() }
            )
    ...
    suite("Functions with receiver", () => {
        ...
        // Improve: check but cause
        // Improve: This test fails to compile @memo annotation attached to typedef, please fix
        // test.expectFailure("Improve: fails in runtime, probably a bug in call AST", "Lambda function with receiver", () => {
    ...
    suite("Getter and Setter with property of memo function type", () => {
        test("Getter and Setter of functional type", () => {
            const instance: TestGetterAndSetter = new TestGetterAndSetter()
            const root = testRoot(
                /** @memo */
                () => {instance.testGetter()}
            )
