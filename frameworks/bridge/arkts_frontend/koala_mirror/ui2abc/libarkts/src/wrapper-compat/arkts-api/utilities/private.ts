...
export function passNodeArray(nodes: readonly AstNode[] | undefined): BigUint64Array {
    return new BigUint64Array(nodes?.map((node) => BigInt(node.peer)) ?? []);
}

export function unpackNonNullableObject<T extends ArktsObject>(
    Type: { new (peer: KNativePointer): T },
    peer: KNativePointer
): T {
    if (peer === nullptr) {
...
export function unpackObject<T extends ArktsObject>(
    Type: { new (peer: KNativePointer): T },
    peer: KNativePointer
): T | undefined {
    if (peer === nullptr) {
...
export { unpackString } from "../../../arkts-api/utilities/private"
...
export function updatePeerByNode<T extends AstNode>(peer: KNativePointer, original: T | undefined): KNativePointer {
    ...
    global.generatedEs2panda._AstNodeSetParent(
        global.context,
        peer,
 ...
 * @deprecated
 */
export function compose<T extends AstNode, ARGS extends any[]>(
    create: (...args: ARGS) => T,
...
export function updateThenAttach<T extends AstNode, ARGS extends any[]>(
...
export function attachModifiers<T extends AstNode>(node: T, original: T): T {
