...
void ShowDialogInner(DialogProperties& dialogProperties,
...
void SetBorder(DialogProperties& dialogProperties, const NativeCustomDialogOptions& options)
{
    ...
    NG::BorderStyleProperty borderStyle = {
        .styleLeft = BorderStyle(options.borderEdgeStyle.left),
    ...
    NG::BorderWidthPropertyT<Dimension> borderWidth = {
        ...
        .bottomDimen = Dimension(
            options.borderWidth.bottom, static_cast<DimensionUnit>(options.borderWidth.bottomUnit))
    };
    ...
    NG::BorderRadiusPropertyT<Dimension> borderRadius = NG::BorderRadiusPropertyT<Dimension>(
        ...
        Dimension(options.cornerRadius.bottomRightRadiuses,
            static_cast<DimensionUnit>(options.cornerRadius.bottomRightUnit)),
        Dimension(options.cornerRadius.bottomLeftRadiuses,
            static_cast<DimensionUnit>(options.cornerRadius.bottomLeftUnit))
    );
...
{
    auto builderFunc = [func = CJLambda::Create(options.builder)]() {
        func();
    };
    ...
    // Parse action
    auto onDidAppear = [lambda = CJLambda::Create(options.onDidAppear)]() {
        lambda();
    };
    auto onDidDisappear = [lambda = CJLambda::Create(options.onDidAppear)]() {
        lambda();
    };
    auto onWillAppear = [lambda = CJLambda::Create(options.onDidAppear)]() {
        lambda();
    };
    auto onWillDisappear = [lambda = CJLambda::Create(options.onDidAppear)]() {
        lambda();
    };
...
{
    std::vector<ButtonInfo> buttons = CreateButtonInfoVector(options.buttons,
        options.buttonsSize, SHOW_DIALOG_BUTTON_NUM_MAX);
    std::set<std::string> callbacks;
    callbacks.emplace("success");
    callbacks.emplace("cancel");
