...
void FfiOHOSAceFrameworkSliderSlideRange(float from, float to, bool from_Exist, bool to_Exist)
{
    if (!from_Exist && !to_Exist) {
        SliderModel::GetInstance()->ResetValidSlideRange();
        return;
    }
    float fromValue = from_Exist ? from : std::numeric_limits<float>::infinity();
    float toValue = to_Exist ? to : std::numeric_limits<float>::infinity();
