...
extern "C" {
    ...
    int32_t FfiAnimatorReset(int64_t id, CAnimatorOptions cOption)
    {
        ...
        if (onframeRef) {
            auto onFrameCallback = [
                onframeRef, weakOption = std::weak_ptr<AnimatorOption>(nativeAnimatorResult->GetAnimatorOption())]
                (double value) {
                    auto option = weakOption.lock();
                    ACE_SCOPED_TRACE("ohos.animator onframe. duration:%d, curve:%s",
                        option->duration, option->easing.c_str());
                    onframeRef(value);
                };
        ...
        if (!animator->HasScheduler()) {
            ...
            if (!result) {
                TAG_LOGW(AceLogTag::ACE_ANIMATION,
                    "Animator: play failed, animator is not bound to specific context, id:%{public}d",
                    animator->GetId());
                return INIT_ERROR;
            }
        }
    ...
    int32_t FfiAnimatorReverse(int64_t id)
    {
        ...
        if (!animator->HasScheduler()) {
            ...
            if (!result) {
                TAG_LOGW(AceLogTag::ACE_ANIMATION,
                    "JsAnimator: reverse failed, animator is not bound to specific context");
                return INIT_ERROR;
            }
        }
        animator->Reverse();
