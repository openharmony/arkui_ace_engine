...
ArkUINativeModuleValue ImageAnimatorBridge::SetImages(ArkUIRuntimeCallInfo *runtimeCallInfo)
{
    ...
    auto srcArray = std::make_unique<char *[]>(arrayLength);
    ...
    if (!ArkTSUtils::ParseStringArray(vm, secondArg, srcArray.get(), arrayLength) ||
        !ArkTSUtils::ParseArray<CalcDimension>(vm, thirdArg, calcDimension.get() + arrayLength * NUM_0, arrayLength,
        parseDimensionStruct) ||
        !ArkTSUtils::ParseArray<CalcDimension>(vm, fourthArg, calcDimension.get() + arrayLength * NUM_1, arrayLength,
        parseDimensionStruct) ||
        !ArkTSUtils::ParseArray<CalcDimension>(vm, fifthArg, calcDimension.get() + arrayLength * NUM_2, arrayLength,
        parseDimensionStruct) ||
        !ArkTSUtils::ParseArray<CalcDimension>(vm, sixthArg, calcDimension.get() + arrayLength * NUM_3, arrayLength,
        parseDimensionStruct) ||
