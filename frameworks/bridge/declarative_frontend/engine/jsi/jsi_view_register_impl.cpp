...
const std::unordered_map<std::string, std::function<void(BindingTarget)>>& GetFormBindFuncsMap()
{
    ...
    return formBindFuncs;
}

const std::unordered_map<std::string, std::function<void(BindingTarget)>>& GetBindFuncsMap()
{
...
{
    auto container = Container::Current();
    auto& bindFuncs = GetBindFuncsMap();
...
void RegisterAllFormModule(BindingTarget globalObj, void* nativeEngine)
{
    ...
    JSLinearGradientBinding::JSBind(globalObj);
    JSColorMetricsLinearGradientBinding::JSBind(globalObj);
    auto& formBindFuncs = GetFormBindFuncsMap();
