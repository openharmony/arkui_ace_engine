...
declare class ViewV2 extends PUV2ViewBase {
    ...
    get __nativeId__Internal(): number;
}
...
export class Puv2ViewBaseTestsV2 implements ITestFile {
    ...
    static withNativeIdMocks<T>(
        assignedElmtId: number,
        subscriberId: number,
        task: () => T
    ): T {
        const originalGetElmtIdToAccountFor = (ViewStackProcessor as any).GetElmtIdToAccountFor;
        const originalMakeId = (SubscriberManager as any).MakeId;
        (ViewStackProcessor as any).GetElmtIdToAccountFor = (): number => assignedElmtId;
        (SubscriberManager as any).MakeId = (): number => subscriberId;
        try {
            return task();
        } finally {
            (ViewStackProcessor as any).GetElmtIdToAccountFor = originalGetElmtIdToAccountFor;
            (SubscriberManager as any).MakeId = originalMakeId;
        }
    }
    ...
    /**
     * Test __nativeId__Internal falls back to front-end id when no native id is assigned
     * Expected: getter returns id_ generated by SubscriberManager.MakeId()
     */
    public testNativeIdGetterFallsBackToFrontIdWhenNoAssignedNativeId(): void {
        const subscriberId = 701;
        Puv2ViewBaseTestsV2.withNativeIdMocks(UINodeRegisterProxy.notRecordingDependencies, subscriberId, () => {
            const view = new ViewV2(undefined, UINodeRegisterProxy.notRecordingDependencies, undefined);
            eq(view.id__(), subscriberId, "front-end id should use SubscriberManager.MakeId result");
            eq(view.__nativeId__Internal, subscriberId, "native id getter should fall back to front-end id");
        });
    }

    /**
     * Test __nativeId__Internal returns the separately assigned native id
     * Expected: getter returns ViewStackProcessor.GetElmtIdToAccountFor() while id__() keeps subscriber id
     */
    public testNativeIdGetterReturnsAssignedNativeIdWhenDifferentFromFrontId(): void {
        const assignedNativeId = 811;
        const subscriberId = 912;
        Puv2ViewBaseTestsV2.withNativeIdMocks(assignedNativeId, subscriberId, () => {
            const view = new ViewV2(undefined, UINodeRegisterProxy.notRecordingDependencies, undefined);
            eq(view.id__(), subscriberId, "front-end id should still come from SubscriberManager.MakeId");
            neq(view.__nativeId__Internal, view.id__(), "native id should be stored separately from front-end id");
            eq(view.__nativeId__Internal, assignedNativeId, "native id getter should return assigned native id");
        });
    }

    /**
     * Test explicit element id keeps native and front ids aligned
     * Expected: getter returns the explicit constructor id and bypasses split-id behavior
     */
    public testNativeIdGetterUsesExplicitConstructorId(): void {
        let getElmtIdCallCount = 0;
        let makeIdCallCount = 0;
        const originalGetElmtIdToAccountFor = (ViewStackProcessor as any).GetElmtIdToAccountFor;
        const originalMakeId = (SubscriberManager as any).MakeId;
        (ViewStackProcessor as any).GetElmtIdToAccountFor = (): number => {
            getElmtIdCallCount++;
            return 9991;
        };
        (SubscriberManager as any).MakeId = (): number => {
            makeIdCallCount++;
            return 9992;
        };
        try {
            const explicitElmtId = 456;
            const view = new ViewV2(undefined, explicitElmtId, undefined);
            eq(view.id__(), explicitElmtId, "front-end id should use explicit constructor id");
            eq(view.__nativeId__Internal, explicitElmtId, "native id getter should match explicit constructor id");
            eq(getElmtIdCallCount, 0, "constructor should not query ViewStackProcessor when explicit id is provided");
            eq(makeIdCallCount, 0, "constructor should not call SubscriberManager.MakeId when explicit id is provided");
        } finally {
            (ViewStackProcessor as any).GetElmtIdToAccountFor = originalGetElmtIdToAccountFor;
            (SubscriberManager as any).MakeId = originalMakeId;
        }
