...
abstract class ViewPU extends NativeViewPartialUpdate
  ...
  */
  ...
  protected aboutToBeDeletedInternal(): void {
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): aboutToBeDeletedInternal`);
    ...
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): onUnRegElementID`);
    ...
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): onUnRegElementID  - DONE`);
  ...
  public purgeDeletedElmtIdsRecursively (): void {
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIdsRecursively  - start`);
    // request list of all (global) elmtIds of deleted UINodes that need to be unregistered
    UINodeRegisterProxy.obtainDeletedElmtIds();
    this.purgeDeletedElmtIdsRecursivelyInternal();
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIdsRecursively  - DONE`);
  }

  private purgeDeletedElmtIdsRecursivelyInternal() : void {
    if (!UINodeRegisterProxy.hasElmtIdsPendingUnregister()) {
      stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIdsRecursively - registry has no more elmtIds needing unregister, therefore, can terminate recursion.`);
      return;
    }
    ...
    this.childrenWeakrefMap_.forEach((weakRefChild: WeakRef<ViewPU>) => {
      ...
      if (child) {
        if (!UINodeRegisterProxy.hasElmtIdsPendingUnregister()) {
          stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIdsRecursively - registry has no more elmtIds needing unregister (globally), therefore, can terminate recursion.`); 
          return;
        }
  ...
  private purgeDeletedElmtIdsInternal(): void {
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIds -  start.`);
    ...
    for (const rmElmtId of elmtIdsOfThisView) {
      if (UINodeRegisterProxy.consume(rmElmtId)) {
        ...
        if (!UINodeRegisterProxy.hasElmtIdsPendingUnregister()) {
          stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIdsInternal - registry has no more elmtIds needing unregister (globally), therefore, can terminate for this @Component early.`);
          return;
        }
    ...
    stateMgmtConsole.debug(`${this.constructor.name}(${this.id__()}): purgeDeletedElmtIds: DONE `);
  }
  ...
  public updateDirtyElements() {
    do {
      stateMgmtConsole.debug(`View ${this.constructor.name} elmtId ${this.id__()}:  updateDirtyElements: sorted dirty elmtIds:
${JSON.stringify(Array.from(this.dirtDescendantElementIds_).sort(ViewPU.compareNumber))}, starting ....`);
  ...
  protected purgeDeletedElmtIds(): void {
    ...
    UINodeRegisterProxy.obtainDeletedElmtIds();
    if (!UINodeRegisterProxy.hasElmtIdsPendingUnregister()) {
      stateMgmtConsole.debug(`View ${this.constructor.name} elmtId ${this.id__()}: purgeDeletedElmtIds - no elmtIds to unregister (globally) - done!`)
      return;
    }
    this.purgeDeletedElmtIdsInternal();
  }
