...
void ParseEmitterPropertyAnnulus(const JSRef<JSObject>& paramObj, EmitterProperty& emitterProperty)
{
    ...
    if (annulusRegionProperty->IsObject()) {
        ...
        innerRadiusValue =
            ((innerRadiusValue.Unit() == DimensionUnit::PERCENT) || LessOrEqual(innerRadiusValue.Value(), 0.0))
            ? 0.0_px
            : innerRadiusValue;
        outerRadiusValue =
            ((outerRadiusValue.Unit() == DimensionUnit::PERCENT) || LessOrEqual(outerRadiusValue.Value(), 0.0))
            ? 0.0_px
            : outerRadiusValue;
