...
{
    if (!argument || !vm) {
        LOGE("ToJsValueHelper: argument or vm is null");
    ...
    switch (argument->GetType()) {
        ...
        case WebJSValue::Type::LIST: {
            ...
            for (size_t i = 0; i < length; ++i) {
                ...
                if (!panda::ArrayRef::SetValueAt(vm, arr, i, ToJsValueHelper(vm, nPtr))) {
                    LOGE("ToJsValueHelper: SetValueAt api call failed");
                    return panda::JSValueRef::Undefined(vm);
                }
            }
            return arr;
        ...
        case WebJSValue::Type::NONE:
            return panda::JSValueRef::Undefined(vm);
        default:
            LOGE("ToJsValueHelper: jsvalue type not support!");
            break;
    }
    return panda::JSValueRef::Undefined(vm);
}
...
{
    if (!webviewValue) {
        LOGE("ParseValue: webviewValue or resultValue is null");
    ...
    if (resultValue->IsBoolean()) {
        ...
        LOGE("ParseValue: object is not supported");
        webviewValue->SetType(WebJSValue::Type::NONE);
