...
def main(argv):
    if len(argv) < 4:
        print("Usage: python script.py <path_to_project> <path_to_node_modules> <js_output_path>")
        sys.exit(1)
    ...
    output_file = os.path.join(js_output_path, "jsXNode.js")

    print(f"XNode: Changing directory to {project_path}. Out dir = {js_output_path}")
    os.chdir(project_path)

    # Check if `node_modules` exists. If yes skip npm install
    if not os.path.exists(node_modules_path):

        print(f"XNode: node_modules directory not found at {node_modules_path}, running npm install")
        ...
        print(f"XNode: node_modules directory exists at {node_modules_path}")

    try:
        subprocess.check_call(["npm", "run", "build_state_mgmt"])
    except subprocess.CalledProcessError as e:
        print(f"Error: 'npm run build_state_mgmt' failed with exit code {e.returncode}.")
    ...
    source_folder = "dist"
    built_file = os.path.join(project_path, source_folder, "jsXNode.js")
    ...
    print(f"XNode: Copying {built_file} to {output_file}")

    try:
        shutil.copy(built_file, output_file)
        print(f"XNode: File successfully copied to {output_file}")
...
if __name__ == '__main__':
    ...
    print(f"XNode: build time: {elapsed_time:.2f} seconds")
