...
{
    static const CJUICanvasRenderingContext2DModifier* cachedModifier = nullptr;
    if (cachedModifier == nullptr) {
        auto* module = DynamicModuleHelper::GetInstance().GetDynamicModule("Canvas");
        CHECK_NULL_RETURN(module, nullptr);
        cachedModifier = reinterpret_cast<const CJUICanvasRenderingContext2DModifier*>(
            module->GetCustomModifier("CJCanvasRenderingContext2D"));
    }
    return cachedModifier;
}
} // namespace NodeModifier
