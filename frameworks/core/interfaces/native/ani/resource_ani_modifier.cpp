...
// }
// } // namespace
// bool ParseAniColor(ani_env* env, ani_object aniValue, Color& color, RefPtr<ResourceObject>& resObj)
// {
//     ani_status status;
...
uint32_t GetColorById(int32_t id, int32_t type, std::string bundleName, std::string moduleName)
{
    ...
    auto resAdapter = ResourceManager::GetInstance().GetOrCreateResourceAdapter(resObj);
    auto color = resAdapter->GetColor(static_cast<int32_t>(id));
    return color.GetValue();
}

const ArkUIResourceModifier* GetResourceAniModifier()
{
    static const ArkUIResourceModifier impl = {
        .getColorById = OHOS::Ace::NG::GetColorById,
    };
    return &impl;
}
} // namespace OHOS::Ace::NG
