...
void AssignArkValue(Ark_RichEditorImageSpanResult& dst, const RichEditorAbstractSpanResult& src, ConvContext *ctx)
{
    ...
    if (src.GetMargin().length()) {
        margin = ArkUnion<Opt_Union_Dimension_Margin, Ark_Dimension>(src.GetMargin(), ctx);
    } else {
    ...
    if (src.GetBorderRadius().length()) {
        borderRadius = ArkUnion<Opt_Union_Dimension_BorderRadiuses, Ark_Dimension>(src.GetBorderRadius(), ctx);
    } else {
