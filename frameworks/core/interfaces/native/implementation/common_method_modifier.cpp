              ...
              const Ark_SizeOptions* value)
{
    ...
    if (width) {
        Width0Impl(node, &width.value());
    }
    auto height = Converter::OptConvert<Ark_Length>(value->height);
    if (height) {
        Height0Impl(node, &height.value());
    }
}
                     ...
                     const Ark_BorderImageOption* value)
{
    ...
    Converter::VisitUnion(value->source,
    ...
    Converter::WithOptional(value->outset, [&borderImage](const auto& src) {
        switch (src.selector) {
            case CASE_0: {
    ...
    Converter::WithOptional(value->slice, [&borderImage](const auto& src) {
        switch (src.selector) {
            case CASE_0: {
    ...
    Converter::WithOptional(value->width, [&borderImage](const auto& src) {
        switch (src.selector) {
            case CASE_0: {
                     ...
                     const Callback_HoverEvent_Void* value)
{
    ...
    auto weakNode = AceType::WeakClaim(frameNode);
    auto onHover = [arkCallback = CallbackHelper(*value), node = weakNode](HoverInfo& hoverInfo) {
        PipelineContext::SetCallBackNode(node);
        const auto event = Converter::ArkHoverEventSync(hoverInfo);
                     ...
                     const Callback_AxisEvent_Void* value)
{
    ...
    CHECK_NULL_VOID(value);
    auto weakNode = AceType::WeakClaim(frameNode);
    auto onAxis = [callback = CallbackHelper(*value), node = weakNode](AxisInfo& info) {
        PipelineContext::SetCallBackNode(node);
                 ...
                 Ark_Boolean value)
{
    auto frameNode = reinterpret_cast<FrameNode *>(node);
    CHECK_NULL_VOID(frameNode);
    auto tabStop = Converter::Convert<bool>(value);
                                  ...
                                  const Ark_String* value)
{
    ...
    CHECK_NULL_VOID(frameNode);
    CHECK_NULL_VOID(value);
    auto nextFocusId = Converter::Convert<std::string>(*value);
    ViewAbstractModelNG::SetAccessibilityNextFocusId(frameNode, nextFocusId);
}
                                        ...
                                        Ark_Boolean value)
{
    auto frameNode = reinterpret_cast<FrameNode *>(node);
    CHECK_NULL_VOID(frameNode);
    auto optScrollTriggerable = Converter::OptConvert<bool>(value);
    LOGE("AccessibilityScrollTriggerableImpl: 'value' should have type Opt_Boolean");
    bool scrollTriggerable = optScrollTriggerable.value_or(false);
    bool resetValue = optScrollTriggerable.has_value() ? false : true;
