...
public:
    ...
    template <typename T>
    static bool CheckCurrentAndAncestorHasFunc(T& ptr, const RefPtr<FrameNode>& node, RefPtr<FrameNode>& parentNode)
    {
        ptr = nullptr;
        CHECK_NULL_RETURN(node, false);
        parentNode = node->GetParentFrameNode();
        if (AccessibilityFunctionUtils::CheckCurrentHasFunc(ptr, node)) {
            return true;
        }
        while (parentNode) {
            if (AccessibilityFunctionUtils::CheckCurrentHasFunc(ptr, parentNode)) {
