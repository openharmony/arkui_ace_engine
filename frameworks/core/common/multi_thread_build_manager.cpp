...
bool MultiThreadBuildManager::AllowNotifyToNode(const RefPtr<NG::UINode>& node)
{
    return node && (!node->IsMultiThreadNode() || node->IsOnMainTree());
}

void MultiThreadBuildManager::SetIsThreadSafeScope(bool isThreadSafeScope)
{
    isThreadSafeScope_ = isThreadSafeScope;
}

bool MultiThreadBuildManager::IsThreadSafeScope()
{
    return isThreadSafeScope_;
}

bool MultiThreadBuildManager::IsThreadSafeScopeOnSubThread()
{
