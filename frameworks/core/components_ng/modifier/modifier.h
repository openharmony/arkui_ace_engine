...
#define ACE_DEFINE_LAYOUT_MODIFIER_TASK(Target, Type)                         \
    do {                                                                      \
        if (!HasValue()) {                                                    \
            return;                                                           \
        }                                                                     \
        auto task = [value = GetValue()](LayoutProperty* layoutProperty) {    \
            auto prop##Target = AceType::DynamicCast<Target>(layoutProperty); \
            if (prop##Target) {                                               \
...
#define ACE_DEFINE_RENDER_MODIFIER_TASK(Target, Type)                         \
    do {                                                                      \
        if (!HasValue()) {                                                    \
            return;                                                           \
        }                                                                     \
        auto task = [value = GetValue()](RenderProperty* renderProperty) {    \
            auto prop##Target = AceType::DynamicCast<Target>(renderProperty); \
            if (prop##Target) {                                               \
                prop##Target->Update##Type(value);                            \
            }                                                                 \
        };                                                                    \
...
#define ACE_DEFINE_RENDER_CONTEXT_TASK(Target, Type)                         \
    do {                                                                     \
        if (!HasValue()) {                                                   \
            return;                                                          \
        }                                                                    \
        auto task = [value = GetValue()](RenderContext* renderContext) {     \
            auto prop##Target = AceType::DynamicCast<Target>(renderContext); \
            if (prop##Target) {                                              \
                prop##Target->Update##Type(value);                           \
            }                                                                \
        };                                                                   \
