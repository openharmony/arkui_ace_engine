...
void RenderSurfaceImpl::ConfigSurface(uint32_t surfaceWidth, uint32_t surfaceHeight)
{
    ...
    double x = (lastRect_.Width() - surfaceWidth) / AVERAGE_VALUE + lastRect_.Left();
    double y = (lastRect_.Height() - surfaceHeight) / AVERAGE_VALUE + lastRect_.Top();
...
bool RenderSurfaceImpl::SetExtSurfaceBoundsSync(int32_t left, int32_t top, int32_t width, int32_t height)
{
    if (extSurface_) {
        ...
        if (isSetConfigSurface_) {
            double x = (width - lastRect_.Width()) / AVERAGE_VALUE + left;
            double y = (height - lastRect_.Height()) / AVERAGE_VALUE + top;
