    ...
    const Offset& offset, TextEncoding encoding)
{
    ...
    auto result = paragrah->GetCharacterIndexByCoordinate(
        offset.GetX(), offset.GetY(), static_cast<RSTextEncoding>(encoding));
    finalResult.position_ = result.index;
    finalResult.affinity_ = static_cast<TextAffinity>(result.affinity);
    return finalResult;
}
    ...
    int32_t start, int32_t end, TextEncoding encoding)
{
    ...
    auto result = paragrah->GetGlyphRangeForCharacterRange(
        ...
        static_cast<RSTextEncoding>(encoding));
...
std::pair<TextRange, TextRange> TxtParagraph::GetCharacterRangeForGlyphRange(
    int32_t start, int32_t end, TextEncoding encoding)
{
    std::pair<TextRange, TextRange> range;
    auto paragrah = GetParagraph();
    CHECK_NULL_RETURN(paragrah, range);
    OHOS::Rosen::Boundary boundary(0, 0);
    auto result = paragrah->GetCharacterRangeForGlyphRange(
        static_cast<size_t>(start), static_cast<size_t>(end), &boundary,
        static_cast<RSTextEncoding>(encoding));
    range.first.start = static_cast<int32_t>(result.leftIndex);
    range.first.end = static_cast<int32_t>(result.rightIndex);
