...
void TextPattern::OnDetachFromFrameNode(FrameNode* node)
{
    ...
    if (selectionChild_) {
        selectionChild_->CloseSelectOverlay(false, CloseReason::CLOSE_REASON_NORMAL);
    } else if (auto selectOverlay = GetSelectOverlay()) {
        selectOverlay->CloseOverlay(false, CloseReason::CLOSE_REASON_NORMAL);
    }
    ClearSelectionChild();
...
void TextPattern::UpdateSelectionChildRegistration()
{
    CHECK_NULL_VOID(directlyCreatedByTextModel_);
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    if (host->GetSelectionContainerId() <= 0) {
        ClearSelectionChild();
        return;
    }
    if (selectionChild_) {
        if (selectionChild_->GetSelectionContainer()) {
            return;
        }
        ClearSelectionChild();
    }
    auto selectionChild = AceType::MakeRefPtr<TextSelectionChild>(WeakClaim(this));
    selectionChild->RegisterChild();
    if (selectionChild->GetSelectionContainer()) {
        selectionChild_ = selectionChild;
    }
}

void TextPattern::ClearSelectionChild()
{
    CHECK_NULL_VOID(directlyCreatedByTextModel_);
    if (!selectionChild_) {
        return;
    }
    selectionChild_->UnregisterChild();
    selectionChild_.Reset();
}
...
{
    if (selectionChild_) {
        selectionChild_->CloseSelectOverlay(animation, CloseReason::CLOSE_REASON_NORMAL);
        return;
    }

    auto host = GetHost();
    FREE_NODE_CHECK(host, CloseSelectOverlay, animation);  // call CloseSelectOverlayMultiThread() by multi thread
    // selectOverlayProxy_ is deprecated, use GetSelectOverlay() instead.
    ...
    if (selectOverlay) {
        selectOverlay->CloseOverlay(animation, CloseReason::CLOSE_REASON_NORMAL);
    }
}
...
void TextPattern::ResetSelection()
{
    ...
    if (selectionChild_) {
        selectionChild_->ResetAllSelection();
        return;
    }
...
void TextPattern::ShowAIEntityMenuForCancel()
{
    ...
    HandleSelectionChange(start, end);
    SetTextResponseType(TextResponseType::LONG_PRESS);
...
void TextPattern::HandleLongPress(GestureEvent& info)
{
    ...
    if (!IsSelectableAndCopy() || isMousePressed_ || IsHandleDragging()) {
        return;
    }
    ...
    RecordSelectedChildAndNotifyChange();
    SetTextResponseType(TextResponseType::LONG_PRESS);
...
void TextPattern::GetSpanItemAttributeUseForHtml(NG::FontStyle& fontStyle, NG::TextLineStyle& textLineStyle,
...
{
    auto subSpanString = GetSelectedSpanString();
    CHECK_NULL_VOID(subSpanString);
#if defined(PREVIEW)
...
RefPtr<SpanString> TextPattern::GetSelectedSpanString()
{
    CHECK_NULL_RETURN(isSpanStringMode_ && !externalParagraph_, nullptr);
    CHECK_NULL_RETURN(styledString_, nullptr);
    auto [start, end] = GetSelectedStartAndEnd();
    CHECK_NULL_RETURN(end > start, nullptr);
    auto subSpanString = styledString_->GetSubSpanString(start, end - start);
    CHECK_NULL_RETURN(subSpanString, nullptr);
    subSpanString->isFromStyledStringMode = true;
    return subSpanString;
}

SelectionCopyPayload TextPattern::GetSelectionCopyPayload()
{
    SelectionCopyPayload payload;
    auto [start, end] = GetSelectedStartAndEnd();
    CHECK_NULL_RETURN(end > start, payload);
    auto selectedText = GetSelectedText(start, end, false, false, true);
    CHECK_NULL_RETURN(!selectedText.empty(), payload);
    payload.plainText = selectedText;

    auto selectedSpanString = GetSelectedSpanString();
    if (selectedSpanString) {
        payload.spanString = selectedSpanString;
        return payload;
    }

    auto spanString = MakeRefPtr<SpanString>(selectedText);
    if (!spans_.empty()) {
        auto spans = GetSpanSelectedContent();
        if (!spans.empty()) {
            spanString->SetSpanItems(std::move(spans));
            payload.spanString = spanString;
            return payload;
        }
    }
    ...
    if (textStyle_.has_value()) {
        ...
        GetSpanItemAttributeUseForHtml(fontStyle, textLineStyle, symbolStyle, textStyle_);
        spanItem->fontStyle = std::make_unique<FontStyle>(fontStyle);
        spanItem->textLineStyle = std::make_unique<TextLineStyle>(textLineStyle);
        spanItem->symbolStyle = std::make_unique<SymbolStyle>(symbolStyle);
        spanItem->content = selectedText;
...
bool TextPattern::IsTextInSelectionContainer() const
{
    auto host = GetHost();
    if (host && host->GetSelectionContainerId() > 0) {
        return true;
    }
    return false;
}
...
std::list<RefPtr<SpanItem>> TextPattern::GetSpanSelectedContent()
{
    ...
    for (const auto& item : spans_) {
        ...
        if (item->position - 1 >= selectStart && item->placeholderIndex == -1 && item->position != -1) {
            ...
            NG::SymbolStyle symbolStyle;
            GetSpanItemAttributeUseForHtml(fontStyle, textLineStyle, symbolStyle, item->GetTextStyle());
            spanItem->fontStyle = std::make_unique<FontStyle>(fontStyle);
            spanItem->textLineStyle = std::make_unique<TextLineStyle>(textLineStyle);
            spanItem->symbolStyle = std::make_unique<SymbolStyle>(symbolStyle);
...
void TextPattern::HideSelectionMenu(bool noAnimation, bool showSubMenu)
{
    if (selectionChild_) {
        selectionChild_->HideMenu(noAnimation, showSubMenu);
        return;
    }
    auto selectOverlay = GetSelectOverlay();
    if (selectOverlay) {
        selectOverlay->HideMenu(noAnimation, showSubMenu);
    }
}

bool TextPattern::SelectOverlayIsOn()
{
    if (selectionChild_) {
        return selectionChild_->SelectOverlayIsOn();
    }
    auto selectOverlay = GetSelectOverlay();
...
{
    if (selectionChild_) {
        return selectionChild_->IsUsingMouse();
    }
    auto selectOverlay = GetSelectOverlay();
    return selectOverlay && selectOverlay->IsUsingMouse();
}
...
void TextPattern::SetTextSelection(int32_t selectionStart, int32_t selectionEnd)
{
    ...
    if (context) {
        context->AddAfterLayoutTask([weak = WeakClaim(this), selectionStart, selectionEnd, eventHub]() {
            ...
            auto copyOption = textPattern->IsTextInSelectionContainer()
                                  ? textPattern->GetCopyOptions()
                                  : textLayoutProperty->GetCopyOptionValue(CopyOptions::None);
...
void TextPattern::ProcessOverlay(const OverlayRequest& request)
{
    SelectAIDetect();
    if (selectionChild_) {
        selectionChild_->ProcessOverlay(request);
        return;
    }

    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->ProcessOverlay(request);
}
...
void TextPattern::InitLongPressEvent(const RefPtr<GestureEventHub>& gestureHub)
{
    ...
    auto longPressCallback = [weak = WeakClaim(this)](GestureEvent& info) {
        auto pattern = weak.Upgrade();
        CHECK_NULL_VOID(pattern);
        pattern->UpdateSourceType(info.GetSourceDevice());
...
void TextPattern::HandleClickEvent(GestureEvent& info)
{
    ...
    if ((IsClickAtHandle(info) && !multipleClickRecognizer_->IsRunning()) || IsHandleDragging()) {
        return;
    }
...
void TextPattern::HandleSingleClickEvent(GestureEvent& info)
{
    ...
    if (SelectOverlayIsOn() && !IsSelectOverlayUsingMouse() &&
        GlobalOffsetInSelectedArea(info.GetGlobalLocation())) {
        if (!IsLocationInFrameRegion(info.GetLocalLocation())) {
            return;
        }
        if (selectionChild_) {
            selectionChild_->SwitchToOverlayMode();
            selectionChild_->ToggleMenu();
        } else if (auto selectOverlay = GetSelectOverlay()) {
            selectOverlay->SwitchToOverlayMode();
            selectOverlay->ToggleMenu();
        }
        return;
    }
...

{
    ...
    dataDetectorAdapter_->onClickMenu_ = [aiSpan, weak = WeakClaim(this), calculateHandleFunc, showSelectOverlayFunc,
        ...
        if (action == COPY) {
            if (pattern->selectionChild_) {
                pattern->selectionChild_->HandleOnCopyFromAI();
            } else {
                pattern->HandleOnCopy();
            }
            pattern->ResetSelection();
        } else if (action == SELECT_TEXT) {
            pattern->RecordSelectedChildAndNotifyChange();
...
RectF TextPattern::CalcAIMenuPosition(const AISpan& aiSpan, const CalculateHandleFunc& calculateHandleFunc)
{
    ...
    RectF viewPort;
    auto selectOverlay = GetOrCreateSelectOverlay();
    if (selectOverlay && selectOverlay->GetClipHandleViewPort(viewPort) &&
...
void TextPattern::SaveOldSelectedType()
{
    oldSelectedType_ = selectedType_.value_or(TextSpanType::NONE);
    if (selectionChild_) {
        selectionChild_->NotifySelectedTypeSave();
    }
}

void TextPattern::SetTextResponseType(TextResponseType type)
{
    textResponseType_ = type;
    if (selectionChild_) {
        selectionChild_->NotifyResponseTypeChanged(type);
    }
}

void TextPattern::RecoverEnableHapticFeedback()
{
    if (!selectionChild_) {
        return;
    }
    RecoverEnableHapticFeedback(CalcEnableHapticFeedback());
}

void TextPattern::RecoverEnableHapticFeedback(bool enableHapticFeedback)
{
    if (hapticFeedbackFlagByUser_) {
        return;
    }
    isEnableHapticFeedback_ = enableHapticFeedback;
}

bool TextPattern::CalcEnableHapticFeedback()
{
    return selectionChild_->GetContainerEnableHapticFeedback().value_or(true);
}

void TextPattern::OnContainerEnableHapticFeedbackUpdate()
{
    if (hapticFeedbackFlagByUser_ || !selectionChild_) {
        return;
    }
    bool newValue = CalcEnableHapticFeedback();
    if (newValue == isEnableHapticFeedback_) {
        return;
    }
    RecoverEnableHapticFeedback(newValue);
}

void TextPattern::RecoverSelectedBackgroundColor()
{
    if (!selectionChild_) {
        return;
    }
    selectedBackgroundColor_ = CalcSelectedBackgroundColor();
}

Color TextPattern::CalcSelectedBackgroundColor()
{
    auto textLayoutProperty = GetLayoutProperty<TextLayoutProperty>();
    CHECK_NULL_RETURN(textLayoutProperty, Color::TRANSPARENT);
    if (textLayoutProperty->HasSelectedBackgroundColorFlagByUser() &&
        textLayoutProperty->GetSelectedBackgroundColorFlagByUserValue(false)) {
        return textLayoutProperty->GetSelectedBackgroundColor().value_or(Color::TRANSPARENT);
    }
    if (selectionChild_) {
        auto containerColor = selectionChild_->GetContainerSelectedBackgroundColor();
        if (containerColor.has_value()) {
    ...
    return theme->GetSelectedColor();
}

void TextPattern::OnContainerSelectedBackgroundColorUpdate()
{
    if (!selectionChild_) {
        return;
    }
    Color newColor = CalcSelectedBackgroundColor();
    if (newColor == selectedBackgroundColor_) {
        return;
    }
    selectedBackgroundColor_ = newColor;
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    host->MarkDirtyNode(PROPERTY_UPDATE_MEASURE_SELF);
}

CopyOptions TextPattern::CalcCopyOption()
{
    auto textLayoutProperty = GetLayoutProperty<TextLayoutProperty>();
    CHECK_NULL_RETURN(textLayoutProperty, CopyOptions::None);
    auto contentHost = GetContentHost();
    CHECK_NULL_RETURN(contentHost, CopyOptions::None);

    CopyOptions result = CopyOptions::None;
    if (textLayoutProperty->GetTextOverflowValue(TextOverflow::CLIP) == TextOverflow::MARQUEE) {
        result = CopyOptions::None;
    } else if (!selectionChild_ ||
        (textLayoutProperty->HasCopyOptionFlagByUser() && textLayoutProperty->GetCopyOptionFlagByUserValue(false))) {
        result = textLayoutProperty->GetCopyOption().value_or(CopyOptions::None);
    } else {
        result = selectionChild_->GetContainerCopyOption().value_or(CopyOptions::None);
    }
...
void TextPattern::OnContainerCopyOptionUpdate()
{
    CopyOptions newCopyOption = CalcCopyOption();
    if (newCopyOption == copyOption_) {
        return;
    }

    RecoverCopyOption(newCopyOption);
}

void TextPattern::RecoverCopyOption()
{
    RecoverCopyOption(CalcCopyOption());
}

void TextPattern::RecoverCopyOption(CopyOptions copyOption)
{
    auto textLayoutProperty = GetLayoutProperty<TextLayoutProperty>();
    CHECK_NULL_VOID(textLayoutProperty);
    auto host = GetHost();
    ...
    if (copyOption_ == CopyOptions::None && !textDetectEnable_ && !textLayoutProperty->GetTextOverflow() &&
        ...
        if (gestureEventHub && (host->IsDraggable() || gestureEventHub->GetTextDraggable())) {
            ClearTextDefaultDrag(gestureEventHub, eventHub);
        }
        return;
    }
    ...
    InitCopyOptionAndOverlay();
}

void TextPattern::InitCopyOptionAndOverlay()
{
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    ...
    // GetSelectOverlay() returns nullptr when IsTextInSelectionContainer is active
    if (auto selectOverlay = GetSelectOverlay()) {
        selectOverlay->SetMenuTranslateIsSupport(IsShowTranslate());
        selectOverlay->SetIsSupportMenuSearch(IsShowSearch());
        selectOverlay->UpdateHandleColor();
    }
...
void TextPattern::InitDefaultTextDraggable()
{
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    if (!ShouldUseDefaultTextDrag() || host->IsUserSet()) {
        return;
    }
    auto context = host->GetContext();
    CHECK_NULL_VOID(context);
    host->SetDraggable(context->GetDraggable<TextTheme>());
}
...
void TextPattern::ClearTextDefaultDrag(const RefPtr<GestureEventHub>& gestureEventHub, const RefPtr<EventHub>& eventHub)
{
    CHECK_NULL_VOID(gestureEventHub);
    CHECK_NULL_VOID(eventHub);
...
void TextPattern::HandleMouseEvent(const MouseInfo& info)
{
    ...
    if (info.GetButton() == MouseButton::LEFT_BUTTON) {
        ...
        UpdateSourceType(info.GetSourceDevice());
...
void TextPattern::HandleMouseLeftPressAction(const MouseInfo& info, const Offset& textOffset)
{
    ...
    if (BetweenContainerSelectedPosition(info.GetGlobalLocation())) {
        blockPress_ = true;
        return;
    }
    ...
    if (selectionChild_) {
        HandleMouseLeftPressForContainer(textOffset);
    } else {
...
void TextPattern::UpdateSourceType(SourceType sourceType)
{
    sourceType_ = sourceType;
    if (selectionChild_) {
        selectionChild_->NotifySourceTypeChanged(sourceType_);
    }
}

void TextPattern::SetMouseMenuOffset(const OffsetF& mouseMenuOffset)
{
    if (selectionChild_) {
        selectionChild_->SetMouseMenuOffset(mouseMenuOffset);
    } else {
        auto selectOverlay = GetOrCreateSelectOverlay();
        CHECK_NULL_VOID(selectOverlay);
...
bool TextPattern::IsTriggerParentToScroll()
{
    ...
    auto selectOverlay = GetSelectOverlay();
    return selectOverlay && selectOverlay->IsTriggerParentToScroll();
}
...
{
    if (selectionChild_) {
        selectionChild_->SetSelectionHoldCallback();
        return;
    }
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->SetSelectionHoldCallback();
}

void TextPattern::UpdateSelectedTypeForRightClick(const MouseInfo& info)
{
    ...
    if (selectionChild_) {
        ...
        summary.type = selectedType_.value_or(TextSpanType::NONE);
        selectionChild_->NotifyContainerSelectionSpanTypeChanged(summary);
    }
}

bool TextPattern::HandleMouseLeftPressForContainer(const Offset& textOffset)
{
    ...
    const MouseInfo& info, const Offset& textOffset, MouseStatus oldMouseStatus, int32_t start, int32_t end)
{
    ...
    if (IsSelected() && oldMouseStatus == MouseStatus::MOVE && IsSelectedBindSelectionMenu()) {
        SetMouseMenuOffset(OffsetF(
            static_cast<float>(info.GetGlobalLocation().GetX()),
...
{
    // Mouse release can be dispatched to both the press target and the current hit target.
    // In selection-container scenarios, only the fixed-anchor child that started the session
    // should finalize the cross-node selection on release.
    if (selectionChild_ && !selectionChild_->IsSelectionSessionOwner()) {
        return;
    }
    ...
    ResetMouseReleaseState(info);
}
 
void TextPattern::ResetMouseReleaseState(const MouseInfo& info)
{
    ResetMouseLeftPressedState();
    moveOverClickThreshold_ = false;
...
void TextPattern::HandleMouseRightButton(const MouseInfo& info, const Offset& textOffset)
{
    if (info.GetAction() == MouseAction::RELEASE) {
        auto rightClickOffset = OffsetF(
            static_cast<float>(info.GetGlobalLocation().GetX()), static_cast<float>(info.GetGlobalLocation().GetY()));
        SetMouseMenuOffset(rightClickOffset);
        if (!BetweenContainerSelectedPosition(info.GetGlobalLocation())) {
            HandleClickAISpanEvent(PointF(textOffset.GetX(), textOffset.GetY()));
            if (GetDataDetectorAdapter()->hasClickedAISpan_) {
                isMousePressed_ = false;
        ...
        SetTextResponseType(TextResponseType::RIGHT_CLICK);
        if (!IsSelected()) {
            UpdateSelectedTypeForRightClick(info);
        }
        ShowSelectOverlay({ .animation = true });
        isMousePressed_ = false;
        auto host = GetHost();
...
bool TextPattern::HandleKeyEvent(const KeyEvent& keyEvent)
{
    ...
    if (keyEvent.IsCtrlWith(KeyCode::KEY_C)) {
        if (selectionChild_ && selectionChild_->HandleOnCopy()) {
            return true;
        }
        HandleOnCopy();
        return true;
    ...
    if (keyEvent.IsCtrlWith(KeyCode::KEY_A)) {
        if (selectionChild_ && selectionChild_->HandleOnSelectAll()) {
            return true;
        }
        auto textSize = static_cast<int32_t>(textForDisplay_.length()) + placeholderCount_;
        HandleSelectionChange(0, textSize);
...
{
    CHECK_NULL_VOID(!selectionChild_);
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    auto eventHub = host->GetEventHub<EventHub>();
    CHECK_NULL_VOID(eventHub);
...
bool TextPattern::ShouldUseDefaultTextDrag() const
{
    auto host = GetHost();
    CHECK_NULL_RETURN(host, false);
    return !host->IsCustomerSet();
}
...
std::function<void(Offset)> TextPattern::GetThumbnailCallback()
{
    return [wk = WeakClaim(this)](const Offset& point) {
        ...
        if (pattern->BetweenContainerSelectedPosition(point) || pattern->IsAiSelected()) {
            auto host = pattern->GetHost();
            CHECK_NULL_VOID(host);
...
TextDragInfo TextPattern::CreateTextDragInfo()
{
    ...
    auto selectOverlay = GetOrCreateSelectOverlay();
    if (selectOverlay) {
        selectOverlay->GetVisibleDragViewHandles(info.firstHandle, info.secondHandle);
    }
    if (IsAiSelected()) {
...
OffsetF TextPattern::GetParentGlobalOffset() const
{
    if (selectionChild_) {
        ...
        selectOverlay->UpdateHandleGlobalOffset();
    }
    auto host = GetHost();
...
bool TextPattern::BetweenContainerSelectedPosition(const Offset& globalOffset)
{
    if (selectionChild_) {
        return selectionChild_->BetweenContainerSelectedPosition(globalOffset);
    }
    return BetweenSelectedPosition(globalOffset);
}
...
void TextPattern::OnModifyDone()
{
    ...
    RecoverCopyOption();
    RecoverEnableHapticFeedback();
    RecoverSelectedBackgroundColor();
...
void TextPattern::ProcessOverlayAfterLayout()
{
    if (SelectOverlayIsOn()) {
        CalculateHandleOffsetAndShowOverlay();
        if (IsTextInSelectionContainer()) {
            // Cross-node: defer to frame end to avoid N updates per frame
            isContainerOverlayDirtyAfterLayout_ = true;
            auto host = GetHost();
            CHECK_NULL_VOID(host);
            auto context = host->GetContext();
            CHECK_NULL_VOID(context);
            context->AddAfterLayoutTask([weakPattern = WeakClaim(this)]() {
                ...
                CHECK_NULL_VOID(pattern);
                pattern->FlushContainerOverlayAfterLayout();
            });
        } else if (auto selectOverlay = GetSelectOverlay()) {
            selectOverlay->UpdateAllHandlesOffset();
            selectOverlay->UpdateViewPort();
        }
    }
}

void TextPattern::FlushContainerOverlayAfterLayout()
{
    if (!isContainerOverlayDirtyAfterLayout_) {
        return;
    }
    isContainerOverlayDirtyAfterLayout_ = false;
    if (selectionChild_) {
        selectionChild_->UpdateAllHandlesOffset();
        selectionChild_->UpdateViewPort();
    }
}
...
void TextPattern::InitSpanItemEvent(bool& isSpanHasClick, bool& isSpanHasLongPress)
{
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    CHECK_NULL_VOID(isSpanHasClick || isSpanHasLongPress);
    auto gestureEventHub = host->GetOrCreateGestureEventHub();
    if (isSpanHasClick) {
        InitClickEvent(gestureEventHub);
    }
...
void TextPattern::BeforeCreateLayoutWrapper()
{
    ...
    if (selectionChild_) {
        selectionChild_->MarkOverlayDirty();
    } else if (auto selectOverlay = GetSelectOverlay()) {
        selectOverlay->MarkOverlayDirty();
    }
...
void TextPattern::SetAccessibilityAction()
{
    ...
    textAccessibilityProperty->SetActionSetSelection(
        [weakPtr = WeakClaim(this)](int32_t start, int32_t end, bool isForward) {
            ...
            CHECK_NULL_VOID(textLayoutProperty);
            auto mode = textLayoutProperty->GetTextSelectableModeValue(TextSelectableMode::SELECTABLE_UNFOCUSABLE);
            auto copyOption = pattern->IsTextInSelectionContainer()
                                  ? pattern->GetCopyOptions()
                                  : textLayoutProperty->GetCopyOptionValue(CopyOptions::None);
            if (copyOption != CopyOptions::None && mode != TextSelectableMode::UNSELECTABLE) {
    ...
    textAccessibilityProperty->SetActionCopy([weakPtr = WeakClaim(this)]() {
        ...
        if (copyOption != CopyOptions::None && mode != TextSelectableMode::UNSELECTABLE) {
            if (pattern->selectionChild_) {
                pattern->selectionChild_->HandleOnCopy();
            } else {
                pattern->HandleOnCopy();
            }
            pattern->CloseSelectOverlay(true);
...
bool TextPattern::OnThemeScopeUpdate(int32_t themeScopeId)
{
    ...
    if (host->GreatOrEqualAPITargetVersion(PlatformVersion::VERSION_TWENTY_SIX)) {
        ...
        // GetSelectOverlay() returns nullptr when IsTextInSelectionContainer is active
        auto selectOverlay = GetSelectOverlay();
        if (SelectOverlayIsOn() && selectOverlay) {
            selectOverlay->UpdateMenuFromThemeChange(themeScopeId);
        }
    }
    return false;
}
...
void TextPattern::SelectAIDetect()
{
    ...
    auto updateTask = [weak = WeakClaim(this)]() -> void {
        ...
        textPattern->UpdateAISelectMenu();
    };
...
RefPtr<TextSelectOverlay> TextPattern::GetOrCreateSelectOverlay()
{
    if (IsTextInSelectionContainer()) {
        return nullptr;
    }
...
RefPtr<TextSelectOverlay> TextPattern::GetSelectOverlay() const
{
    if (IsTextInSelectionContainer()) {
        return nullptr;
    }
...
bool TextPattern::HasOrUpdateRenderTransform(bool needCreate)
{
    if (selectionChild_) {
        return selectionChild_->HasOrUpdateRenderTransform();
    }
    RefPtr<TextSelectOverlay> selectOverlay;
    if (needCreate) {
        selectOverlay = GetOrCreateSelectOverlay();
    } else {
        selectOverlay = GetSelectOverlay();
    }
    return selectOverlay && selectOverlay->HasRenderTransform();
}

void TextPattern::DisableSelectMenu()
{
    if (selectionChild_) {
        selectionChild_->DisableMenu();
        return;
    }
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->DisableMenu();
}

void TextPattern::UpdateAISelectMenu()
{
    if (selectionChild_) {
        selectionChild_->UpdateAISelectMenu();
        return;
    }
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
...
bool TextPattern::IsCurrentMenuVisibile()
{
    if (selectionChild_) {
        return selectionChild_->IsCurrentMenuVisibile();
    }
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_RETURN(selectOverlay, false);
    return selectOverlay->IsCurrentMenuVisibile();
}

bool TextPattern::IsHandleDragging()
{
    if (selectionChild_) {
        return selectionChild_->GetIsHandleDragging();
    }
    auto selectOverlay = GetSelectOverlay();
    return selectOverlay && selectOverlay->GetIsHandleDragging();
}

bool TextPattern::IsClickAtHandle(const GestureEvent& info)
{
    if (selectionChild_) {
        return selectionChild_->IsClickAtHandle(info);
    }
    auto selectOverlay = GetSelectOverlay();
    return selectOverlay && selectOverlay->IsClickAtHandle(info);
}

bool TextPattern::IsTouchAtHandle(const TouchEventInfo& info)
{
    if (selectionChild_) {
        return selectionChild_->IsTouchAtHandle(info);
    }
    auto selectOverlay = GetSelectOverlay();
    return selectOverlay && selectOverlay->IsTouchAtHandle(info);
}

bool TextPattern::IsShowMouseMenu()
{
    if (selectionChild_) {
        return selectionChild_->IsShowMouseMenu();
    }
    auto selectOverlay = GetSelectOverlay();
    return selectOverlay && selectOverlay->IsShowMouseMenu();
}
    ...
    const NG::OnMenuItemClickCallback&& onMenuItemClick, const NG::OnPrepareMenuCallback&& onPrepareMenuCallback)
{
    ...
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->OnSelectionMenuOptionsUpdate(
...
void TextPattern::HandleSelectionChange(int32_t start, int32_t end)
{
    ...
    if (selectOverlay) {
        if (textSelector_.IsValid()) {
            selectOverlay->SetTextSelectionHolderId(host->GetId());
        } else {
            selectOverlay->RemoveTextSelectionHolderId(host->GetId());
        }
    }
...
{
    if (selectionChild_) {
        return selectionChild_->IsSelectedBindSelectionMenu();
    }
...
void TextPattern::UpdateSelectionSpanType(int32_t selectStart, int32_t selectEnd)
{
    ...
    if (selectionChild_) {
        ChildSelectionSummary summary;
        summary.contributes = !GetSelectedText(selectStart, selectEnd, false, false, true).empty();
        summary.type = selectedType_.value_or(TextSpanType::NONE);
        selectionChild_->NotifyContainerSelectionSpanTypeChanged(summary);
    }
}
...
void TextPattern::SetSelectionFlag(int32_t selectionStart, int32_t selectionEnd, const SelectionOptions options)
{
    ...
    CHECK_NULL_VOID(textLayoutProperty);
    auto mode = textLayoutProperty->GetTextSelectableModeValue(TextSelectableMode::SELECTABLE_UNFOCUSABLE);
    auto copyOption = IsTextInSelectionContainer()
                          ? GetCopyOptions()
                          : textLayoutProperty->GetCopyOptionValue(CopyOptions::None);
    if (mode == TextSelectableMode::UNSELECTABLE ||
        copyOption == CopyOptions::None ||
...
void TextPattern::ActSetSelectionFlag(int32_t selectionStart, int32_t selectionEnd, const SelectionOptions options)
{
    ...
    if (!IsShowHandle()) {
        ...
        bool isShowMenu = IsShowMenu(options.menuPolicy, IsCurrentMenuVisibile());
...
void TextPattern::RecordSelectedChildAndNotifyChange()
{
    CHECK_NULL_VOID(selectionChild_);
    auto selectedText = GetSelectedText(textSelector_.GetTextStart(), textSelector_.GetTextEnd(), false, false, true);
    CHECK_NULL_VOID(!selectedText.empty());
    selectionChild_->RecordSelectedChild();
    selectionChild_->NotifySelectionChanged(selectedText);
}
...
Offset TextPattern::ConvertGlobalToLocalOffset(const Offset& globalOffset)
{
    ...
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_RETURN(selectOverlay, Offset());
    selectOverlay->RevertLocalPointWithTransform(localPoint);
    return Offset(localPoint.GetX(), localPoint.GetY());
}
...
void TextPattern::StartGestureSelection(int32_t start, int32_t end, const Offset& startOffset)
{
    if (selectionChild_) {
        ...
        selectionChild_->HandleSelectionStart(startOffset, start, end);
        return;
    }
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
...
void TextPattern::OnTextGestureSelectionUpdate(int32_t start, int32_t end, const TouchEventInfo& info)
{
    ...
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->TriggerScrollableParentToScroll(
...
void TextPattern::OnTextGestureSelectionEnd(const TouchLocationInfo& locationInfo)
{
    ...
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->TriggerScrollableParentToScroll(scrollableParent_.Upgrade(), Offset(), true);
    if (magnifierController_) {
...
void TextPattern::DumpInfo(std::unique_ptr<JsonValue>& json)
{
    ...
    if (pManager_ && !pManager_->GetParagraphs().empty()) {
        ...
        if (contentMod_) {
            auto paintInfo = "ContentOffset:" + contentMod_->GetPaintOffset().ToString();
            json->Put("PaintInfo", paintInfo.c_str());
        }
    }
    if (SystemProperties::GetDebugEnabled()) {
...
{
    if (IsTextInSelectionContainer()) {
        return selectedBackgroundColor_.ColorToString();
    }
    auto context = PipelineContext::GetCurrentContextSafelyWithCheck();
    CHECK_NULL_RETURN(context, "");
    auto theme = context->GetTheme<TextTheme>();
...
std::optional<Color> TextPattern::GetSelectedBgColorWhenAcrossText() const
{
    if (!selectionChild_) {
        return std::nullopt;
    }
...
{
    if (selectionChild_) {
        selectionChild_->UpdateMenuOnWindowSizeChanged(type);
        return;
    }
    auto selectOverlay = GetSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
    selectOverlay->UpdateMenuOnWindowSizeChanged(type);
}
...
RefPtr<LayoutProperty> TextPattern::CreateLayoutProperty()
{
...
bool TextPattern::IsDragging() const
{
    return status_ == Status::DRAGGING;
}

bool TextPattern::IsAtomicNode() const
{
    auto host = GetHost();
    CHECK_NULL_RETURN(host, false);
...
bool TextPattern::IsTextNode() const
{
    auto host = GetHost();
    CHECK_NULL_RETURN(host, false);
...
bool TextPattern::DefaultSupportDrag()
{
    return true;
}

FocusPattern TextPattern::GetFocusPattern() const
{
    return { FocusType::NODE, false };
}

TextSelector TextPattern::GetTextSelector() const
{
    return textSelector_;
}

const std::u16string& TextPattern::GetTextForDisplay() const
{
    return textForDisplay_;
}

const OffsetF& TextPattern::GetStartOffset() const
{
    return textSelector_.selectionBaseOffset;
}

const OffsetF& TextPattern::GetEndOffset() const
{
    return textSelector_.selectionDestinationOffset;
}

double TextPattern::GetSelectHeight() const
{
    return textSelector_.GetSelectHeight();
}

float TextPattern::GetBaselineOffset() const
{
    return baselineOffset_;
}

RefPtr<TextContentModifier> TextPattern::GetContentModifier()
{
    return contentMod_;
}
...
bool TextPattern::GetTextDetectEnable()
{
...
std::string TextPattern::GetTextDetectTypes()
{
    return textDetectTypes_;
}

RefPtr<DataDetectorAdapter> TextPattern::GetDataDetectorAdapter()
{
    if (!dataDetectorAdapter_) {
        auto host = GetHost();
        if (host) {
...
RefPtr<DataDetectorAdapter> TextPattern::GetSelectDetectorAdapter()
{
...
const std::map<int32_t, AISpan>& TextPattern::GetAISpanMap()
{
    return GetDataDetectorAdapter()->aiSpanMap_;
}

const std::u16string& TextPattern::GetTextForAI()
{
    return GetDataDetectorAdapter()->textForAI_;
}

void TextPattern::SetOnResult(std::function<void(const std::string&)>&& onResult)
{
    GetDataDetectorAdapter()->onResult_ = std::move(onResult);
}

TextDataDetectResult TextPattern::GetTextDetectResult()
{
    return GetDataDetectorAdapter()->textDetectResult_;
}

void TextPattern::MarkAISpanStyleChanged()
{
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    host->MarkDirtyWithOnProChange(PROPERTY_UPDATE_MEASURE);
...
std::list<RefPtr<SpanItem>> TextPattern::GetSpanItemChildren()
{
    return spans_;
}

int32_t TextPattern::GetPlaceholderCount()
{
    return placeholderCount_;
}

int32_t TextPattern::GetDisplayWideTextLength()
{
...
// ===========================================================
// TextDragBase implementations
bool TextPattern::IsTextArea() const
{
    return false;
}

const RectF& TextPattern::GetTextRect() const
{
    return contentRect_;
}

const RefPtr<FrameNode>& TextPattern::MoveDragNode()
{
    return dragNode_;
}

const RefPtr<Paragraph>& TextPattern::GetDragParagraph() const
{
    return pManager_->GetParagraphs().front().paragraph;
}

bool TextPattern::CloseKeyboard(bool /* forceClose */)
{
    return true;
}

bool TextPattern::HasSurfaceChangedCallback()
{
    return surfaceChangedCallbackId_.has_value();
}

void TextPattern::UpdateSurfaceChangedCallbackId(int32_t id)
{
    surfaceChangedCallbackId_ = id;
}

bool TextPattern::HasSurfacePositionChangedCallback()
{
    return surfacePositionChangedCallbackId_.has_value();
}

void TextPattern::UpdateSurfacePositionChangedCallbackId(int32_t id)
{
    surfacePositionChangedCallbackId_ = id;
}

void TextPattern::SetOnClickEvent(GestureEventFunc&& onClick, double distanceThreshold)
{
    onClick_ = std::move(onClick);
    distanceThreshold_ = distanceThreshold;
}

const std::vector<std::u16string>& TextPattern::GetDragContents() const
{
    return dragContents_;
}

void TextPattern::InitSpanImageLayout(const std::vector<int32_t>& placeholderIndex,
...
const std::vector<int32_t>& TextPattern::GetPlaceHolderIndex()
{
    return placeholderIndex_;
}

const std::vector<RectF>& TextPattern::GetRectsForPlaceholders()
{
    return rectsForPlaceholders_;
}

OffsetF TextPattern::GetContentOffset()
{
    return imageOffset_;
}

bool TextPattern::IsMeasureBoundary() const
{
    return isMeasureBoundary_;
}

void TextPattern::SetIsMeasureBoundary(bool isMeasureBoundary)
{
    isMeasureBoundary_ = isMeasureBoundary;
}

void TextPattern::SetIsCustomFont(bool isCustomFont)
{
    isCustomFont_ = isCustomFont;
}

bool TextPattern::GetIsCustomFont()
{
    return isCustomFont_;
}

void TextPattern::SetImageSpanNodeList(std::vector<WeakPtr<FrameNode>> imageNodeList)
{
    imageNodeList_ = imageNodeList;
}

std::vector<WeakPtr<FrameNode>> TextPattern::GetImageSpanNodeList()
{
    return imageNodeList_;
}

std::vector<ParagraphManager::ParagraphInfo> TextPattern::GetParagraphs() const
{
...
void TextPattern::ResetContChange()
{
    contChange_ = false;
}

bool TextPattern::GetContChange() const
{
    return contChange_;
}

bool TextPattern::GetShowSelect() const
{
    return showSelect_;
}

int32_t TextPattern::GetRecoverStart() const
{
    return recoverStart_;
}

int32_t TextPattern::GetRecoverEnd() const
{
    return recoverEnd_;
}

void TextPattern::ResetDragOption()
{
    CloseSelectOverlay();
    ResetSelection();
}

int32_t TextPattern::GetDragRecordSize()
{
    return dragRecordSize_;
}

void TextPattern::ResetDragRecordSize(int32_t size)
{
    dragRecordSize_ = size;
}

void TextPattern::SetTextController(const RefPtr<TextController>& controller)
{
    textController_ = controller;
}

const RefPtr<TextController>& TextPattern::GetTextController()
{
    return textController_;
}

void TextPattern::ClearSelectionMenu()
{
    selectionMenuMap_.clear();
}

CopyOptions TextPattern::GetCopyOptions() const
{
    return copyOption_;
}

// style string
void TextPattern::SetSpanItemChildren(const std::list<RefPtr<SpanItem>>& spans)
{
    spans_ = spans;
}

void TextPattern::SetSpanStringMode(bool isSpanStringMode)
{
    isSpanStringMode_ = isSpanStringMode;
}

bool TextPattern::GetSpanStringMode() const
{
    return isSpanStringMode_;
}

void TextPattern::AllocStyledString()
{
    if (!styledString_) {
        auto host = GetHost();
        if (host) {
...
void TextPattern::SetIsAskCeliaEnabled(bool isAskCeliaEnabled)
{
    isAskCeliaEnabled_ = isAskCeliaEnabled && IsNeedAskCelia();
}

bool TextPattern::IsAskCeliaEnabled() const
{
    return isAskCeliaEnabled_;
}

void TextPattern::SetIsShowAskCeliaInRightClick(bool isShowAskCeliaInRightClick)
{
    isShowAskCeliaInRightClick_ = isShowAskCeliaInRightClick && IsNeedAskCelia();
}

bool TextPattern::IsShowAskCeliaInRightClick() const
{
...
bool TextPattern::IsSelectedTypeChange()
{
...
bool TextPattern::IsUsingMouse()
{
    return sourceType_ == SourceType::MOUSE;
}

void TextPattern::CopySelectionMenuParams(SelectOverlayInfo& selectInfo)
{
    CopySelectionMenuParams(selectInfo, textResponseType_.value_or(TextResponseType::NONE));
}

void TextPattern::InitCustomSpanPlaceholderInfo(const std::vector<CustomSpanPlaceholderInfo>& customSpanPlaceholder)
{
    customSpanPlaceholder_ = customSpanPlaceholder;
}

std::vector<CustomSpanPlaceholderInfo> TextPattern::GetCustomSpanPlaceholderInfo()
{
    return customSpanPlaceholder_;
}

TextSelectionOptions TextPattern::GetTextSelectionOptions()
{
    return textSelectionOptions_;
}

void TextPattern::ClearCustomSpanPlaceholderInfo()
{
    customSpanPlaceholder_.clear();
}

const std::list<RefPtr<UINode>>& TextPattern::GetChildNodes() const
{
    return childNodes_;
}

// add for capi NODE_TEXT_CONTENT_WITH_STYLED_STRING
void TextPattern::SetExternalParagraph(void* paragraph)
{
    ACE_TEXT_SCOPED_TRACE("SetExternalParagraph");
    externalParagraph_ = paragraph;
}

const std::optional<void*>& TextPattern::GetExternalParagraph()
{
    return externalParagraph_;
}

void TextPattern::SetExternalParagraphStyle(const std::optional<ParagraphStyle>& paragraphStyle)
{
...
TextStyle TextPattern::GetTextStyle()
{
    return textStyle_.value_or(TextStyle());
}

bool TextPattern::IsOnlyFontSizeOrColorChanged()
{
    return textStyle_.has_value() ? textStyle_->CheckIsFontSizeOrColorChanged() : false;
}

std::optional<ParagraphStyle> TextPattern::GetExternalParagraphStyle() const
{
...
void TextPattern::OnPrepareMenuCallbackUpdate(const NG::OnPrepareMenuCallback&& onPrepareMenuCallback)
{
    auto selectOverlay = GetOrCreateSelectOverlay();
    CHECK_NULL_VOID(selectOverlay);
...
void TextPattern::UpdateParentGlobalOffset()
{
    parentGlobalOffset_ = GetParentGlobalOffset();
}
...
void TextPattern::SetIsUserSetResponseRegion(bool isUserSetResponseRegion)
{
    isUserSetResponseRegion_ = isUserSetResponseRegion;
}

uint64_t TextPattern::GetSystemTimestamp()
{
...
void TextPattern::SetEnableHapticFeedback(bool isEnabled, bool flagByUser)
{
    isEnableHapticFeedback_ = isEnabled;
    hapticFeedbackFlagByUser_ = flagByUser;
}

bool TextPattern::IsEnabledObscured() const
{
    return true;
}

void TextPattern::RegisterAfterLayoutCallback(std::function<void()> callback)
{
    afterLayoutCallback_ = callback;
}

void TextPattern::UnRegisterAfterLayoutCallback()
{
...
void TextPattern::EmplaceSymbolColorIndex(int32_t index)
{
    symbolFontColorResObjIndexArr.emplace_back(index);
}

void TextPattern::SetIsShowAIMenuOption(bool isShowAIMenuOption)
{
    isShowAIMenuOption_ = isShowAIMenuOption;
}

bool TextPattern::IsShowAIMenuOption() const
{
    return isShowAIMenuOption_;
}

void TextPattern::SetAIItemOption(const std::unordered_map<TextDataDetectType, AISpan>& aiMenuOptions)
{
    aiMenuOptions_ = aiMenuOptions;
}

const std::unordered_map<TextDataDetectType, AISpan>& TextPattern::GetAIItemOption() const
{
    return aiMenuOptions_;
}

RefPtr<TextEffect> TextPattern::GetTextEffect()
{
    return textEffect_;
}

bool TextPattern::IsNeedAskCelia() const
{
...
void TextPattern::UpdateTextSelectorSecondHandle(const RectF& rect)
{
    textSelector_.secondHandle = rect;
}

bool TextPattern::IsEnableMatchParent()
{
    return true;
}

void TextPattern::SetExternalDrawCallback(ExternalDrawCallback&& callback)
{
...
const ExternalDrawCallback& TextPattern::GetExternalDrawCallback()
{
    return externalDrawCallback_;
}

bool TextPattern::AllowVisibleAreaCheck() const
{
    auto textLayoutProperty = GetLayoutProperty<TextLayoutProperty>();
    CHECK_NULL_RETURN(textLayoutProperty, false);
    return textLayoutProperty->GetTextOverflowValue(TextOverflow::CLIP) == TextOverflow::MARQUEE;
...
void TextPattern::MarkMeasured(bool isMeasured)
{
    isMeasured_ = isMeasured;
}

bool TextPattern::IsMeasured() const
{
    return isMeasured_;
}

int32_t TextPattern::GetClickedSpanPosition()
{
    return clickedSpanPosition_;
}

void TextPattern::CreateMultipleClickRecognizer()
{
...
void TextPattern::SetCurrentDragTool(SourceTool tool)
{
    lastDragTool_ = tool;
}

std::optional<RenderContext::ContextParam> TextPattern::GetContextParam() const
{
    return RenderContext::ContextParam { .type = RenderContext::ContextType::CANVAS,
                                         .surfaceName = std::nullopt };
}

SourceTool TextPattern::GetCurrentDragTool() const
{
