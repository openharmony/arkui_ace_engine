...
#include "frameworks/core/components_ng/pattern/text/span/symbol_span_model_ng.h"
...
FrameNode* GetFrameNode(ArkUINodeHandle node)
{
    if (node) {
        ...
        auto nativeNode = reinterpret_cast<ArkUINodeHandle>(uiNode);
        return reinterpret_cast<FrameNode*>(nativeNode);
    }
}

SymbolSpanModel* GetInstance()
{
...
void CreateSymbolSpanModel(ArkUI_Uint32 symbolId)
{
    GetInstance()->Create(symbolId);
}
...
void SetSymbolSpanFontFamilies(ArkUI_CharPtr* fontFamilies, ArkUI_Uint32 length)
{
    ...
    if (!fontFamilies) {
        GetInstance()->SetFontFamilies(familyNames);
        return;
    }
    for (ArkUI_Uint32 index = 0; index < length; ++index) {
        if (fontFamilies[index]) {
            familyNames.emplace_back(fontFamilies[index]);
        }
    }
    GetInstance()->SetFontFamilies(familyNames);
}

void SetSymbolSpanType(ArkUI_Uint32 type)
{
    GetInstance()->SetSymbolType(static_cast<SymbolType>(type));
}
...
const ArkUISymbolSpanModifier* GetSymbolSpanDynamicModifier()
{
    ...
    static const ArkUISymbolSpanModifier modifier = {
        ...
        .createModel = CreateSymbolSpanModel,
        .setSymbolSpanFontFamilies = SetSymbolSpanFontFamilies,
        .setSymbolSpanType = SetSymbolSpanType,
    };
