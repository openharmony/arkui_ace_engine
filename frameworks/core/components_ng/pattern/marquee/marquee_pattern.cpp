...
void MarqueePattern::StartMarqueeAnimation()
{
    ...
    if (repeatCount != 1) {
        needSecondPlay = LessNotEqual(textGeoNode->GetMarginFrameOffset().GetX(), padding.left.value_or(0));
        if (direction == MarqueeDirection::RIGHT) {
            needSecondPlay = GreatNotEqual(
                textGeoNode->GetMarginFrameOffset().GetX(), marqueeSize.Width() - textGeoNode->GetFrameSize().Width() - padding.right.value_or(0));
        }
    }
