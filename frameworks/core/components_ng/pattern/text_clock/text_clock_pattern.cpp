...
void TextClockPattern::ParseInputFormat()
{
    ...
    if (Container::GreatOrEqualAPITargetVersion(PlatformVersion::VERSION_TWENTY) && is12h) {
        is24H_ = false;
    }
}
...
std::string TextClockPattern::GetFormat() const
{
    ...
    if (isForm_) {
        ...
        if (Container::GreatOrEqualAPITargetVersion(PlatformVersion::VERSION_TWENTY) && is24H_) {
