...
#include "frameworks/compatible/components/image-animator/image_animator_model_impl.h"

namespace OHOS::Ace {
#ifndef CROSS_PLATFORM
ImageAnimatorModel* GetImageAnimatorModelImpl()
{
    static auto loader = DynamicModuleHelper::GetInstance().GetLoaderByName("image-animator");
    static ImageAnimatorModel* instance =
        loader ? reinterpret_cast<ImageAnimatorModel*>(loader->CreateModel()) : nullptr;
    return instance;
}
#endif
} // namespace OHOS::Ace
...
void CreateModel()
{
    ImageAnimatorModelNG::CreateImageAnimator();
}
...
void SetJSImages(ArkUINodeHandle node, struct ArkUIImageJSProperties* images, int32_t length)
{
    auto* frameNode = GetFrameNode(node);
    CHECK_NULL_VOID(frameNode);
    if (length == 0) {
    ...
    std::vector<ImageProperties> imageList;
    for (int32_t i = 0; i < length; i++) {
        ArkUIImageJSProperties* image = images + i;
        CHECK_NULL_VOID(image);
        CalcDimension dimension[IMAGES_LENGTH];
        ParseImage(dimension, IMAGES_LENGTH, reinterpret_cast<const ArkUIImagePropertiesStruct*>(image));
        ImageProperties property { std::string(image->src), nullptr,
            std::string(image->bundleName ? image->bundleName : ""),
            std::string(image->moduleName ? image->moduleName : ""), dimension[0], dimension[1], dimension[2],
            dimension[3], image->duration };
        if (image->pixelMap) {
            property.pixelMap = reinterpret_cast<PixelMap*>(image->pixelMap);
        }
        imageList.emplace_back(property);
    }
    ImageAnimatorModelNG::SetImages(frameNode, imageList);
...
{
    GetImageAnimatorModelImpl()->Create();
}

void SetImagesImpl(ArkUINodeHandle node, struct ArkUIImagePropertiesStruct* images, int32_t length)
{
    if (length == 0) {
        GetImageAnimatorModelImpl()->SetImages(std::vector<ImageProperties>());
        return;
    }
    CHECK_NULL_VOID(images);
    if (length <= 0) {
    ...
    GetImageAnimatorModelImpl()->SetImages(imageList);
}

void SetStateImpl(ArkUINodeHandle node, int32_t state)
{
    int32_t value = state;
    if (value < static_cast<int32_t>(Animator::Status::IDLE) ||
        value > static_cast<int32_t>(Animator::Status::STOPPED)) {
    ...
    GetImageAnimatorModelImpl()->SetState(value);
}

void SetDurationImpl(ArkUINodeHandle node, int32_t duration)
{
    if (duration < 0) {
        duration = DEFAULT_DURATION;
    }

    GetImageAnimatorModelImpl()->SetDuration(duration);
}

void SetImageAnimatorIterationImpl(ArkUINodeHandle node, int32_t value)
{
    int32_t iteration = value;
    if (iteration < -1) {
        iteration = DEFAULT_ITERATIONS;
    ...
    GetImageAnimatorModelImpl()->SetIteration(iteration);
}

void SetFillModeImpl(ArkUINodeHandle node, int32_t fillMode)
{
    int32_t value = fillMode;
    if (value < static_cast<int32_t>(FillMode::NONE) || value > static_cast<int32_t>(FillMode::BOTH)) {
        value = static_cast<int32_t>(DEFAULT_FILL_MODE);
...
void SetFixedSizeImpl(ArkUINodeHandle node, uint32_t fixedSize)
{
    GetImageAnimatorModelImpl()->SetFixedSize(static_cast<bool>(fixedSize));
}

void SetAutoMonitorInvisibleAreaImpl(ArkUINodeHandle node, ArkUI_Bool value)
{
    GetImageAnimatorModelImpl()->SetAutoMonitorInvisibleArea(value);
}

void SetImageAnimatorOnStartImpl(ArkUINodeHandle node, void* callback)
{
    if (callback) {
        auto onStart = reinterpret_cast<std::function<void()>*>(callback);
        GetImageAnimatorModelImpl()->SetOnStart(std::move(*onStart));
    } else {
        GetImageAnimatorModelImpl()->SetOnStart(nullptr);
    }
}

void SetImageAnimatorOnPauseImpl(ArkUINodeHandle node, void* callback)
{
    if (callback) {
        auto onPause = reinterpret_cast<std::function<void()>*>(callback);
        GetImageAnimatorModelImpl()->SetOnPause(std::move(*onPause));
    } else {
        GetImageAnimatorModelImpl()->SetOnPause(nullptr);
    }
}

void SetImageAnimatorOnRepeatImpl(ArkUINodeHandle node, void* callback)
{
    if (callback) {
        auto onRepeat = reinterpret_cast<std::function<void()>*>(callback);
        GetImageAnimatorModelImpl()->SetOnRepeat(std::move(*onRepeat));
    } else {
        GetImageAnimatorModelImpl()->SetOnRepeat(nullptr);
    }
}

void SetImageAnimatorOnCancelImpl(ArkUINodeHandle node, void* callback)
{
    if (callback) {
        auto onCancel = reinterpret_cast<std::function<void()>*>(callback);
        GetImageAnimatorModelImpl()->SetOnCancel(std::move(*onCancel));
    } else {
        GetImageAnimatorModelImpl()->SetOnCancel(nullptr);
    }
}

void SetImageAnimatorOnFinishImpl(ArkUINodeHandle node, void* callback)
{
    if (callback) {
        auto onFinish = reinterpret_cast<std::function<void()>*>(callback);
        GetImageAnimatorModelImpl()->SetOnFinish(std::move(*onFinish));
    } else {
        GetImageAnimatorModelImpl()->SetOnFinish(nullptr);
    }
}
#endif
...
const ArkUIImageAnimatorModifier* GetImageAnimatorDynamicModifier()
{
    ...
    if (!isCurrentUseNewPipeline) {
        ...
        static const ArkUIImageAnimatorModifier modifier = {
            .createModel = ImageAnimatorModifier::CreateModelImpl,
            .setState = ImageAnimatorModifier::SetStateImpl,
            .resetState = nullptr,
            .setDuration = ImageAnimatorModifier::SetDurationImpl,
            .setFixedSize = ImageAnimatorModifier::SetFixedSizeImpl,
            ...
            .resetImages = nullptr,
            .setImageAnimatorIteration = ImageAnimatorModifier::SetImageAnimatorIterationImpl,
            .resetImageAnimatorIteration = nullptr,
            .setAutoMonitorInvisibleArea = ImageAnimatorModifier::SetAutoMonitorInvisibleAreaImpl,
            .setImageAnimatorSrc = nullptr,
            .setJSImages = nullptr,
            .resetDuration = nullptr,
            .getIsReverse = nullptr,
            .getDuration = nullptr,
            .getState = nullptr,
            ...
            .resetImageAnimatorOnStart = nullptr,
            .setImageAnimatorOnPause = ImageAnimatorModifier::SetImageAnimatorOnPauseImpl,
            .resetImageAnimatorOnPause = nullptr,
            .setImageAnimatorOnRepeat = ImageAnimatorModifier::SetImageAnimatorOnRepeatImpl,
            .resetImageAnimatorOnRepeat = nullptr,
            .setImageAnimatorOnCancel = ImageAnimatorModifier::SetImageAnimatorOnCancelImpl,
            .resetImageAnimatorOnCancel = nullptr,
            .setImageAnimatorOnFinish = ImageAnimatorModifier::SetImageAnimatorOnFinishImpl,
            .resetImageAnimatorOnFinish = nullptr,
            .setImageAnimatorOnStartExt = nullptr,
            .setImageAnimatorOnPauseExt = nullptr,
            .setImageAnimatorOnRepeatExt = nullptr,
            .setImageAnimatorOnCancelExt = nullptr,
            .setImageAnimatorOnFinishExt = nullptr,
            .createImageAnimatorFrameNode = ImageAnimatorModifier::CreateImageAnimatorFrameNode,
        };
    ...
    CHECK_INITIALIZED_FIELDS_BEGIN(); // don't move this line
    static const ArkUIImageAnimatorModifier modifier = {
        .createModel = ImageAnimatorModifier::CreateModel,
        ...
        .setJSImages = ImageAnimatorModifier::SetJSImages,
        ...
        .setImageAnimatorOnStartExt = ImageAnimatorModifier::SetImageAnimatorOnStartExt,
        .setImageAnimatorOnPauseExt = ImageAnimatorModifier::SetImageAnimatorOnPauseExt,
        .setImageAnimatorOnRepeatExt = ImageAnimatorModifier::SetImageAnimatorOnRepeatExt,
        .setImageAnimatorOnCancelExt = ImageAnimatorModifier::SetImageAnimatorOnCancelExt,
        .setImageAnimatorOnFinishExt = ImageAnimatorModifier::SetImageAnimatorOnFinishExt,
        .createImageAnimatorFrameNode = ImageAnimatorModifier::CreateImageAnimatorFrameNode,
    };
    CHECK_INITIALIZED_FIELDS_END(modifier, 0, 0, 0); // don't move this line
