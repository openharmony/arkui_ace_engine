...
void ControlledAnimator::PostPlayTask(int32_t idx, int32_t iteration, int32_t idxOffset)
{
    if (iteration >= iteration_ && iteration_ != ANIMATION_INFINITE) {
        Finish();
        return;
    }
    ACE_SCOPED_TRACE("ControlledAnimator postPlayTask %d-%d-%d", idx, iteration, idxOffset);
    auto runningTime = static_cast<int32_t>(pictureInfos_[idx].first * GetDuration());
    runningIdx_ = idx;
    ...
    NG::ImageUtils::PostDelayedTaskToUI(
        currentPostTask_, runningTime, "ArkUIImageAnimatorPostPlayTask");
}
