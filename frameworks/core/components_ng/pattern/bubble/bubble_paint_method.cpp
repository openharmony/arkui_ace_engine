...
void BubblePaintMethod::PaintBubble(RSCanvas& canvas, PaintWrapper* paintWrapper)
{
    ...
    littleSpace_ = static_cast<float>(popupTheme->GetLittleSpace().ConvertToPx());
...
void BubblePaintMethod::PaintTopBubble(RSCanvas& rsCanvas)
{
    ...
    float childOffsetY = childOffset_.GetY() - littleSpace_;
    float arrowPositionX = arrowPosition_.GetX();
    float arrowPositionY = arrowPosition_.GetY() - littleSpace_;
...
void BubblePaintMethod::BuildCompletePath(RSPath& path)
{
    ...
    path.Reset();
    path.MoveTo(childOffset_.GetX() + radiusPx, childOffset_.GetY() - littleSpace_);
