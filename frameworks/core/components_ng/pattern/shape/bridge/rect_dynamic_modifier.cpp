...
Framework::RectModelImpl* GetRectModelImpl()
{
...
#ifndef CROSS_PLATFORM
void SetRectRadiusWidthImpl(
    ArkUINodeHandle node, ArkUI_Float32 radiusWidthValue, ArkUI_Int32 radiusWidthUnit, void* resObjPtr)
{
    Dimension radiusWidth(radiusWidthValue, static_cast<DimensionUnit>(radiusWidthUnit));
    GetRectModelImpl()->SetRadiusWidth(radiusWidth);
}

void ResetRectRadiusWidthImpl(ArkUINodeHandle node)
{
    Dimension defaultDimension;
    GetRectModelImpl()->SetRadiusWidth(defaultDimension);
}

void SetRectRadiusHeightImpl(
    ArkUINodeHandle node, ArkUI_Float32 radiusHeightValue, ArkUI_Int32 radiusHeightUnit, void* resObjPtr)
{
    Dimension radiusHeight(radiusHeightValue, static_cast<DimensionUnit>(radiusHeightUnit));
    GetRectModelImpl()->SetRadiusHeight(radiusHeight);
}

void ResetRectRadiusHeightImpl(ArkUINodeHandle node)
{
    Dimension defaultDimension;
    GetRectModelImpl()->SetRadiusHeight(defaultDimension);
}

void SetRectRadiusWithArrayImpl(ArkUINodeHandle node, ArkUI_Float32* radiusValues, ArkUI_Int32* radiusUnits,
    ArkUI_Uint32* radiusValidPairs, ArkUI_Uint32 radiusValidPairsSize, void* radiusXResObjArray,
    void* radiusYResObjArray)
{
    CHECK_NULL_VOID(radiusValues);
    CHECK_NULL_VOID(radiusUnits);
    CHECK_NULL_VOID(radiusValidPairs);
    ResetRectRadiusWidthImpl(node);
    ResetRectRadiusHeightImpl(node);
    for (size_t index = 0; index < radiusValidPairsSize; index++) {
        if (radiusValidPairs[index] != VALID_RADIUS_PAIR_FLAG) {
            continue;
        }
        ...
        Dimension radiusXDimension(radiusX.Value(), radiusX.Unit());
        Dimension radiusYDimension(radiusY.Value(), radiusY.Unit());
        GetRectModelImpl()->SetRadiusValue(radiusXDimension, radiusYDimension, index);
    }
}

void SetRectRadiusWithValueImpl(
    ArkUINodeHandle node, ArkUI_Float32 radiusValue, ArkUI_Int32 radiusUnit, void* resObjPtr)
{
    CalcDimension radius(radiusValue, static_cast<DimensionUnit>(radiusUnit));
    Dimension radiusDimension(radius.Value(), radius.Unit());
    GetRectModelImpl()->SetRadiusWidth(radiusDimension);
    GetRectModelImpl()->SetRadiusHeight(radiusDimension);
}

void ResetRectRadiusImpl(ArkUINodeHandle node)
{
    Dimension defaultDimension;
    GetRectModelImpl()->SetRadiusWidth(defaultDimension);
    GetRectModelImpl()->SetRadiusHeight(defaultDimension);
...
const ArkUIRectModifier* GetRectDynamicModifier()
{
    ...
    if (!isCurrentUseNewPipeline) {
        ...
        static const ArkUIRectModifier modifier = {
            .createRect = CreateRectImpl,
            .createRectFrameNode = nullptr,
            .setRectRadiusWidth = SetRectRadiusWidthImpl,
            .resetRectRadiusWidth = ResetRectRadiusWidthImpl,
    ...
    static const ArkUIRectModifier modifier = {
        ...
        .createRectFrameNode = CreateRectFrameNode,
