...
void CircleBridge::RegisterCircleAttributes(Local<ObjectRef> object, EcmaVM* vm)
{
    const char* functionNames[] = { "create" };

    Local<JSValueRef> funcValues[] = {
        panda::FunctionRef::New(const_cast<panda::EcmaVM*>(vm), CircleBridge::CreateCircle),
    };

    auto circle = panda::ObjectRef::NewWithNamedProperties(vm, ArraySize(functionNames), functionNames, funcValues);
    object->Set(vm, panda::StringRef::NewFromUtf8(vm, "circle"), circle);
}

ArkUINativeModuleValue CircleBridge::CreateCircle(ArkUIRuntimeCallInfo* runtimeCallInfo)
{
    EcmaVM* vm = runtimeCallInfo->GetVM();
    CHECK_NULL_RETURN(vm, panda::NativePointerRef::New(vm, nullptr));
    Local<JSValueRef> firstArg = runtimeCallInfo->GetCallArgRef(NUM_0);
    GetArkUINodeModifiers()->getCircleModifier()->createCircle();
