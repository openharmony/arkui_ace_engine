...
void SetEllipseOptionsImpl(Ark_NativePointer node, const Opt_EllipseOptions* options)
{
    auto frameNode = reinterpret_cast<FrameNode *>(node);
    CHECK_NULL_VOID(frameNode);
    auto ellipseOptions = Converter::OptConvertPtr<Converter::EllipseOptions>(options);
    if (ellipseOptions && ellipseOptions.value().width) {
        ShapeAbstractModelNG::SetWidth(frameNode, ellipseOptions.value().width.value());
    }
    if (ellipseOptions && ellipseOptions.value().height) {
        ShapeAbstractModelNG::SetHeight(frameNode, ellipseOptions.value().height.value());
    }
}
