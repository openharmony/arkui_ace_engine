...
RefPtr<NodePaintMethod> PathPattern::CreateNodePaintMethod()
{
...
RefPtr<PaintProperty> PathPattern::CreatePaintProperty()
{
    return MakeRefPtr<PathPaintProperty>();
}

RefPtr<LayoutAlgorithm> PathPattern::CreateLayoutAlgorithm()
{
    return MakeRefPtr<PathLayoutAlgorithm>(GetAncestorPaintProperty());
}

void PathPattern::SetClipBounds()
{
    auto host = GetHost();
