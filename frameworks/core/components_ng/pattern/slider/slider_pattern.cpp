...
void SliderPattern::StartVibrateFeedback()
{
    ...
    if (valueChangeFlag_ && reachBoundary_) {
        ...
        TAG_LOGD(AceLogTag::ACE_SELECT_COMPONENT, "slider StartVibrateFeedback %{public}s",
            CROWN_VIBRATOR_STRONG);
...
SliderContentModifier::Parameters SliderPattern::UpdateContentParameters()
{
    ...
    auto sliderMode = sliderLayoutProperty->GetSliderMode().value_or(SliderModel::SliderMode::OUTSET);
    Color trackColor = (sliderMode == SliderModel::SliderMode::OUTSET || sliderMode == SliderModel::SliderMode::NONE)
                           ? theme->GetOutsetAndNoneTrackColor()
