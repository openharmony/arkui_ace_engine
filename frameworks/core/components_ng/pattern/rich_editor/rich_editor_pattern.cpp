...
void RichEditorPattern::AddSpansByPaste(const RefPtr<SpanString>& spanString)
{
    CHECK_NULL_VOID(spanString);
    const auto& spans = spanString->GetSpanItems();
    AddSpansByPaste(spans);
}
    ...
    bool shouldCommitInput)
{
    ...
    InsertValueInner(text, operationType, shouldCommitInput);
}

void RichEditorPattern::InsertValueInner(const std::u16string& text, OperationType operationType,
    bool shouldCommitInput)
{
    OperationRecord record;
...
void RichEditorPattern::DeleteBackward(int32_t oriLength)
{
    ...
    DeleteBackwardInner(length);
}

void RichEditorPattern::DeleteBackwardInner(int32_t length)
{
    OperationRecord record;
...
void RichEditorPattern::DeleteForward(int32_t oriLength)
{
    ...
    TAG_LOGD(AceLogTag::ACE_RICH_TEXT, "oriLength=%{public}d, length=%{public}d, isDragging=%{public}d",
        oriLength, length, IsDragging());
    CHECK_NULL_VOID(!IsDragging());
    DeleteForwardInner(length);
}

void RichEditorPattern::DeleteForwardInner(int32_t length)
{
    if (IsPreviewTextInputting()) {
...
void RichEditorPattern::HandleDragFromStyledString(const RefPtr<SpanString>& spanString, bool isCopy)
{
...
void RichEditorPattern::HandleOnDragDropStyledString(const RefPtr<OHOS::Ace::DragEvent>& event, bool isCopy)
{
    ...
    if (!arr.empty()) {
        ...
        if (!spanStr->GetSpanItems().empty()) {
            HandleDragFromStyledString(spanStr, isCopy);
            return;
        }
    }
...
bool RichEditorPattern::HandleOnDeleteComb(bool backward)
{
    ...
    IF_PRESENT(undoManager_, RecordSelectionBefore());
    if (textSelector_.IsValid()) {
        CloseSelectOverlay();
        SetCaretPosition(textSelector_.GetTextStart());
        ResetSelection();
    ...
    IF_PRESENT(undoManager_, ClearSelectionBefore());
    MoveCaretToContentRect();
    StartTwinkling();
    auto host = GetHost();
    CHECK_NULL_RETURN(host, false);
...
void RichEditorPattern::DeleteRange(int32_t start, int32_t end, bool isIME)
{
    ...
    auto length = end - start;
    DeleteRangeInner(start, length);
}

void RichEditorPattern::DeleteRangeInner(int32_t start, int32_t length)
{
    OperationRecord record;
    record.beforeCaretPosition = caretPosition_;
    RichEditorChangeValue changeValue;
