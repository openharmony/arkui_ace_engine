...
void QRCodeModifier::onDraw(DrawingContext& context)
{
    ...
    // 2.Calculate the max block of the QRcode. Ratio = qrCodeSize_ / qrWidth.
    // 3.Scale the qrcode ,the scale ratio is qrCodeSize_ / (blockNumber * qrWidth)
    ...
    int32_t width, const QrcodeImage& qrCode, const Color& color, const Color& backgroundColor) const
{
    ...
    for (int32_t i = 0; i < width; i++) {
        for (int32_t j = 0; j < width; j++) {
            if (QrcodeGetModule(&qrCode, j / blockWidth, i / blockWidth)) {
