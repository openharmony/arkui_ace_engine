...
void ValidateGridContainerSizeNonNegative(std::optional<V2::GridContainerSize>& value)
{
    if (!value.has_value()) {
        return;
    }
    auto val = value.value();
    if (Negative(val.lg) || Negative(val.md) || Negative(val.sm) || Negative(val.xl) || Negative(val.xs) ||
        Negative(val.xxl)) {
        value.reset();
    }
}

std::optional<V2::GridContainerSize> ConvertGridContainerSizeFromInt(int32_t value)
{
    V2::GridContainerSize result;
    result.xs = value;
    result.sm = value;
    result.md = value;
    result.lg = value;
...
std::optional<V2::GridContainerSize> ParseGridRowColumns(const Opt_Union_I32_GridRowColumnOption& value)
{
    auto optValue = Converter::GetOpt(value);
    ...
    switch (optValue->selector) {
        case 0:
            return ConvertGridContainerSizeFromInt(Converter::Convert<int32_t>(optValue->value0));
        case 1:
            return Converter::Convert<V2::GridContainerSize>(optValue->value1);
        default:
            return std::nullopt;
    }
}
...
void ValidateNonNegative(std::optional<V2::Gutter>& value)
{
    if (value.has_value()) {
        auto val = value.value();
        bool fail = val.xLg.IsNegative() || val.yLg.IsNegative() || val.xMd.IsNegative() || val.yMd.IsNegative() ||
            val.xSm.IsNegative() || val.ySm.IsNegative() || val.xXl.IsNegative() || val.yXl.IsNegative() ||
            val.xXs.IsNegative() || val.yXs.IsNegative() || val.xXXl.IsNegative() || val.yXXl.IsNegative();
        if (fail) {
            value.reset();
        }
    }
...
void SetGridRowOptionsImpl(Ark_NativePointer node, const Opt_GridRowOptions* option)
{
    ...
    if (convValue.has_value()) {
        ...
        auto breakpoints = optBreakPoints.has_value()
            ? AceType::MakeRefPtr<V2::BreakPoints>(optBreakPoints.value())
            : nullBreakPoints;
        breakpoints->userDefine = optBreakPoints.has_value();
        GridRowModelNG::SetBreakpoints(frameNode, breakpoints);

        auto optColumns = ParseGridRowColumns(arkOptions.columns);
        ValidateGridContainerSizeNonNegative(optColumns);
        auto columns = optColumns.has_value()
            ? AceType::MakeRefPtr<V2::GridContainerSize>(optColumns.value())
            : nullGridContainerSize;
...
void SetOnBreakpointChangeImpl(Ark_NativePointer node, const Opt_synthetic_Callback_String_Void* value)
{
    auto frameNode = reinterpret_cast<FrameNode*>(node);
    CHECK_NULL_VOID(frameNode);
...
void SetAlignItemsImpl(Ark_NativePointer node, const Opt_ItemAlign* value)
{
    auto frameNode = reinterpret_cast<FrameNode*>(node);
    CHECK_NULL_VOID(frameNode);
    std::optional<FlexAlign> convValue;
    auto optValue = Converter::GetOptPtr(value);
    if (optValue.has_value()) {
        switch (optValue.value()) {
            case ARK_ITEM_ALIGN_AUTO:
                convValue = FlexAlign::AUTO;
                break;
            case ARK_ITEM_ALIGN_START:
                convValue = FlexAlign::FLEX_START;
                break;
            case ARK_ITEM_ALIGN_CENTER:
                convValue = FlexAlign::CENTER;
                break;
            case ARK_ITEM_ALIGN_END:
                convValue = FlexAlign::FLEX_END;
                break;
            case ARK_ITEM_ALIGN_BASELINE:
                convValue = FlexAlign::BASELINE;
                break;
            case ARK_ITEM_ALIGN_STRETCH:
                convValue = FlexAlign::STRETCH;
                break;
            default:
                break;
        }
...
{
    static const GENERATED_ArkUIGridRowModifier modifier {
