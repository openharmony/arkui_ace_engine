...
void SwiperLayoutAlgorithm::Layout(LayoutWrapper* layoutWrapper)
{
    ...
    for (auto index = startIndex_.value(); index <= endIndex_.value(); ++index) {
        ...
        if (axis == Axis::HORIZONTAL) {
            offset += OffsetF(size.Width() * (index - currentIndex_), 0.0f);
        } else if (axis == Axis::VERTICAL) {
            offset += OffsetF(0.0f, size.Height() * (index - currentIndex_));
        } else {
        ...
        LOGE("CCCC index: %{public}d, offset: %{public}s, parentOffset: %{public}s", index, offset.ToString().c_str(), parentOffset.ToString().c_str());
    }
}
