...
#include "frameworks/core/interfaces/native/node/node_arc_swiper_modifier.h"
...
void* GetArcAsyncEventHandlers(uint32_t kind)
{
    static const ComponentAsyncEventHandler arcSwiperNodeAsyncEventHandlers[] = {
        SetArcSwiperChange,
        SetArcSwiperAnimationStart,
        SetArcSwiperAnimationEnd,
        SetArcSwiperGestureSwipe,
    };
    if (kind >= sizeof(arcSwiperNodeAsyncEventHandlers) / sizeof(ComponentAsyncEventHandler)) {
        TAG_LOGE(AceLogTag::ACE_NATIVE_NODE, "NotifyComponentAsyncEvent kind:%{public}d NOT IMPLEMENT", kind);
        return nullptr;
    }
    return reinterpret_cast<void*>(arcSwiperNodeAsyncEventHandlers[kind]);
}
...
const ArkUIArcSwiperModifier* GetArcSwiperModifier()
{
    static const ArkUIArcSwiperModifier modifier = {
        ...
        .getAsyncEventHandlers = GetArcAsyncEventHandlers,
    };
    return &modifier;
}
} // namespace NodeModifier
