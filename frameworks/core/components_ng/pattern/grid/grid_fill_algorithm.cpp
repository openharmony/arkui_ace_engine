...
{
    params_.mainGap = GridUtils::GetMainGap(AceType::Claim(&props_), viewport, axis);
    ...
    auto res = ParseTemplateArgs(GridUtils::ParseArgs(args), viewport.CrossSize(axis),
        GridUtils::GetCrossGap(AceType::Claim(&props_), viewport, axis), totalCnt);
...
{
    GridIrregularFiller filler(&info_, nullptr);
