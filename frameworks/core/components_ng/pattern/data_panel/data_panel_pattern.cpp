...
RefPtr<NodePaintMethod> DataPanelPattern::CreateNodePaintMethod()
{
    ...
    float width = std::max<float>(
        ...
        shadowOption.offsetX - x + frameSize.Width() + shadowOption.radius * 2,
        std::abs(shadowOption.offsetX) + frameSize.Width() + shadowOption.radius * 2 }
    );
    float height = std::max<float>(
        { oldBoundsRect->Height(), oldBoundsRect->GetY() + oldBoundsRect->Height() - y,
        shadowOption.offsetY - y + frameSize.Height() + shadowOption.radius * 2,
        std::abs(shadowOption.offsetY) + frameSize.Height() + shadowOption.radius * 2 }
    );
