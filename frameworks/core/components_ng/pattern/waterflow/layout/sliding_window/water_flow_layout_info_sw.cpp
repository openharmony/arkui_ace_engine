...
bool WaterFlowLayoutInfoSW::OutOfBounds() const
{
    ...
    if (itemStart_ && Positive(sections_[0][0].startPos - TopMargin())) {
        return true;
    }
    if (offsetEnd_) {
        return std::all_of(sections_.back().begin(), sections_.back().end(), [this](const Lane& lane) {
...
void WaterFlowLayoutInfoSW::PrepareSection(int32_t idx, bool forward)
{
    ...
    if (prevSeg < curSeg) {
        for (int32_t i = prevSeg + 1; i <= curSeg; ++i) {
            float pos = SectionEndPos(sections_[i - 1]);
            ...
            std::for_each(sections_[i].begin(), sections_[i].end(), [pos](Lane& lane) {
        ...
        for (int32_t i = prevSeg - 1; i >= curSeg; --i) {
            float pos = SectionStartPos(sections_[i + 1]);
