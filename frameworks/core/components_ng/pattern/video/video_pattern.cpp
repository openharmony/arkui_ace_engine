...
PositionUpdatedEvent BuildPositionUpdatedEvent(const WeakPtr<VideoPattern>& weak,
...
StateChangedEvent BuildStateChangedEvent(const WeakPtr<VideoPattern>& weak,
    const SingleTaskExecutor& uiTaskExecutor, int32_t instanceId)
{
...
CommonEvent BuildErrorEvent(const WeakPtr<VideoPattern>& weak, const SingleTaskExecutor& uiTaskExecutor,
    int32_t instanceId)
{
...
VideoErrorEvent BuildVideoErrorEvent(const WeakPtr<VideoPattern>& weak, const SingleTaskExecutor& uiTaskExecutor,
    int32_t instanceId)
{
...
CommonEvent BuildResolutionChangeEvent(const WeakPtr<VideoPattern>& weak,
    const SingleTaskExecutor& uiTaskExecutor, int32_t instanceId)
{
    return [weak, uiTaskExecutor, instanceId]() {
...
CommonEvent BuildStartRenderFrameEvent(const WeakPtr<VideoPattern>& weak,
    const SingleTaskExecutor& uiTaskExecutor, int32_t instanceId)
{
    return [weak, uiTaskExecutor, instanceId]() {
        uiTaskExecutor.PostSyncTask([weak, instanceId] {
...
SeekDoneEvent BuildSeekDoneEvent(const WeakPtr<VideoPattern>& weak, const SingleTaskExecutor& uiTaskExecutor,
    int32_t instanceId)
{
    return [weak, uiTaskExecutor, instanceId](uint32_t currentPos) {
...
void VideoPattern::ResetMediaPlayerOnBg()
{
    ...
    bgTaskExecutor.PostTask(
        ...
        TAG_LOGI(AceLogTag::ACE_VIDEO, "Video[%{public}d] trigger mediaPlayer PrepareAsync", hostId);
...
void VideoPattern::ResetMediaPlayer()
{
    ...
    PrepareSurface();
    TAG_LOGI(AceLogTag::ACE_VIDEO, "Video[%{public}d] trigger mediaPlayer PrepareAsync", hostId_);
...
void VideoPattern::checkNeedAutoPlay()
{
    ...
    if (autoPlay_) {
        TAG_LOGI(AceLogTag::ACE_VIDEO, "Video[%{public}d] Start by checkNeedAutoPlay", hostId_);
        Start();
    }
}
    ...
    const RefPtr<VideoController>& videoController, const SingleTaskExecutor& uiTaskExecutor)
{
    videoController->SetStartImpl([weak = WeakClaim(this), uiTaskExecutor]() {
        uiTaskExecutor.PostTask([weak]() {
            ...
            TAG_LOGI(AceLogTag::ACE_VIDEO, "Video[%{public}d] Start by VideoController", targetPattern->hostId_);
            targetPattern->Start();
            }, "ArkUIVideoStart");
    });
}
    ...
    const RefPtr<VideoController>& videoController, const SingleTaskExecutor& uiTaskExecutor)
{
    videoController->SetPausetImpl([weak = WeakClaim(this), uiTaskExecutor]() {
        uiTaskExecutor.PostTask([weak]() {
            ...
            auto targetPattern = pattern->GetTargetVideoPattern();
            CHECK_NULL_VOID(targetPattern);
            TAG_LOGI(AceLogTag::ACE_VIDEO, "Video[%{public}d] Pause by VideoController", targetPattern->hostId_);
            targetPattern->Pause();
            }, "ArkUIVideoPause");
    });
}
...
void VideoPattern::Start()
{
    ...
    if (isStop_) {
        TAG_LOGI(AceLogTag::ACE_VIDEO, "Video[%{public}d] trigger mediaPlayer PrepareAsync from Start", hostId_);
        if (mediaPlayer_->PrepareAsync() != 0) {
...
void VideoPattern::Pause()
{
    ...
    if (ret != 0) {
        TAG_LOGW(AceLogTag::ACE_VIDEO, "Video[%{public}d] mediaPlayer Pause failed: ret=%{public}d", hostId_, ret);
    }
...
void VideoPattern::Stop()
{
    ...
    auto ret = mediaPlayer_->Stop();
    if (ret != 0) {
        TAG_LOGW(AceLogTag::ACE_VIDEO, "Video[%{public}d] mediaPlayer Stop failed: ret=%{public}d", hostId_, ret);
    }
    isStop_ = true;
    SetIsSeeking(false);
}
