...
using UiLayoutTask = CancelableCallback<void()>;
using UiRenderTask = CancelableCallback<void()>;
...
public:
    ...
    // Called to sync render property like background color and so on.
    void FlushModifyTask(StateModifyTask& stateModifyTask);

    std::shared_ptr<UiLayoutTask> CreateUiLayoutTask();

    std::shared_ptr<UiLayoutTask> CreateUiRenderTask();
    ...
    const RefPtr<GeometryNode>& GetGeometryNodeSafe()
    {
