...
void UIDisplaySync::DelFromPipeline(WeakPtr<PipelineBase>& pipelineContext)
{
    ...
    if (!context) {
        TAG_LOGD(AceLogTag::ACE_DISPLAY_SYNC, "[DisplaySync] DelFromPipeline CurrentContext is nullptr.");
        return;
    }

    RefPtr<UIDisplaySyncManager> dsm = context->GetOrCreateUIDisplaySyncManager();
    if (!dsm) {
        TAG_LOGD(AceLogTag::ACE_DISPLAY_SYNC, "[DisplaySync] DSM is nullptr.");
        return;
    }
