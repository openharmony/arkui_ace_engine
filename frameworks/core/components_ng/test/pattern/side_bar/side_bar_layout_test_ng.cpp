 ...
 * @tc.desc: Test SideBar InitRealSideBarWidth
 * @tc.type: FUNC
 */
HWTEST_F(SideBarLayoutTestNg, SideBarLayoutTestNg030, TestSize.Level1)
{
    ...
    ASSERT_NE(layoutProperty, nullptr);
    auto layoutConstraint = LayoutConstraintF();
    layoutConstraint.selfIdealSize.SetWidth(1);
    layoutConstraint.selfIdealSize.SetHeight(1);
    layoutProperty->layoutConstraint_ = layoutConstraint;
    layoutAlgorithm->adjustMaxSideBarWidth_ = DEFAULT_MIN_SIDE_BAR_WIDTH;
    layoutAlgorithm->SetPattern(AceType::WeakClaim(AceType::RawPtr(sideBarContainerPattern)));
    layoutAlgorithm->preSideBarWidth_ = Dimension(1.0f, DimensionUnit::PX);
    layoutAlgorithm->InitRealSideBarWidth(&layoutWrapper, parentWidth);
    EXPECT_TRUE(layoutProperty->layoutConstraint_);
}
...
HWTEST_F(SideBarLayoutTestNg, SideBarLayoutTestNg031, TestSize.Level1)
{
    ...
    ASSERT_NE(pipeline, nullptr);
    pipeline->minPlatformVersion_ = 12;
    layoutAlgorithm->needInitRealSideBarWidth_ = false;
    layoutAlgorithm->typeUpdateWidth_ = 50000.0f;
    layoutAlgorithm->type_ = SideBarContainerType::EMBED;
    layoutAlgorithm->realSideBarWidthDimension_ = Dimension(5.0f, DimensionUnit::PERCENT);
    layoutAlgorithm->Measure(&layoutWrapper);
    layoutAlgorithm->isControlButtonClick_ = true;
    layoutAlgorithm->Measure(&layoutWrapper);
    layoutAlgorithm->type_ = SideBarContainerType::OVERLAY;
    layoutAlgorithm->Measure(&layoutWrapper);
    EXPECT_TRUE(layoutAlgorithm->isControlButtonClick_);
}
} // namespace OHOS::Ace::NG
