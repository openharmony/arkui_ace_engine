...
public:
    ...
    void CheckPoints(ShapePoint* startPoint, ShapePoint* endPoint)
    {
        ...
        if (hasStart) {
            ...
            EXPECT_FLOAT_EQ(lineProperty->GetStartPointValue().second.ConvertToPx(),
                DEFAULT_POINT.second.ConvertToPx());
        }
        if (hasEnd) {
            ...
            EXPECT_FLOAT_EQ(lineProperty->GetEndPointValue().second.ConvertToPx(),
                DEFAULT_POINT.second.ConvertToPx());
        }
        if (hasStart && hasEnd) {
            Draw(frameNode);
    ...
    void MeasureContentTest(ShapePoint* startPoint, ShapePoint* endPoint,
        bool bVaild = true, bool bSetStrokeWidth = true)
    {
        auto lineModelNg = LineModelNG();
        lineModelNg.Create();
        auto frameNode = AceType::DynamicCast<FrameNode>(ViewStackProcessor::GetInstance()->GetMainElementNode());
